...
| **G19** | **Dark-on-prod activations** — six packs (candidate v1 on prod), three owner-approved revenue pins, `cockpit-agent` v25/v26 candidates, `media-director` v3 | 08-24 note step 1 | **Open.** Hours, not weeks: restore OpenAI credit, run gates, `activateSkill` per deployment. |
...
| **G22** | **Money-in lane parked** — invoice reminders, call list, lead triage, cash flow exist as skills; connectors exist as code; all parked by owner choice 08-31 | Phase 28 | **Open.** Unpark ONE connector (Stripe read or QuickBooks) + invoice reminders; it is the only capability whose outcome is a bank deposit. |
