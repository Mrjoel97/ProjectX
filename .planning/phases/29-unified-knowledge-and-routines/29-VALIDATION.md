...
| Criterion | Status |
|---|---|
| Exact-version **activation** of a tenant pack candidate | **STILL NOT MET, and deliberately so.** `planTenantActivation` refuses every `pack-*` with `PACK_GATE`, fail-closed. The recorded price of lifting it is unchanged — `provenance` and `browserEvidence` columns on `tenantSkills` plus a tenant-scoped pack eval runner. The owner chose the run-time VALUES route for the user-facing gap instead (row below), which needs none of that, so this row was not bought. **Its ROOT BLOCKER is gone though:** the eval gate that would have certified such a body used to certify pins it never checked, and no longer does (see `29-VERIFICATION.md`). |
