...
| Criterion | Status |
|---|---|
| Exact-version **activation** of a tenant pack candidate | **THE LANE NOW EXISTS AND IS EARNABLE (2026-08-31) — but nothing can yet earn it.** `assertTenantPackActivationEvidence` replaced the unconditional refusal and demands the SAME THREE PLANES a global pack body clears, keyed to the row id. **The price recorded here was overstated:** it named "`provenance` and `browserEvidence` columns plus a tenant-scoped pack eval runner", and measured, ONE column was owed. Provenance needs none — the row already stores `templateId`, `templateVersion`, `customizationValues` and `customizationHash`, so it is RECOMPUTED at activation, which is strictly stronger than the global plane (whose own docstring admits it never checks `bodySha256` against the body beside it). ⚠ **STILL NOT REACHABLE IN PRACTICE:** nothing PRODUCES tenant pack evidence — the golden runner refuses `pack-`-named rows and no browser gate emits a `tenantTarget`-pinned artifact. A smaller, better-defined job than it was; still a job. |
| **Rollback** of a tenant pack candidate | **NOT MET**, same reason — the gate it depends on is open now, the evidence producer it depends on is not. |
| A published customization taking effect | **MET 2026-08-30**, by the route that needed no new gate. `runWorkflowPack` reads the tenant's saved VALUES and renders them into the run through the approved template's own schema; `cockpit.ts` still passes no `tenantSkillIds` and `PACK_GATE` still refuses to activate the composed body. What a tenant edits is a closed schema of four settings, not a prompt — so the approved template stays the governing body and nothing new is certified. Ordering is the security property: code-owned source truth, then settings, then the untrusted request LAST. |
...
| `oauth-expiry-reauth` / `dst-boundary` / `provider-read` **live** evidence | **STILL ABSENT — and now much harder to fake.** The gate proved a ref named a real distinct file and nothing more, so `dst-boundary` citing the recurrence spike's unit test was one word (`automated` -> `live`) from a fabricated `enable-safe`. A `live` row must now cite an artifact `collect-recurrence-evidence.mjs` wrote for that row's own probe. All three still refuse: no real provider grant exists on any deployment here (every `gmailTokens` row is a fixture, `connectorConnections` empty), and a DST trace needs a run spanning a real transition. **THE DATE STATED HERE WAS WRONG BY SEVEN WEEKS.** It read "2026-10-25 (EU) / 2026-11-01 (US)" — the two a northern-hemisphere author reaches for first. Scanning all 418 IANA zones, the next transition is **2026-09-06** (America/Santiago, Pacific/Easter), and `nextTransition`/`soonestTransition` now DERIVE it from `Intl` so the answer cannot go stale or be wrong next year. `--eligibility` still exits 1. |
