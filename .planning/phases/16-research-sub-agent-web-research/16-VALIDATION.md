...
| SC#2 fence | the specialist's OUTPUT crosses back inside the labelled `<research_findings …>` fence (see D5-CORRECTED) | unit | core quick run (pure fence builder) | ❌ W0 | ⬜ |
...
| R4 envelope | two research dispatches in ONE executive turn share ONE envelope; the second starts where the first stopped | integration | `… dispatch.test.ts -t "envelope"` | ✅ extend `:339` | ⬜ |
