...
| 26-17 | 13 | RPRT-01 | synchronized filters, privacy DOM and board-pack download | from `apps/web`: `npx vitest run "app/(app)/dashboard/reports"`, `npx tsc --noEmit`, `npx next build`, then `npx playwright test e2e/reports.spec.ts --project=chromium` (the `--` form does NOT filter) | 🟨 Task 1 green, Task 2 (owner UAT) OPEN — component **22/22**, web typecheck + prod build clean, core 40/1096, backend typecheck clean, biome + watcher clean, and **`e2e/reports.spec.ts` EXECUTED 7/7 IN THE BROWSER** on a rebuilt `:3111`. The board pack is the one NON-seeded claim: `markdownToPdf` (pdf-lib, deterministic, no provider, no cent) and `ctx.storage.store` really ran, the href was a minted `https:` storage URL, and the second click returned "Already generated for this window" — 26-16's content-hash replay observed end to end. Three run-only lessons folded back in: wait for the window line by PATTERN not by "it changed"; never hardcode a timezone (the runner reports `Africa/Dar_es_Salaam`); and **`convex run` ENDS THE BROWSER SESSION**, so the owner bootstrap/revoke test must run LAST. Nav still dark pending the owner's verdict. |
