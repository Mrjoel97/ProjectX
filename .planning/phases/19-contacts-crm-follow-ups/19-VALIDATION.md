...
|----------|-------------|------------|--------|
| Connected browser UAT — BRAND conformance of the Pipeline page, the three never-seen send-refusal notes, the withheld report's TONE, the CRM plan card, the `Records update` trace verb, the CAN-SPAM footer in a real inbox, and the rendered unsubscribe landing page as a RECIPIENT sees it | PIPE-01 SC#8, SC#6 | BRAND conformance and "does this read as information or as a failure" are human judgements no assertion encodes. The unsubscribe page is inline-styled from BRAND hex (a Convex `httpAction` cannot import `globals.css`) and matches no screenshot | ⚠️ **RUN — 15/15, but NOT YET JUDGED.** ~~NOT RUN — this is the blocking owner gate 19-10 stops at.~~ Corrected at 19-13: the UAT executed at 19-12 (`d575b3f`) as `apps/web/e2e/pipeline-uat.spec.ts`, **15 steps, 15 green**, both `test.fail()` markers deleted, measured spend **$0.0400**, evidence on disk at `.planning/phases/19-contacts-crm-follow-ups/uat/` (7 PNGs + `spend.json` + `tenant.txt`). The MECHANICAL half of this row is covered. **What is still outstanding is the owner LOOKING at the seven screenshots and judging BRAND conformance and tone** — a human act, not a run. Three steps are recorded in the spec as not browser-automatable (12: the footer boundary is `buildMime`; 3: no UI affordance for a free-standing follow-up; 10: "clear your postal address" is not executable) |
