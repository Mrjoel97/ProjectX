...
**Success Criteria** (what must be TRUE):
  1. DECIDED 2026-08-01 (ADR-011): provider is **fal.ai**, default model **Wan 2.5** (~$0.05/s), auth is one `FAL_KEY` deployment secret, async via fal queue + webhook onto `convex/http.ts`. Replicate is the recorded fallback. Remaining work under this SC: re-read fal live rates when the adapter is written, and authenticate the webhook.
...
**Plans**: 12 plans (waves 1-9). The old SC #1 (an MCP auth/token-exchange spike) is DISCHARGED: it ran, and it refuted its own premise. Waves 1-8 are runnable now and are entirely free of Lane-R contention; **20-12 alone is PARKED** behind Phase 16 closing the shared `cockpit-agent` candidate stream (the 18-08 precedent) — the phase is demonstrable without it, only the conversational entry point waits. Note that 20-12 and 18-08 edit the same body and therefore serialize with each other.
Plans:
- [ ] 20-01-PLAN.md — The pure media core: the SHOT DECK parser, the (model, resolution) price table with the D4 boundaries, the new `media.md` playbook and the `packages/cost/` watch.json registration (Wave 1)
- [ ] 20-02-PLAN.md — Schema + trace freeze: `mediaJobs`, `plans.shots`/`artDirection`, `guardrailConfig.mediaKillSwitch`, and the `dispatchMedia` step literal shipped WITH its `cards.tsx` VERB entry (Wave 1)
- [ ] 20-03-PLAN.md — The `media-director` skill: koda /art-direction + /storyboard + /generate ported into ONE registry row, DELIBERATELY UNGATED, with a body↔parser round-trip test (Wave 2)
- [ ] 20-04-PLAN.md — The media budget rail: a second per-tenant window plus a keyless deployment ceiling, its own kill switch, and `reserveBatch` — one transactional mutation, mutation-checked (Wave 2)
- [ ] 20-05-PLAN.md — The fal submit adapter: the request body is a function of the priced spec (resolution, duration and audio all pinned), idempotent per shot, with the $0 offline seam (Wave 3)
- [ ] 20-06-PLAN.md — The authenticated webhook and the landing plane: HMAC path segment, bytes downloaded into `_storage`, the honest four-value verdict, delta reconciliation, refs-only audit scans (Wave 4)
- [ ] 20-07-PLAN.md — `media` joins `ACTION_TYPES`; the `externalAction` arm becomes a table, with a calendar-behaviour-unchanged regression (Wave 5)
- [ ] 20-08-PLAN.md — The dispatch surface: `media` in `SPECIALIST_ROUTES` with `SPECIALIST_TOOLS` reused verbatim, the `dispatchMedia` tool, `runMedia` and `persistStoryboard` (Wave 6)
- [ ] 20-09-PLAN.md — The canvas backend plane: the estimate-before-spend query, the two paid entry points through the ONE money gate, the four free editor mutations, and the isolation assertion (Wave 6)
- [ ] 20-10-PLAN.md — The canvas is SEEN: `MediaCanvas.tsx` in the workspace right pane as one more `plan.kind`, BRAND Output-card tiles, live status with no polling (Wave 7)
- [ ] 20-11-PLAN.md — ADR-012 (the route + the corrected ADR-011 arithmetic), the REQUIREMENTS/ROADMAP/todo corrections, four playbooks, and the NON-NEGOTIABLE owner live gate at ~$0.26 (Wave 8, has a blocking checkpoint)
- [ ] 20-12-PLAN.md — Teach `cockpit-agent` the media route + regenerate its one-line mirror, PARKED behind Phase 16 closing the shared candidate stream (Wave 9, has a blocking checkpoint)
...
| 18. Document & Content Creation | 5/10 | In Progress (waves 1-3 complete: 18-01 pure-core format parameterization + `renderHtmlDocument`, 18-02 schema/trace registration surfaces, 18-03 ungated `content-drafter` v1, 18-04 vault write plane, 18-05 the `skillName`/`format` threading that makes `content-drafter` reachable; wave 4 next at 18-06) | 2026-08-01 |
