...
- [ ] 03.8-02-PLAN.md — Lane 1: PDF + image extraction (unpdf text-layer-first + hosted OCR fallback) + live smoke — wave 2, own worktree — tasks 1-2 + automated live PDF smoke DONE 2026-07-18 (lane-1/vault-extract, 15/15 offline, text-layer verified on deployed runtime); Task-3 human-verify (real image OCR) PENDING
- [x] 03.8-03-PLAN.md — Lane 2: Office parsers (pure-TS fflate + XML text-walk, DOCX/XLSX/PPTX, zero convex edits) — wave 2, own worktree — completed 2026-07-18 on `lane-2/office-parsers` (EXTR-C: real extractOfficeText, 15-test fixture suite, vault 42/42 + typecheck green, barrel untouched; awaiting plan 06 merge)
