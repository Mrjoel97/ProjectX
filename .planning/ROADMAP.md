...
- [ ] **Phase 19.1: Bulk Contact Import (CSV)** (INSERTED 2026-08-10) - CSV migration into the phase-19 contacts substrate: browser-side RFC-4180 parse with the file never stored, ≤1000 rows, fill-empty-only merge, and one consent attestation recorded per contact as `consentSource: "imported-attested"`. Design spec approved 2026-08-10; 1 of 4 contact-acquisition subsystems
...
- [ ] 19.1-07-PLAN.md — The REAL browser: a fixture CSV uploaded end to end, the un-overwritten name asserted from stored state, both playbook gates, and the owner judgement checkpoint (Wave 7, has a blocking checkpoint)
...
| 19.1 Bulk Contact Import (CSV) | 6/7 | In Progress — 19.1-01 complete 2026-08-10: the PURE import brain in `@pikar/core` (`parseCsv` hand-rolled RFC-4180 carrying the PHYSICAL file line, `detectMapping`/`mapRows`, within-file dedup under fill-empty-only, `IMPORT_ATTESTATION` byte-identical to the design spec, the three size constants). Identity/validity are the IMPORTED `normalizeAddress`/`isValidEmail`, never re-derived. Core suite 35 files/813 passed, typecheck clean, `check-playbooks` exit 0, both load-bearing guards mutation-proven red, $0.00. **No Convex, no schema change, no UI yet — ACTN-05 stays Pending.** **19.1-02 complete 2026-08-10 — THE RISKIEST PLAN IS DONE AND IT WAS RIGHT TO FEAR IT.** `contacts.origin` gains `imported`, `consentSource` gains `imported-attested`, plus optional `company`/`phone`/`title`, and all 19 consuming sites landed in ONE commit (`102ebe8`, 7 files): 5 widened, 3 deliberately NOT widened and each pinned by a mutation-proven test, 1 silent UI registration, ~10 verified unaffected. The UI site failed exactly as predicted — the first RED run rendered the origin chip as `<span></span>`, a BLANK cell with no error and nothing for `tsc` to say; `ORIGIN_LABELS` is now bound to the read model AT ITS DECLARATION via `satisfies`. The Pipeline consent chip renders `row.consent.source` (owner decision 2026-08-10) instead of flattening a batch attestation into the same chip as per-person consent. One `export type ConsentSource` replaced two independent inline literal pairs. Backend typecheck delta 0 against a 0-error baseline (the finance lane`s `cash.ts` TS2739 were already fixed); backend 72 files/1494 passed, `@pikar/web` 170/170, `@pikar/core` contacts 29/29, `pnpm typecheck` 10/10, `check-playbooks` exit 0, $0.00. 19.1-03 complete 2026-08-10 (`f281fce`): `fillEmptyOnly` arrived as a DEFAULT-OFF FLAG on `upsertContactRow` — one writer, not two — with consent written ONLY onto a row carrying none, and hand-add proven byte-identical rather than asserted so; 8 tests, all reading rows back, 3 guards mutation-proven. 19.1-04 complete 2026-08-10 (`c887528`): `matchExisting` (per-address `{exists, empty}`, `IMPORT_MATCH_TOO_MANY` rather than a silent slice) and `importContacts` (re-normalizes and re-validates at the boundary, a bad row is a `rejected` ENTRY not a throw, counts defined over the four mappable fields ONLY) both in `convex/contacts.ts` — **still no second CRM store**. **AN IMPORT WRITES NO AUDIT ROW,** and the §4 test asserts that as a ROW COUNT over a real import run, not a substring. Batch sizes documented by their REAL reason (retry blast radius, progress granularity, OCC window), the comment stating outright that every Convex limit would permit the whole file in one call. 4 mutation-proofs recorded and reverted; `contacts.test.ts` 87/87, `COVERED` 12 entries, `pnpm typecheck` 10/10, $0.00. **19.1-05 complete 2026-08-10 (`e860094`): `pipelineTiles` REPORTS ITS SCAN BOUND.** It took exactly `SCAN_LIMIT` with no probe row and returned four bare integers, so ONE max-size import into a book that already held a single contact silently under-counted all four ALWAYS-KNOWN tiles — while the e2e integer-parse assertion kept passing, because a wrong integer is still an integer. All three scans now take `SCAN_LIMIT + 1`, counts are computed over `slice(0, SCAN_LIMIT)`, and `partial: "row-cap" | null` reaches the page, which renders `1000+` — a FLOOR stated honestly, not the hedge invariant 3 bans, and `Number.parseInt("1000+", 10)` is still `1000`. **Owner decision: fix the honesty bug at its root, do NOT raise the cap** — five readers share `SCAN_LIMIT`, `savedForName` runs on every cockpit resolution, and a bigger number only moves the same lie further out. `partial` is a bound signal and NOT a tile: `TILES` is keyed on a derived `TileCountKey`. Two mutation-proofs recorded and reverted (`expected null to be 'row-cap'`; `expected '<section class="stat-grid"...' to contain '>1000+<'`). `contacts.test.ts` 89/89, `@pikar/web` 171/171, `pnpm typecheck` 10/10, `check-playbooks` exit 0, $0.00. **19.1-06 complete 2026-08-10 (`4b78f5f`): THE PANEL EXISTS AND IS REACHABLE.** Until this commit `matchExisting` and `importContacts` had ZERO callers — the phase-19 clock-plane failure rebuilt on purpose. `ImportPanel.tsx` is a fourth connected section between the tiles and the contact table (no new route): choose a file, preview the mapping with every column overridable and the counts `N new · M enriched · K unchanged · R rejected`, rejected rows named by PHYSICAL FILE LINE, then an attestation checkbox that STARTS UNTICKED with Confirm carrying `disabled` until it is ticked. The file is parsed with `file.text()` and NEVER uploaded, which is why this feature has no retention rule and no cleanup job — a test scans the panel for `generateUploadUrl`/`vaultUpload`. Every refusal is inline and grey (no `window.alert`, no `window.confirm`, no `<dialog>`, never amber). The over-1 000-row file is refused at the picker naming the limit; a missing email column is refused IN the preview, beside the selects that fix it. The empty state now offers TWO actions and the invariant-1 test was rewritten rather than weakened. Two mutation-proofs red and reverted; `pipelineView.test.ts` 27/27, `@pikar/web` 176/176, `pnpm typecheck` 10/10, `next build` green (the deliberate `PipelineView ⇄ ImportPanel` style cycle resolves in the real bundler), `check-playbooks` exit 0, $0.00. **NO BROWSER HAS EVER DRIVEN IT WITH A REAL CSV — 19.1-07 owns the fixture, the e2e and the owner judgement.** Remaining 1 plan | 2026-08-10 |
