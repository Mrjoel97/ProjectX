...
| 15.3 Vault Folders - Folder Ingest, Synthesis & Drill-In (INSERTED) | 9/9 | Complete — implementation (15.3-09 landed the Google Drive rail: the reservation is taken from files.list metadata BEFORE any byte is downloaded, both shared-drive params are pinned by a source scan because their failure mode is a silently empty folder, and re-import is keyed driveFileId+modifiedTime). **ONE LIVE GATE OWED: no real shared-drive import has ever run** | - |
