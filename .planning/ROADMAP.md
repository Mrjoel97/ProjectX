...
- [ ] **Phase 17: Calendar Actions** - Governed Google/Microsoft calendar events (read in-loop, write plan-gated). **UNTICKED 2026-08-14 — this row was `[x]` against its own verifier.** `17-VERIFICATION.md` is `status: gaps_found` and states plainly: "`ACTN-02` remains correctly unchecked and Pending. It must not be marked complete from the narrow Google-create result." What ships is Google-only and create-only; `calendarEvents.ts` and `microsoftCalendar.ts` do not exist on disk. TWO REQUIREMENT GAPS, both honest deferrals but still gaps: (1) no update/move/cancel/delete, no ETag/`If-Match`/412-concurrency path — the adapter exports only `freeBusy` and `createEvent`; (2) no Microsoft Graph Calendar endpoint, grant, adapter, action type or completion terminal. Six gap-closure plans (17-06…17-11, waves 2-7) are on disk and unexecuted; 17-05 landed the `calendarManagement.ts` substrate they build on. Owner live gates H1-H3 (real consent, live create, pre-widening reconnect) validate the shipped Google slice only — **they cannot make the missing provider and operations exist**
