...
current_phase: 25.1
current_plan: 6 of 7 (phase 25.1 wave 6 complete - all autonomous waves done)
status: executing
stopped_at: "Completed 25.1-06-PLAN.md 2026-08-21 (D12/D13/D14, 6 commits 483d0ed/e8dc04f/c21b0f9/f5a032e/2abbfc5/d1ae47e, $0). THE HYGIENE CLUSTER, AND TWO PLAN PREMISES CORRECTED ON EVIDENCE. D12: ENV_MANIFEST's drift scan matched only a LITERAL process.env.X, and every media name in this codebase is read through requireEnvMedia(NAME) - a process.env[name] lookup behind a helper - so the guard was blind to all of them. MEDIA_RENDER_URL is the sharp one: read INSIDE the scheduled renderReel action, so unset it throws where no user is waiting and the plan sits at 'rendering' for ever while /admin reports the deployment READY. The scan now runs a SECOND regex over the same file walk matching the call sites' STRING LITERALS - not a [A-Z_] shape, because Video_and_image_API_Key is Alibaba's own mixed case and a shape rule drops exactly one of the three - and the new test also asserts those names are NOT reachable as literals anywhere, which is what stops the extension being silently deleted later. MEDIA_RENDER_URL, WAN_API_BASE_URL and Video_and_image_API_Key are now classified. MEDIA_SANDBOX_SNAPSHOT_ID IS DELIBERATELY NOT, AGAINST THE PLAN'S OWN must_have: its only reader in the repo is apps/web/app/api/media/render/route.ts, a VERCEL variable no Convex process can see, so listing it would make envCheck report a name missing on every healthy deployment - the exact reasoning env.ts already carries for NEXT_PUBLIC_CONVEX_URL, and deploy-production.yml already asserts it. D14: THE fal WEBHOOK IS DELETED, AND ITS CALLER SET WAS PROVED EMPTY HOP BY HOP BEFORE A LINE WAS REMOVED. submitLine's third parameter was already _webhookUrl? - UNUSED - and its one caller passes two args, so nothing has minted a fal_webhook URL since ADR-017; resolveJob had exactly ONE caller (the route); FAL_WEBHOOK_SECRET had exactly ONE reader (resolveJob). The route's only reachable caller was somebody HOLDING THE SECRET, for whom it offered an outbound fetch to a caller-named URL and a mutation writing a terminal 'succeeded' onto a mediaJobs row - an unused AUTHENTICATED WRITE PATH, not conservatism. Gone: the route, the HMAC segment, the +/-300s replay window, the SSRF host allow-list, resolveJob, the manifest entry (217 lines out of http.ts, 55 out of mediaComplete.ts, 941 out of media.test.ts). FAL_FIXTURE SURVIVES and is asserted to - the name is a fossil, the seam is live. ADR-024 records the OpenAI provider (sora-2 720p 4s, gpt-image-2, tts-1, whisper-1, endpoints + price pins) and supersedes ADR-017's VISUAL half and its open-ended 'retain the old callback temporarily' clause; RESEARCH WAS WRONG that ADR-011/012 still document the live provider - ADR-017 superseded ADR-011's provider line back on 2026-08-13 and what was unrecorded is the SECOND cutover, Wan -> OpenAI. ADR-011 and ADR-012 are BYTE-UNCHANGED (verified by git diff); ADR-017 gets a Status-line pointer, the only edit docs/README.md permits. ADR-024 also names the line a reader would otherwise carry away wrong: ADR-017 says 'it does not receive prompts for visual jobs' and that is FALSE as of this cutover. D13: THE RESEARCH PREMISE WAS TWO-THIRDS WRONG - deadLetters.listNew/newCount have existed since 02-06 and /ops has rendered them with a Mark-resolved button ever since; only 'no re-drive' was true. The genuine gap is that every insert site stamps the FAILING TENANT's id while listNew is a tenantQuery, so the OPERATOR could not see another tenant's failure at all. deadLetters.listAll (ownerQuery) is that read: cross-tenant, status=new, newest-first over the by_status index that had existed with NO READER since 02-06, default 50 / server ceiling 200, take(cap+1) so 'truncated' is KNOWN not guessed, projection a pinned key set with the payload byte-identical. /ops gains an owner-only 'Dead letters - all tenants' section gated BY MOUNTING per the page's own rule. SEEING IS NOT ACTING: markResolved stays tenantMutation and a source scan bans any ownerMutation on the module; re-drive stays deferred. 17 MUTATIONS, each reddening only its own test - AND ONE VACUOUS CLAIM CAUGHT: swapping take(cap+1) for .collect() left EVERY response assertion GREEN (same rows, same order, truncated still correct, because the handler's own slice still caps the payload). The RESPONSE is identical; only the READ changes, and on a deployment with fifty thousand dead letters that IS the hazard. Test renamed to what it proves ('the RESPONSE is capped from the newest end') and the read-bound pinned in SOURCE, labelled a mechanism test. Third phase running that a mutation caught a test asserting less than its name. THREE GUARDS FIRED CORRECTLY and were updated deliberately: microsoftAuth's http.route count 8->7 (a route DISAPPEARING is as worth noticing as one appearing), skills.test's >200-char inline-literal ban (a whatBreaks string - shortened, the long version belongs in the playbook), and isolation.test's self-growing owner surface, which caught deadLetters.listAll as the 16th owner endpoint / 6th module and gave it a free OWNER_REQUIRED case. apps/web's opsPresentation fixture THROWS on an unrecognised query, so it forced deliberate registration of the new owner-only hook in BOTH halves (fixture returns a NON-EMPTY row on purpose: an empty result renders the 'none' paragraph, indistinguishable from a deleted section). It also exposed a LATENT BUG: tenantSkillReview.test sliced the panel body 'to the next top-level declaration' in prose and to OpsPage in code, so its BRAND hex pin went red over another component's colour - fixed to match its own documentation. FULL GATE: RED, ON EXACTLY ONE PRE-EXISTING TEST. typecheck 10/10 GREEN; build GREEN; test 2219 passed / 1 FAILED - isolation.test.ts skills.activateAgentCandidate, DEFERRED ITEM 1, red on this branch before this plan started (found by 25.1-03, recorded still-red by 25.1-05). Not fixed here per instruction, but MEASURED so 25.1-07 knows the distance: adding ONE line to OWNER_ARGS - skills.activateAgentCandidate: { candidateId: 'id:tenantSkills' } - takes that file to 32/32 and the whole gate GREEN, AND PROVES THE OWNER GATE ON THAT ENDPOINT IS SOUND, not defective: the test was dying on the validator before requireOwner ever ran. Measurement reverted; working tree carries none of it. Deferred item 1's SECOND half (de-vacuifying REQUIRES_ARGS, which is derived from OWNER_ARGS and so can only flag endpoints that already have a fixture) is NOT covered by that line. pnpm gate:lint is red with 85 biome FORMAT errors, all CRLF, NONE in a file this plan touched (verified by intersecting the reporter's file list with git diff --name-only) - and gate:lint is not part of pnpm gate. Also measured: the three plan suites 259 passed / 0 skipped (baseline 266 passed | 24 skipped - the 24 skips and 17 route-bound actives deleted, 21 added), apps/web 453/453, both typechecks clean, check-playbooks exit 0, graphify rebuilt (18326 nodes, +513 convex edges, +73 table edges). TWO DEFERRED ITEMS ADDED (5, 6). Item 5 matters: pollWanTask IS SCHEDULED ONLY BY ITS OWN RETRY - no submit path enqueues it - so the legacy Wan poller's 'pre-cutover tasks' retention has silently expired and it is dead in the same sense the fal route was. NOT REMOVED: the plan named it as the thing not to break, and unlike the route it is an internalAction with no HTTP surface and no external caller, so it is a dead-code question and not a security one. Recorded in ADR-024 section 3; both its env names are now classified so their absence is at least honest. TWO SHARED FILES CARRY FOREIGN UNCOMMITTED HUNKS (schema.ts's 50-line table index, cockpit.md's calendar.test.ts watch entry) and git stash is banned here - 25.1-05's dance repeated: snapshot / git checkout -- / reapply only my block / stage / commit / restore, with git diff --cached inspected before the commit. Both foreign hunks are back, unstaged, at their original sizes. mediaComplete.ts was normalised CRLF->LF on disk (its whole file read as one biome formatting error and hid real diagnostics); git normalises on add so the commit is unchanged. ONE DEPLOYMENT ACTION, AND IT IS A DELETION: npx convex env remove FAL_WEBHOOK_SECRET (and --prod). Nothing breaks if skipped - the name is simply unread. NOT SEEN IN A BROWSER: the 'all tenants' DLQ section is renderToStaticMarkup-only. 25.1-07 is the owner checkpoint and should look at it - the most likely honest outcome is that it renders EMPTY on a single-tenant production, which is itself a real answer. media.md is now FRONT-LOADED with the provider truth and the 3700 lines beneath it still describe fal in the present tense - deliberate, and the top block says so in its first sentence. Counters below carried forward +1 completed plan, NOT re-derived. Next: 25.1-07 (wave 7, OWNER CHECKPOINT - the last plan of the phase)."
