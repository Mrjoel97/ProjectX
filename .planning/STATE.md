...
current_phase: 19
current_plan: 10 (UAT 15/15 green after 19-12 — owner sign-off still owed)
status: in_progress
stopped_at: "PHASE 19 IS 10/10 ON DISK BUT NOT CLOSED: 19-10 IS PARKED AT ITS BLOCKING OWNER BROWSER UAT AND WAS NOT SELF-APPROVED. Commits e326ef9 + ed1dbdb + ef23a52. **THE HEADLINE: ACTN-05 DOES NOT WORK, AND IT IS NOW MEASURED.** `cockpit-agent@18` IS ACTIVE (owner-authorized activation; skillId kh74zgpwx8y9qs98dhdje2fncn8c424a) so every document calling v18 a CANDIDATE or ACTN-05 `certified but not live` is STALE. BUT: asked in plain language to add a dated follow-up for a named person, the live v18 body stages an `addContact` with NO `dueAt` and NO follow-up at all. Run `309b1c3d`, `--only 36`, $0.0142; plan row read back at $0 holds exactly one `addContact` (origin `mailbox-resolved`). On the run's OTHER attempt it staged nothing and the plan stayed `collecting` — so it is not even uniform: 1 of 2 attempts reached the tool, neither made a follow-up. 19-09's 35/35 was green over this because `crmOperationCount` is a COUNT and cannot tell op TYPES apart. **19-10 ADDED `datedFollowUpCount` TO THE CLOSED `EXPECT_KEYS` VOCABULARY** — counts `addFollowUp` ops with a finite `dueAt`, is a SUBSET key the runner REFUSES without `crmOperationCount` (so it cannot be satisfied alongside unrequested extras), and BOTH halves are mutation-proven red-able at $0. **FIXTURE 36 IS NOW RED AGAINST THE ACTIVE BODY AND A FULL GATE IS 34/35 UNTIL THE BODY REACHES THE TOOL. THE ASSERTION WAS NOT WEAKENED TO RESTORE GREEN.** DO NOT fix this by adding another prohibition to the body — 19-09 proved that road: the body forbids the adjacent failure VERBATIM and the model did it anyway 2/2. Fix the TOOL SHAPE (one grammar taking person+due together) or refuse a follow-up request that yields a contact-only op list. **ACTN-05 AND PIPE-01 STAY PENDING; `.planning/REQUIREMENTS.md` WAS NOT TOUCHED** (phase-25 lane owns it) — verified its checkbox and status table AGREE at not-complete. **`eval:golden --self-check` IS GREEN FOR THE FIRST TIME SINCE PHASE 20.** `media-director` was never an owner architectural call: `skill.ts` ALREADY carried a written dated DELIBERATELY UNGATED justification for it, one of SIX such rows. The runner now DERIVES that exemption set from `skill.ts` (same read-off-disk idiom it uses for GATED_SKILLS), so a new exemption registers itself the day it is written and can ONLY register BY writing the justification; neither gated nor justified still fails hard. The route→skill ternary is gone, replaced by `SPECIALISTS[route].skillName` — that second copy is why the error named the route `media` instead of the real skill `media-director`. TWO RESIDUAL RISKS LEFT OPEN AND RECORDED: a `media-director` body edit still activates with no eval evidence (accepted cost, stated in skill.ts; re-raise if media ever ingests untrusted third-party input), and **`runLive()` STILL NEVER CALLS `selfCheck()`** — which is exactly why this stayed invisible for a whole phase. **`e2e/pipeline.spec.ts` RAN FOR THE FIRST TIME: 2/2 PASSED in 12.3s** against live convex on :3210 + a PRODUCTION build on :3111. **19-07's `an executor cannot mint E2E_USER_EMAIL/E2E_USER_PASSWORD` IS FALSE for a local deployment** — convex/auth.ts runs the Convex Auth Password provider and /signup is a real form, so a throwaway user is one scripted signup away, AND a fresh signup is how the empty-tenant precondition was met without building the reset seam 19-07 declined to build. A new tenant IS an empty tenant. The first run found a bug IN THE SPEC (not the product): `(await locator.count()) > 0 ? a : b` does not auto-wait, so right after a goto it committed to `add-contact`, which only renders when contacts exist and so could never appear on the empty tenant the file itself asserts — 30s hang. `locator.or()` is the native fix. Re-running against that same user now fails test 1 as a PRECONDITION failure (test 2 created a contact); sign up another. **`pnpm --filter <pkg> test -- <name>` DOES NOT FILTER ANYWHERE IN THIS REPO** — pnpm forwards the literal `--`, vitest matches nothing and runs EVERYTHING (core: 32 files vs 1 without the `--`; backend: 72 files / 1415 tests / ~2min, not the claimed ~90s filtered). DROP THE `--`. Same root cause as `pnpm --filter @pikar/web test:e2e -- <file>`, which ran all 25 e2e specs for 8 minutes instead of one. **NASTIEST CONSEQUENCE:** seven VALIDATION rows named `-- cockpitTools`, and cockpitTools.test.ts has ZERO executePlan tests (grep 0, vs 78 in cockpit.test.ts). They pass ONLY because the broken `--` accidentally runs cockpit.test.ts too — repair the `--` without repairing the file names and six rows become vacuous greens. Both corrected in 19-VALIDATION.md. **ALL 22 VALIDATION ROWS FILLED FROM RE-MEASURED COMMANDS: 21 green + 1 qualified (row 7 — three of the four arms are proven by ABSENCE OF CHANGE, not a new claim), 0 red, 0 NOT RUN.** The manual-only owner-UAT row is NOT RUN and says so. Recorded there too: the 22 rows all test the MECHANISM and none asks whether the live model reaches it — which is why 22/22 green coexists with ACTN-05 unmet. **ROADMAP.md IS FINALLY COMMITTED** (the phase-25 lane had committed its ~17 lines, so the diff was phase-19-only; verified zero phase-25 content in the added lines before committing). All THREE stale Phase-19 locations corrected: the Phase-26 note said TWO new tables (three shipped — `suppressions` is what makes SC#5 structurally clean, since the guard reads it ONLY and never `contacts`), the amended-2026-08-07 bullet still said unplanned, and the Phase Index row said 5/10. **A FULL EVAL GATE COSTS ~$0.35, NOT THE ~$0.12-0.15 QUOTED THROUGHOUT THIS PHASE** — corrected in ROADMAP, VALIDATION, contacts-crm.md and agent-runtime.md. `run-eval-golden.mjs --list` DOES NOT EXIST (falls through to a full PAID runLive); the offline command is `--self-check`. The plan cited `14feb4b7` as v17's certifying gate; the evidence row says `d17039a8` — trust the row. **THE PLAN'S OWN VERIFY SCRIPTS ARE PARTLY UNPASSABLE:** its ROADMAP regex spans 27 lines from an unrelated `Phase 19` mention into a DIFFERENT phase's TBD, and two of its scans fail on a document that QUOTES the bad string while correcting it. Scoped assertions used instead. GREEN: pnpm test 9/9 (backend 72 files / 1415 tests), pnpm typecheck 10/10 exit 0, backend `npx tsc --noEmit` RE-MEASURED at exit 0 no output, web build green, check-playbooks exit 0, biome clean, --self-check PASSED. Per-file re-measured: cockpit 63, gmail 42, plans 22, backend contacts 62, core contacts 23, actionType 12, pipelineView 17, crmCard 2, llmRedaction 60, traceParity 2, cockpitTools 101. SPEND THIS PLAN: $0.0142 against a $0.05 ceiling. No full gate was run and none was authorized. STATE.md's `stopped_at` quote bug was fixed in 12bde78 BEFORE this plan; escape every `\"` as backslash-quote when appending here, or prefer backticks — this block carries THREE frontmatter blocks, so a parser dying on the first falls through to a STALE one, which is the real story behind the long-running `gsd-tools clobbered the frontmatter` complaint. No `gsd-tools state *` subcommand was run; hand-edited. Convex dev left RUNNING on :3210; a Next PRODUCTION server was left on :3111. NEXT: the owner UAT, then a plan for the ACTN-05 defect (tool shape, not body text), then `runLive()` should call `selfCheck()`, then repair the `--` idiom repo-wide. Prior: MULTI-LANE — per-lane position lives in '## Lane Status'; this block is the single tool-readable summary. Phase 19 is 9/10: 19-09 landed THE BODY EDIT, THE OWED FIXTURE, AND A GREEN GATE — BUT NOTHING IS ACTIVATED. **GATE `086f8267`: 35/35, ZERO RETRIES, $0.3505, ON `cockpit-agent@18`; evidence is stamped on the v18 row so it SATISFIES `EVAL_GATE` and is one `activateCandidate` click from live — and that click was deliberately NOT made. `cockpit-agent@17` IS STILL THE ACTIVE BODY, so in production the model STILL CANNOT SEE `stageCrmWrite`.** Commits f5e527f (body + byte-identical mirror) + 53254e7 (fixture 36, the `crmOperationCount` observable, floor 34 -> 35) + 8a0612b/e3baaa4 (interim playbook records) + 497f8f8 (THE FIXTURE REPAIR) + ff59e09 (both playbooks, final outcome). **THE PLAN WAS RESUMED ACROSS ITS BLOCKING OWNER CHECKPOINT TWICE** — Tasks 1-2 in a prior session, then seed+diagnostic, then owner approval of the no-spend call, then the repair+gate. **v18 WAS HASH-VERIFIED BEFORE A CENT WAS SPENT** (28 368 chars, sha256 `6ca4d937639cc6b97a90f4f4ca54315f6c39ae9c818b9c94c1ec3d72a17510ad`, minted maxVersion+1 from a clean 17-max = FORWARD, not a rollback); do this every time. **FIXTURE 36 FAILED ITS FIRST LIVE EXECUTION TWICE, IDENTICALLY, FOR $0.0115, AND THE FIXTURE — NOT THE BODY — WAS WHAT CHANGED.** It expected `crmOperationCount` 1 / `recipientCount` 0 and got 0 / 1; both plan rows read `status: collecting`, `recipients: [<addr>]`, a composed `subject`, NO `crmOperations` field — the agent never called the tool once. Turn 1 said `Remind me on Thursday to chase Rhea Calloway ... her address is <addr>`: an outreach verb plus an inline address, the strongest recipient-collection cue the cockpit has. Rewriting turn 1 in the records grammar TURN 2 ALREADY USED (`Add a follow-up for Thursday with ...`) passed first try for $0.0071 — and also repaired a real incoherence, since turn 2’s `Also ... that one` presupposed a follow-up add that turn 1 never was. **THE BODY WAS DELIBERATELY NOT TOUCHED: it already forbids this VERBATIM** (`This is not a compose: do not add the person as a recipient to get at their address`) and the model did it anyway on 2/2 runs — uniform behaviour is not fixable by another sentence, and a third prohibition buys a third identical failure. **THE ADDRESS DELIBERATELY STAYED IN TURN 1**: it is the temptation that gives `recipientCount: 0` its teeth, and needles are LEAK CANARIES (`smokeAssert:assertEvalCaseClean` asserts them ABSENT from audit/deadLetters/telemetry), so a needle in no turn never enters the system and tests nothing. NO assertion was weakened and turn 2 is byte-identical. **OPEN DEFECT, FOUND ON THE PASSING RUN — DO NOT OVER-TRUST FIXTURE 36:** the staged op is `addContact` with NO `due`, not the `addFollowUp` its prose describes; `crmOperationCount` is a COUNT and cannot tell op types apart. 36 proves one CRM op staged, `kind: crm_write`, `status: proposed`, `recipients: []`, and turn 2 added no second — real ACTN-05 teeth — but NOT that a dated follow-up was created, and an unrequested contact is in tension with SC#7’s explicit-acts-only rule. Closing it needs a NEW KEY in the CLOSED `EXPECT_KEYS` vocabulary (op-type and/or `due`) — a code change; 19-10 should own it. **BUDGET OVERRUN, OWNED: $0.3691 SPENT AGAINST A $0.30 CEILING (over by $0.0691).** A full gate costs ~$0.35, NOT the ~$0.12-0.15 every plan and checkpoint quoted — the three research fixtures alone are ~$0.11 of specialist spend. **THIS WAS PREVENTABLE AT $0: v17’s own `skills.evidence` row records the previous 34-case gate `d17039a8` at `costUsd: 0.357`.** Read `skills.evidence` on the ACTIVE row before budgeting any gate; the gate is one uninterruptible command, so the only decision point is before launching it. (Note: the plan cited `14feb4b7` as v17’s certifying gate but the recorded evidence says `d17039a8` — trust the row, not the prose.) **`run-eval-golden.mjs --list` DOES NOT EXIST — 19-09-PLAN.md names it twice as an offline check and is WRONG both times**; unknown argv is ignored and execution falls through to `runLive`, a full PAID run. The offline command is `--self-check`, which is RED on main for a pre-existing reason (`media-director` dispatchable but not in `GATED_SKILLS`) written up in `deferred-items.md`; it does not block the gate because `runLive()` never calls `selfCheck()`. **ACTN-05 IS CERTIFIED BUT NOT LIVE** — recorded in the SUMMARY frontmatter only; `.planning/REQUIREMENTS.md` was NOT touched (phase-25 lane owns it). **20-12 and 20.1-01 MUST REBASE ON THIS BODY**, and the tree is no longer a clean 17-max: the next `seedSkills` mints v19. Convex dev was left RUNNING on `:3210`. ROADMAP.md IS HAND-TICKED BUT DELIBERATELY NOT COMMITTED (it still mixes the phase-25 lane’s uncommitted lines; 19-10 reconciles). Next is 19-10. Prior: Phase 19 is 8/10: 19-08 landed CONTACTS-FIRST RESOLUTION AND THE CRM STAGING TOOL — b982c68 (RED, 10 failing) + c9b42eb (the tool + all three registration surfaces) + c3cf060 (three playbooks). **resolveContacts NOW CHECKS `contacts` BEFORE GMAIL HEADERS** via a new `internal.contacts.savedForName`, inside the EXISTING tool (a second resolution tool would be a second registration surface for one job); BOTH planes rank with the SAME `rankCandidates`, fed saved rows shaped as header records, so the saved record and the header inference cannot disagree about who a name means. A saved hit returns that contact's OPEN follow-ups in the same call (each note through `scanText` — a note is prose that could hold an address). **THE ORDERING IS PROVEN BY ROW COUNT, NOT BY READING THE REPLY:** `gmail.search` always writes exactly one refs-only `mailbox.searched` audit row, so ZERO rows means the mailbox was never touched — MUTATION-VERIFIED (inserting an unconditional `gmail.search` above the saved branch reddens it: `expected [ { …(8) } ] to have a length of +0 but got 1`), and the pre-existing test was extended to assert the count is 1 when nothing is saved, so the pair is non-vacuous both ways. **`resolveContacts` STILL WRITES NO CONTACT ROW on either plane** — counted across a resolution that matched nothing, one and several; that is contacts-crm.md's NEW invariant 16 and the SC#7 enforcement point. DO NOT 'warm the cache' by upserting what resolution found. **`stageCrmWrite` IS ONE TOOL CARRYING A LIST** — it patches kind/status/crmOperations and applies NOTHING; the Approve gate (19-06's inline arm) is the only application path. `parseCrmOperations` runs at the WRITE boundary too, so the plan row stores NORMALIZED addresses. FOUR DELIBERATE NARROWINGS: the agent may only ADD (completeFollowUp/cancelFollowUp refused — closing a follow-up is the human's call on the Pipeline page, which also avoids handing the model row ids); `due` is the user's WORDS through `parseSendTime` + the trusted `clientContext` (a model-supplied epoch would be a fabricated date on an approvable card — §2-D); `origin` is hardcoded `mailbox-resolved` server-side; and it REFUSES over a half-composed email (a thread has ONE plan row, so patching `kind` onto a drafted row would strand the draft — NOT in the plan, a data-loss guard). `CrmOperation`'s required `email` on addFollowUp was NOT relaxed. Every refusal is a RETURNED SENTENCE. **THREE REGISTRATION SURFACES AND THE PLAN WAS RIGHT** (the first correct enumeration this phase) — but the SMOKE op is FIVE sites, not the plan's four (the grammar comment is the fifth). BOTH guards mutation-proven: dropping the schema literal reddens cockpitTools' key-scan (`expected [ 'stageCrmWrite' ] to deeply equal []`) AND traceParity's orphan side; dropping the VERB entry reddens traceParity's missing-verb side. **A GUARD NOBODY PREDICTED:** dropping the schema literal ALSO fails `tsc` (`TS2322: Type '\"stageCrmWrite\"' is not assignable…`) because `SMOKE_OP_TOOL` is `Record<AgentSmokeOp[\"kind\"], StepTool>` and `StepTool` derives from the schema union — STRONGER than 19-06's crm_write case, but it exists ONLY for tools with a SMOKE op; do not generalise it. **`SMOKE::agent::crm=<email>[:<note>]` IS THE FIFTEENTH OP, $0, NO MODEL CALL, AND NEEDS NO NESTED `SMOKE::route=direct_llm::` PREFIX** — that is `create=`'s constraint (createDocument calls draftDocument), and a prefix here would land inside the email address. Verbatim strings are in agent-runtime.md. NO smoke.ts change was needed despite the plan listing it. **THE TOOL IS NOT YET VISIBLE TO THE MODEL:** registering a tool does not teach it — 19-09 owns the cockpit-agent body edit and the eval fixture; until then only the smoke string reaches it. **LINE ENDINGS ARE MIXED PER FILE, MEASURED A THIRD TIME:** cockpit.md and contacts-crm.md are LF (0 CR), agent-runtime.md is CRLF (852/852), ROADMAP.md is CRLF (1334), STATE.md is LF. Both the standing 'they are CRLF' note and 19-07's 'the playbooks are LF' correction are HALF-WRONG — measure the individual file. pnpm test 9/9 — backend 72 files / 1415 tests (1405 -> 1415), web 122, core 697; pnpm typecheck 10/10 exit 0 delta 0; web build green; check-playbooks exit 0; NO codegen needed (a new FUNCTION on an existing module leaves `_generated/` untouched — 19-07 measured the same); biome delta ZERO on all seven touched source files. ROADMAP.md IS HAND-TICKED BUT DELIBERATELY NOT COMMITTED (it still mixes the phase-25 lane's uncommitted lines; 19-10 reconciles). Next is 19-09. Prior: Phase 19 is 7/10: 19-07 landed THE CONNECTED PIPELINE ROUTE — e32f0b7 + efc6dd4 (three bounded read models) + 351e599 + 7f28c8f (the page) + ead5eff (the e2e spec + three playbooks). `/dashboard/pipeline` is connected, tenant-safe and bounded, REACHABLE BY URL ONLY — the nav item is still `soon: true` and 26-18 owns the href (adding it IS the activation; contacts-crm.md invariant 15 and the e2e spec both guard it). **contacts.ts HAS PUBLIC READS FOR THE FIRST TIME** — pipelineTiles / listContacts / listUnassignedFollowUps land in the SUBSTRATE MODULE, not a second store, which is PIPE-01 satisfied by construction; 545 -> 838 lines, the export-set pin grew by three, and the no-opportunities structural scan was NOT weakened (it now covers 293 more lines). NO opportunities table, NO stage enum, NO amountCents. **THE FOUR TILES ARE ALWAYS-KNOWN: a real zero renders as `0`, and the test asserts the ABSENCE of `—` and `Unknown` in the tile markup** — the 26-10 defect (1a63992) from the other side. lastTouchAt is DERIVED at read time (one bounded requests fold through by_tenant_status_createdAt + the newest completedAt), deliberately NOT a denormalized field: that would be a write-path obligation this phase does not have. Its 1000-row ceiling and the upgrade path (recordDeliveryTerminal AND setFollowUpStatus, both, or the field lies) are contacts-crm.md invariant 14. **MY OWN FIXTURE WAS UNTESTABLE AND A MUTATION-SHAPED FIX CAUGHT IT:** setFollowUpStatus stamps completedAt at Date.now(), so in a fixture EVERY completion beats EVERY seeded send and the max-picks-the-send branch could never run — the old completion is now aged by hand. **THE PAGE'S SOURCE SCANS HAD TO STRIP COMMENTS** (the 19-02 gravestone rule): the scans for window.confirm, --held and /amountCents|opportunit|stage/ all hit the file's own comments explaining why those are absent. **THE PLAYBOOKS ARE LF IN THIS TREE, NOT CRLF — STATE/memory say CRLF and that half is WRONG for docs/playbooks/*.md today** (measured: 0 CR across all three before the edit); the single-line-anchor rule still held and is still worth keeping. NO e2e reset seam was added to contacts.ts — a deletion path in the phase's most governed module for a spec that has never run is speculative; the empty-tenant precondition is in the spec header with the seam named as the upgrade path. **e2e/pipeline.spec.ts IS AUTHORED AND HAS NEVER EXECUTED** (2 tests, --list-discoverable; a --list is NOT a run and a blank result means NOT RUN) — 19-10 runs it and owes the page its first human eyeball, including whether a done crm_write plan is visible here (19-06 ships no chat receipt on purpose and said the fix would be this page). pnpm test 9/9 — backend 72 files / 1405 tests (1391 -> 1405), web 122 (105 -> 122), core 697; pnpm typecheck 10/10 exit 0 delta 0; web build green with /dashboard/pipeline in the route table; check-playbooks exit 0; no codegen needed (new functions on an EXISTING module leave _generated/ untouched). ROADMAP.md IS HAND-TICKED BUT DELIBERATELY NOT COMMITTED — it still mixes the phase-25 lane's uncommitted lines; 19-10 reconciles it. Next is 19-08. Prior: Phase 19 is 6/10: 19-06 landed THE FIFTH ACTION TYPE — ccad3ce + 5bce978. **THIS PLAN WAS RESUMED AFTER A SESSION KILL: the first executor wrote 871 uncommitted insertions across 15 files and created ZERO commits. Nothing was discarded; the resume finished the Task-3 tail (playbooks) and the whole verification gate.** `crm_write` is the `inline` arm's SECOND occupant — a CRM write targets OUR OWN contacts/followUps tables, so it is `a single transactional write` (this file's own definition of `inline`), not `externalAction`; no fetch, nothing for the retrier to retry, and the arm sits ABOVE the Gmail and postal-address pre-checks so a CRM plan approves on a tenant that could not send at all. ALL-OR-NONE AND DOUBLE-APPROVE ARE BOTH FREE: a Convex mutation is one serializable transaction and `applyCrmOperations` is called DIRECTLY (not via runMutation) so it shares the CAS transaction — asserted, not assumed (an invalid THIRD operation applies none of the first two and the plan stays `proposed`). **THE PLAN SAID ELEVEN REGISTRATION SITES; THERE ARE FOURTEEN.** The three it did not name: `approvals.ts` planKind (compile error), the ApprovalsView badge/title/actionLabel trio (compile error via planKind), and **`llm.ts` buildAgentContext, which is SILENT** — and whose own comment falsely claimed it was a compile-error site. It is not: `PlanRow` does not declare `kind` at all, so widening ACTION_TYPES never breaks the call, and `media` proved it by shipping in Phase 20 without ever reaching that param union. **EVERY NEW ACTION TYPE NEEDS A HAND EDIT IN `llm.ts` OR THE MODEL IS TOLD THE PLAN IS AN EMAIL.** The 14-site checklist now lives in cockpit.md. `contacts.ts` was refactored so the identity/upsert rule has ONE implementation (three shared helpers; the public tenantMutations are one-line delegations the applier also calls). MUTATION-VERIFIED Pitfall 1: dropping `v.literal(crm_write)` from patchPlan's hand-maintained mirror alone turns 2 plans.test.ts tests RED while EVERY typecheck still passes — reverted. pnpm test 9/9 TWICE (backend 72 files / 1391 tests, core 697, web 105); pnpm typecheck 10/10 exit 0 delta 0; web build green; check-playbooks exit 0 after a THIRD playbook (dashboard-pages.md) the plan did not name. codegen ran and `_generated/` is UNCHANGED. **ROADMAP.md IS HAND-TICKED BUT DELIBERATELY NOT COMMITTED** — it mixes 17 uncommitted lines from the concurrent phase-25 lane; 19-10 reconciles it. Next is 19-07. Prior: 19-05 landed THE TRUST BOUNDARY — 4fc4941 (executePlan: the pre-CAS no_postal_address refusal, the per-address suppression drop BEFORE the group join (mode:group collapses recipients into one comma-joined string on the next line, so a per-address drop is impossible afterwards), all_recipients_suppressed, and withheld[] on the ok arm; MUTATION-VERIFIED — moving the guard below the CAS turns the \"plan stays proposed\" assertion RED), 0bc7c76 (gmail.send refuses a suppressed recipient BEFORE a credential is minted — zero fetches — and appends the CAN-SPAM footer at the buildMime CALL SITE, never inside it; recordDeliveryTerminal gains a third \"suppressed\" outcome -> requests.status blocked + recipientTotal decremented so the counters balance and queuedCount reaches 0), cc58004 (both approve surfaces name the two refusals and the withheld report — one note affordance, two tones, no amber; cockpit.md + contacts-crm.md invariant 12 + dashboard-pages.md), 988e24b (the new send tests skip the audit aggregate they never touch). pnpm test 9/9 TWICE — backend 72 files / 1383 tests; pnpm typecheck 10/10 exit 0 delta 0; web build green; check-playbooks exit 0. **THE PLAN'S CONVERGENCE PREMISE WAS WRONG: gmail.send has TWO production callers, not one — deliverApprovedPlan.ts AND pipeline.ts:379. The pipeline lane would have stranded a suppressed row at `delivering` forever; it now patches blocked (no planId there, so no counters). Re-grep before claiming convergence.** __seedOnboardedTenant now seeds a postal address or EVERY e2e spec that approves an email plan refuses — the e2e suite has NOT been run (needs a live deployment). UNSUBSCRIBE_SECRET is now load-bearing for SENDING, not just links: unset ⇒ footerFor null ⇒ gmail.send THROWS ⇒ the retrier dead-letters; only the LOCAL deployment has it. 19-10 UAT owes an eyeball to three never-seen strings (the withheld note, the no_postal_address note + its /dashboard/profile link, the all-suppressed note) as well as the unsubscribe landing page. TEST-HARNESS LESSON, PAID TWICE THIS PLAN AND ONCE IN 19-02: every convexTest instance is another in-memory backend and every registerComponent loads that component's whole module tree into it — register per TEST, not per file, or the shared vitest fork crashes and takes vaultDigest.test.ts down with it (8 failures) while your own files stay green. Next is 19-06. Prior: 19-04 landed the public unsubscribe route (f4ee077, 9cb5fed, 2e641b0); 19-01 the contacts substrate (0abc73b, 38ac3d2, a78a169); 19-03 the CAN-SPAM postal address end to end (5a72ac4, bcf825e); 19-02 the person store (5939f08, 55fd303, 62428db, 7448205, 85ad661). PIPE-01 stays Pending — nothing ticked until the phase closes. NOTE: gsd-tools state advance-plan AND record-metric AND record-session ALL CLOBBER this block; roadmap update-plan-progress 19 ALSO writes over PHASE 20.1's plan list and guts the phase-19 table row — both hand-edited again here. Verify this block AND ROADMAP.md diff after ANY gsd-tools call. Do NOT re-add a second frontmatter block on merge. **UPDATE 2026-08-10 (19-11, second pass) — THE PHASE'S LAST OPEN DEFECT IS CLOSED AND EVERYTHING ABOVE ABOUT ACTN-05 NOT WORKING IS STALE.** Two 19-11 passes fixed it, both with the skill body BYTE-UNCHANGED so gate `086f8267`'s 35/35 still stands and NO re-gate is owed. Pass 1 (commits f559d47/ef2dd8b/60d517c/6c9e442/d814535): the cause was never the model — `runAgentLoop` passed `undefined` for `buildCockpitTools`' clientContext, so every dated follow-up took the `no_clock` refusal; fixture 36 green, run `266ef8f4`, $0.0056. Pass 2 (THIS ONE): `parseCrmOperations` accepted ANY non-empty string as an email, so the agent satisfied the required-`email` brake by INVENTING `email: no-email` — and because `patchPlan` REPLACES `crmOperations` wholesale, that invented row OVERWROTE turn 1's Rhea follow-up, making fixture 36's counts satisfiable by REPLACEMENT rather than by turn 2 declining (a partly vacuous green). FIXED by reusing `isValidEmail` — the repo's ONE email regex, already the send path's bounce rule via `applyRecipientEdit` — at the CRM parse boundary, plus two `CRM_PARSE_REFUSAL` sentences that forbid substituting a placeholder. `patchPlan`'s replace was DELIBERATELY LEFT ALONE: a plan row is the current staged state, not a log, and the data loss came from a REFUSAL reaching `patchPlan`, which cannot happen because every `stageCrmWrite` refusal returns above the mutation. Re-verified `--only 36` run `0b2b6b22`, $0.0057 PASS, green for the RIGHT reason — `agentSteps` shows turn 2 refused TWICE and the plan row still holding turn 1's Rhea op. Full `pnpm test` 9/9 (backend 72 files/1446 tests, core 761), `pnpm typecheck` 10/10 exit 0, `--self-check` PASSES, `check-playbooks` exit 0. Playbooks `contacts-crm.md` (NEW invariant 18: the CRM's address rule IS the send path's — do not write a second validator), `cockpit.md` and `agent-runtime.md` all bumped; `deferred-items.md` item 4 CLOSED, nothing left describing a resolved defect as open. **STILL NOT DONE AND STILL BLOCKING THE PHASE: the owner browser UAT on 19-10 has NOT run, and ACTN-05/PIPE-01 stay unticked. `.planning/REQUIREMENTS.md` was NOT touched (phase-25 lane owns it).** No 19-11-PLAN.md/SUMMARY.md exists — both 19-11 passes were defect-close work recorded in the playbooks and deferred-items, so the phase is still 10 plans. **UPDATE 2026-08-10 (19-12) — BOTH BLOCKING UAT DEFECTS ARE CLOSED AND `e2e/pipeline-uat.spec.ts` IS 15/15 WITH BOTH `test.fail()` MARKERS DELETED, MEASURED SPEND $0.0400.** Commits 135943d + 53eec19 + 96d68d2 + 0beef0d. **(1) THE BROWSER NEVER SENT A CLOCK.** 19-11 fixed the loop; no web caller ever sent `clientContext`, so `stageCrmWrite` took `no_clock` on EVERY human turn and `setSendTime`/`checkAvailability`/`proposeCalendarEvent` took the identical exit — phase 17's calendar tools had been refusing in production for two phases. Fixed with ONE hook, `apps/web/app/(app)/dashboard/workspace/useSendCockpitMessage.ts`, NOT the one-line call-site literal 19-10 proposed: five copies of the field whose omission WAS the defect is five chances to forget it again. All FIVE web callers found by grep and routed through it (ChatPane, cards.tsx regenerate, SegmentAnatomy, AbnormalBriefBanner, PostCall); `tz` degrades to UTC in a try/catch because `undefined` into a `v.string()` throws the whole turn away, and `nowMs` is read inside the callback so a long-mounted pane cannot send a stale instant. **UAT step 7 now stages an `addFollowUp` with a finite `dueAt` from a real browser turn**, and NEW step 7c proves the calendar half structurally: the plan row carries `eventTz: Africa/Dar_es_Salaam`, and `clientContext.tz` is that field's only writer anywhere. **(2) THE WITHHELD REPORT WAS UNREACHABLE BY CONSTRUCTION.** New optional `plans.withheldRecipients`, written by `executePlan` in the same patch as the counters, rendered by the new pure `@pikar/core` `withheldNote()` on BOTH post-approve surfaces (cockpit `PlanCards` above the REPORT card, Approvals `InFlightRow`), `role=status` in `--ink-soft`. The dead `res.withheld` -> `setNote` branch is deleted. **THE RULE: any UI state produced BY a status transition must not live in a component gated ON that status — it is unreachable and every offline test still passes.** **DECISION RECORDED — the implied partial write needs NO new refusal:** `stageCrmWrite` is already all-or-nothing per call (`staged` is local, every refusal returns above the single `patchPlan`) and 19-11's `followUpRefusedThisTurn` already refuses the contact-only retry; the live sentence about Jane was the model reporting step 5's already-approved contact, and that turn's plan row held no ops at all. **THE COSTLIEST FINDING IS NEITHER DEFECT: `convex dev` TYPECHECKS BEFORE IT PUSHES.** `packages/backend/convex/cash.ts` (2 errors from the concurrent finance lane's commit f36cb8b, untouched here) had been blocking every deploy since it landed, so the watcher silently kept serving the LAST GOOD BUILD and TWO full UAT runs measured a defect already fixed in source. Recovered with `CONVEX_LOCAL_BACKEND_STARTUP_TIMEOUT_SECS=180 npx convex dev --typecheck=disable` from `packages/backend` — **the watcher on :3210 is STILL RUNNING WITH THAT FLAG; drop it once the cash lane catches up.** The same 2 errors also fail `pnpm --filter @pikar/web build` (worked around with a TEMPORARY `ignoreBuildErrors` in next.config.ts, restored and never committed — `git diff apps/web/next.config.ts` is empty) and are the ONLY red in `pnpm typecheck` (8/10 tasks green). **VERIFIED: `pnpm test` 9/9 tasks, 140 files / 2724 tests, ZERO failures** (backend 72/1446, core 34/783, web 10/149, vault 12/170, voice 5/57, cost 2/56, extraction 2/28, contracts 2/27, pii 1/8). Both new guards mutation-proven red-able. UAT step 9(a)'s blanket `dropped address appears NOWHERE` was NARROWED — not weakened — to `nowhere except inside the withheld note`, because 9(b) makes the blanket form logically impossible; it clones the pane, removes the note and requires absence from the remainder. `beforeAll` got a 180s SETUP budget (a real signup plus three `convex run` CLI round-trips never fit Playwright's 30s default against a cold backend); no assertion relaxed. Playbooks bumped: `cockpit.md` (closure block at the top; the two 19-10 blocks describing these as OPEN are marked SUPERSEDED in place), `contacts-crm.md`, `dashboard-pages.md`, `voice.md`, `onboarding.md`; `deferred-items.md` carries the cash.ts blocker. The skill body was NOT touched — `cockpit-agent@18` is byte-identical and gate `086f8267` still stands. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it); ACTN-05/PIPE-01 remain unticked pending owner sign-off. No `19-12-PLAN.md`/`SUMMARY.md` exists — like 19-11 this was defect-close work, so the phase is still 10 plans. Servers left up: convex dev on :3210 (with `--typecheck=disable`) and a FRESH production Next build on :3111. NEXT: owner sign-off on the UAT, then tick ACTN-05/PIPE-01 and close phase 19."
last_updated: "2026-08-09T22:48:11.831Z"
progress:
  total_phases: 51
  completed_phases: 30
  total_plans: 343
