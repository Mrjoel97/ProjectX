...
current_plan: 4 (of 7 — 19.1-04 complete)
status: in_progress
stopped_at: "19.1-04 COMPLETE — the two public functions the CSV import panel drives are live in the ONE substrate module (`c887528`, 3 files, ONE commit, $0.00). **`matchExisting`** (tenantQuery) answers, per REQUESTED address in request order, `{ email, exists, empty }` where `empty` names which of name/company/phone/title are unset — one `by_tenant_email` indexed read each, the `suppressedAmong` shape, NO new index. That is what lets the preview state `N new · M enriched · K unchanged` BEFORE any write: the counts are a projection of this answer, not a guess. An array longer than `IMPORT_MATCH_CHUNK` is REFUSED with `IMPORT_MATCH_TOO_MANY` **before any read** — an implicit slice would silently under-report and the preview would promise a write it never makes. Addresses are normalized SERVER-side. **`importContacts`** (tenantMutation) refuses `> IMPORT_BATCH_ROWS` with `IMPORT_BATCH_TOO_LARGE`, refuses a blank-after-trim attestation with `CONSENT_WORDING_REQUIRED` BEFORE the loop, then per row re-runs `normalizeAddress` + `isValidEmail` — **RE-VALIDATED, NOT TRUSTED**, the `parseCrmOperations` discipline — and calls `upsertContactRow` with `fillEmptyOnly: true`, `origin: imported` and the `imported-attested` consent block. **A BAD ROW IS A `rejected` ENTRY, NEVER A THROW:** one hand-crafted address must not discard the 99 good rows sharing its transaction — and the mutation-proof shows exactly that, since dropping the re-validation makes the WHOLE batch throw `CONTACT_EMAIL_REQUIRED`. **THE COUNTS ARE DEFINED OVER THE FOUR MAPPABLE FIELDS ONLY:** a contact that gained a consent record but no field values counts as `unchanged`, or `unchanged` would be structurally always 0 and the preview a lie. **THE BATCH NUMBERS ARE DOCUMENTED BY THEIR REAL REASON** — the comment says outright that 100 rows is ~1% of every hard Convex limit and that every limit would permit the whole 1,000-row file in ONE call, so retry blast radius / progress granularity / OCC window is the justification and a limit must never be cited. **AN IMPORT WRITES NO AUDIT ROW, DELIBERATELY** — `assertConsent` writes none either, this module whole audit surface is still ONE event (`contact.unsuppressed`) under a key-set EQUALITY test, and CLAUDE.md §4 governs what a payload may CARRY, not that every write must have one. The strongest guarantee that an address or the attestation wording never becomes a payload is that the path emits NO payload, and the §4 test now runs a REAL import inside itself and asserts that as a **ROW COUNT** before falling back to the substring scan. **14 NEW TESTS (87/87, was 73), EVERY STORED-STATE ASSERTION READING `contactRows(h)` BACK.** Both functions are isolation-tested asA/asB and both joined `COVERED`, now **12 entries** — 19-13 established that bumping the pin without the test defeats it. The PIPE-01 structural scan now also covers `core/src/contactImport.ts`. **FOUR MUTATION-PROOFS, RED AND REVERTED:** dropping `IMPORT_MATCH_TOO_MANY` gives `promise resolved [ { email: p0@x.com, …(2) }, …(500) ] instead of rejecting`; dropping `isValidEmail` gives `CONTACT_EMAIL_REQUIRED` (whole batch down); `fillEmptyOnly: false` gives `expected S. CHEN (OLD CRM) to be Sarah Chen` PLUS `expected { created: +0, enriched: 1, …(2) } to deeply equal { created: +0, enriched: +0, …(2) }`; removing `importContacts` from `COVERED` gives `expected [ assertConsent, …(11) ] to deeply equal [ assertConsent, …(10) ]`. **ONE DEVIATION WORTH CARRYING FORWARD: THE PLAN TIE PREMISE WAS WRONG.** It states that rows inserted in one transaction share a `Date.now()`; written that way the test MEASURED 19 distinct timestamps across 30 inserts — **convex-test clock ADVANCES mid-transaction where the real backend freezes it**. The 30-way tie is therefore set with a SENTINEL (`createdAt: 1_234`) and the one-distinct-value precondition is ASSERTED before the paging loop; every id is then reached exactly once through `listContacts` at `limit: 10`. Any future test relying on an in-transaction clock freeze under convex-test has the same hole. MEASURED: `contacts.test.ts` **87/87**, backend `tsc --noEmit` exit 0 (delta 0), `pnpm typecheck` **10/10**, biome clean on both code files, `graphify update` + `extract-convex-edges` re-run. Full backend suite showed `vaultDigest.test.ts` + `intake.test.ts` red — the KNOWN shared-fork hazard: `intake.test.ts` is 10/10 ALONE, `vaultDigest.test.ts` fails 1 alone at this commit and contains ZERO references to `contacts`, and the only two dirty code files in the tree were mine. **NOT DONE AND NOT CLAIMED:** no panel, no `Dropzone`, no e2e — nothing in the product reaches either function. `pipelineTiles` still takes exactly `SCAN_LIMIT` and reports NO bound, so a max-size import into a non-empty book will under-count all four tiles until **19.1-05**. **FOREIGN LANES:** ROADMAP.md, four `20*`/`20.1*` plan files and `graphify-out/*` were dirty at start and were not staged; `check-playbooks` reports a block for **`vault.md`** (`vaultDrive.ts`, the Drive lane) — NOT this plan; `contacts-crm.md` is satisfied. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it)."
last_updated: "2026-08-10T20:20:00.000Z"
progress:
  total_phases: 52
  completed_phases: 31
  total_plans: 365
