...
current_plan: 5 (of 7 — 19.1-05 complete)
status: in_progress
stopped_at: "19.1-05 COMPLETE — `pipelineTiles` REPORTS ITS SCAN BOUND (`e860094`, 5 files, ONE commit, $0.00). **THE BUG:** it took exactly `SCAN_LIMIT` (1 000) on all three scans with NO probe row and returned four bare integers, while its two siblings `listContacts`/`listUnassignedFollowUps` have taken `SCAN_LIMIT + 1` and reported their bound since 19-07. Nothing caps a tenant's contact count — the 1 000-row ceiling is PER IMPORT — so ONE max-size CSV import into a book that already held a SINGLE contact silently under-counted all four ALWAYS-KNOWN tiles (playbook invariant 3). **WHY PHASE 19'S VERIFICATION COULD NEVER SEE IT: the e2e assertion is `Number.parseInt` + `Number.isInteger`, and A WRONG INTEGER IS STILL AN INTEGER.** **THE FIX:** all three scans take `SCAN_LIMIT + 1` (the extra row is a BOUND PROBE, never counted), every count is computed over `slice(0, SCAN_LIMIT)` so none can exceed the bound, and the return carries `partial: \"row-cap\" | null` — ONE nullable literal, not three per-scan flags, because the tiles are read as a SET and any capped scan makes every one of the four a FLOOR rather than a total. The page renders `1000+`. **`1000+` IS A FLOOR STATED HONESTLY, WHICH IS NOT THE HEDGE INVARIANT 3 BANS** — the page still knows a number, it just knows there are at least that many — and `Number.parseInt(\"1000+\", 10)` is still `1000`, so the e2e integer-parse assertion keeps working and `pipeline-uat.spec.ts` was NOT touched (touching `apps/web/e2e/` would also have demanded a second playbook bump for zero behavioural gain). A real zero is still `0`. **OWNER DECISION 2026-08-10, HONOURED: FIX THE HONESTY BUG AT ITS ROOT, DO NOT MERELY RAISE THE CAP.** `SCAN_LIMIT` is shared by FIVE readers including `savedForName`, which the cockpit calls on EVERY contact resolution; raising it charges four other read models for one reader's symptom and the first book past the new number is wrong in exactly the same way. The recorded upgrade path is a counter or an aggregate component, NEVER a bigger cap. **TWO DEVIATIONS, BOTH ABOUT A SITE `tsc` WOULD HAVE LET THROUGH.** (1) The plan's render form was TWO JSX children (`{tiles[tile.key]}{... ? \"+\" : \"\"}`); shipped as ONE TEMPLATE LITERAL instead, because invariant 3's enforcement test counts occurrences of the literal substring `\">0<\"` and a value split across two text nodes would have made that assertion's meaning depend on a `renderToStaticMarkup` detail. (2) `TILES` was `satisfies ReadonlyArray<{ key: keyof Tiles }>` and `keyof Tiles` now ADMITS `partial` — adding it would have rendered the string `row-cap` in a stat cell with nothing for `tsc` to say, the exact 19.1-02 `ORIGIN_LABELS` defect class. `TILES` is now keyed on a DERIVED `TileCountKey` (the keys of `Tiles` whose value is a `number`): a new COUNT stays usable, a new NON-COUNT is a compile error. **TWO EXISTING `toEqual` SITES HAD TO GAIN `partial: null`** (the empty-tenant test and the asA/asB isolation test) plus the `Object.values(tiles)` typeof-number loop narrowed to a named `COUNT_KEYS` tuple — 'the new field is additive' is NOT true of a `toEqual`. **TESTS +3:** backend, 3 contacts → four exact counts AND `partial: null` (the non-vacuity floor) and `SCAN_LIMIT + 1` contacts → `partial: \"row-cap\"` with `needingAttention` exactly 1 000 and every count `<= 1 000`; web, the `row-cap` case asserting `>1000+<`/`>12+<`/`>4+<` AND that `—`/`Unknown`/`row-cap` still appear nowhere and there are still exactly FOUR tiles; plus the exact-integer case strengthened with `not.toContain(\"+\")`, the other direction. The 1 001-row seed is split across four `t.run` calls of 260 to stay inside the 20s `testTimeout` — THE THRESHOLD WAS NOT LOWERED, the whole point being that the test fails on the code that shipped before this plan. **PRE-IMPLEMENTATION RED:** `expected undefined to be 'row-cap'` (the field did not exist). **TWO MUTATION-PROOFS, RED AND REVERTED:** reverting the contacts scan to `.take(SCAN_LIMIT)` gives `expected null to be 'row-cap' // Object.is equality` (the SUBTLER regression — `partial` present but the probe gone — which the old assertion could never have distinguished); dropping the `+` suffix gives `expected '<section class=\"stat-grid\" aria-label...' to contain '>1000+<'`. MEASURED: `contacts.test.ts` **89/89** (was 87), `@pikar/web` **171/171** across 10 files (was 170), `pnpm typecheck` **10/10**, backend `tsc --noEmit` exit 0 (delta 0), biome clean on all four code files, `check-playbooks` **exit 0 with no output** (`contacts-crm.md` satisfied — no foreign block this run). **NOT DONE AND NOT CLAIMED:** no panel, no `Dropzone`, no e2e — nothing in the product reaches `matchExisting`/`importContacts` yet (19.1-06/07). This plan makes the CONSEQUENCE of a max-size import honest before the surface that produces one exists. **SHARED WORKING TREE:** `.planning/ROADMAP.md` was dirty with phase-20/20.1/21 edits AND `docs/README.md` + `docs/governance/iso-9001-conformance-map.md` were already STAGED by another lane — the ROADMAP was staged HUNK-ONLY via a `git hash-object -w` blob built from `HEAD` + this plan's three edits, and the doc commit was built through a TEMPORARY `GIT_INDEX_FILE` so the other lane's staged files were neither committed nor unstaged. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it)."
last_updated: "2026-08-10T21:00:00.000Z"
progress:
  total_phases: 52
  completed_phases: 31
  total_plans: 365
