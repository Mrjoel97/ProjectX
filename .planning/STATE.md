...
current_phase: 25.1
current_plan: 3 of 7 (phase 25.1 wave 3 complete)
status: executing
stopped_at: "Completed 25.1-03-PLAN.md 2026-08-21 (D5-D8, TDD, 5 commits d4bd4cb/c544228/8f7c01b/8693d33/b1a1c9e, $0). BOTH MEDIA KINDS NOW REACH THE VAULT AT THEIR OWN SUCCESS TERMINALS. D5: mediaComplete.saveImageToVault runs inside landResult's success arm - NOT in media.ts as the plan said, because media.ts:1174 is storeAndLand (an ACTION helper that cannot insert) and only ONE of three callers; landResult is the single terminal the fal webhook, storeAndLand AND the 25.1-02 sweep all route through, and putting the save in media.ts would also have created a mediaComplete->media import CYCLE. The doc mirrors the reel: mimeType text/markdown (the reviewed PROMPT is the text, startIngest embeds it), storedMimeType = the image bytes, sourcePlanId, and category IMAGES (a deliberate divergence from the reel's workspace-docs - categoryFor is handed the BYTES' mime and its own rule decides; the user looks under Images). THE PLAN'S SCHEMA PREMISE WAS WRONG: vaultDocuments.kind is v.string(), there is no union to widen - only mediaJobs.vaultDocId was added, PER JOB not per plan (D8 lets one plan hold several images). D6: the !captionsComing gate is GONE, not narrowed - captions are pinned on for every reel so that gate was TRUE at every render terminal that has ever run, making the caption burn the only save site in practice; a stalled/swept caption pass therefore published a reel that NEVER reached the vault. Consequence recorded in media.md: the vault doc now tracks the plan's CURRENT final at every terminal, so on a re-render the superseded captioned cut is released one terminal earlier - a 33-05 test changed subject to say so. D7: resetPlan clears reelVaultDocId (saveReelToVault UPSERTS on it, so reel #2 patched reel #1's doc and deleteOrphanedFinals then deleted reel #1's mp4 - destruction, not staleness); deleteOrphanedFinals itself needed NO change, it reads the live set fresh. D8: generateImage refuses only on queued|submitted - succeeded was a PERMANENT lock because nothing ever deletes a mediaJobs row. SEVEN MUTATIONS, each reddening exactly one test - and TWO OF MY OWN TESTS WERE VACUOUS FIRST PASS: the mediaMode exclusion passed with the discriminator deleted (the reel fixture had no imagePrompt, so the prompt's absence decided it) and the tenant cross-check had no fixture that ever crossed a job and a plan. Both rewritten until only the guard can decide. llmRedaction pins 12->13 literals, 7->8 media audit sites (mediaComplete 1->2), no new allow-list key needed. ALSO FIXED, from wave 1 not this plan: isolation.test.ts was RED on plans.by_render_run - 25.1-01 added the third retrier run-id index and never ran that suite. ONE RED LEFT ON THE BRANCH and it is 23-05's, not media's: skills.activateAgentCandidate's owner gate is UNPROVEN (no OWNER_ARGS fixture, so the non-owner call dies on the validator before the owner check) and the guard that should catch that is VACUOUS BY CONSTRUCTION (REQUIRES_ARGS is derived from OWNER_ARGS' own keys) - written up in the phase's new deferred-items.md with the other two deferrals. OPERATIONAL NEAR-MISS worth not repeating: I ran `git stash --keep-index` in this SHARED worktree; the pop restored every foreign modification and packages/core/src/actionType.ts came back line-ending-normalized with content byte-identical to HEAD, so nothing was lost - but NEVER git stash in this tree. Measured: media+plans+renderReel+llmRedaction+reliabilitySweep 377 passed/24 skipped, full backend sweep 2223 passed across 90 files, apps/web 435 passed, backend tsc clean, biome clean on the five changed source files, check-playbooks exit 0. schema.ts and cockpit.md were staged SURGICALLY again (foreign hunks left unstaged, via a filtered git apply --cached). NOT SEEN IN A BROWSER: the image doc is a new row shape in the vault's Images tab. Counters below carried forward +1 completed plan, NOT re-derived - `gsd-tools state update-progress` REPLACED this whole block with a phase-33 one and had to be reverted. Next: 25.1-04 (wave 4)."
