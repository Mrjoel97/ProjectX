...
current_plan: 8 of 10 (phase 33)
status: completed
stopped_at: "Completed 33-08-PLAN.md - the grounded citations and the clear-failure cards. THE CANVAS IS COMPLETE. Every scene that states a figure now shows WHERE IT CAME FROM - the vault doc title, clickable through to PreviewModal via cards.tsx's VaultDocButton (exported, not copied: it already routes through api.vault.vaultDoc, whose null-for-another-tenant answer IS the guarantee a citation click-through needs) - or shows that nobody has vouched for it, with the one confirm click that changes that. THE PLAN NAMED THREE CITATION STATES; THERE ARE FOUR. A verified:false source (foreign/malformed/deleted id, deliberately indistinguishable) CANNOT render as needs_confirmation: confirmClaim answers not_a_claim for a scene the parser never flagged, and firstUnconfirmedClaim - the gate on BOTH jobEstimate and the reserve - keys on needsConfirmation alone, so that badge would promise a button that can only refuse AND announce a block that is not there. So `unverified` is its own state: plain text, no link, no button, no count. The link is computed ONCE above every branch, so no arm can mint one from an unverified row. blockLine counts the claims beside the disabled button; the estimate's refusal stays the authoritative why. failureCards renders four families through ONE card: per-scene, render (free Retry render), HELD (NO ARM OF ITS OWN - it points at the failed scene's card, because a retry over a hole buys a second sandbox) and CAPTION (degraded deliverable, no re-burn mutation exists, 20-17's rule said to the user). THE MONEY IS THE POINT: sunk = actualCents when a face LANDED and its estUsd reservation when it FAILED (media.ts: actualCents stays absent if it failed; UNLANDED_RESOLVES.media is false) so the line says SPENT, never pending; the retry price adds the estimate's OWN render line because regenerateBlock reserves one, and with no estimate loaded it says "plus the re-assembly" rather than understating. A kind switch is FREE and buys nothing - the cheaper picture is bought by the regenerate that follows, a fortieth of a clip - and text_card is the arm that ends a hold with no spend at all. SWAP-TESTED BY TRANSPOSITION, not deletion: swapping the two rendered money expressions in the implementation (both numbers still present, wrong labels) reddens 4 tests; the fixtures keep them unequal ($0.52 sunk, $0.47 adds) for exactly that reason. failureClause gives an unknown code a generic sentence so a code never becomes prose; every card prints its code once, in .trace-line, underneath. media.byPlan's JobFace gained failureReason (2 lines) so the code travels on the SAME face whose status the card describes - assetUrls carries it too but returns every attempt, so on a regenerated scene the two can name different rows. FOUND AND DEFERRED: a cited doc's TITLE is model-authored even when verified is true (verified answers only "is that docId yours?") - the provenance-laundering class one door further along, ~3 lines to fix in sceneCitations, written up in the phase's deferred-items.md for 33-09/33-10. mediaCanvas.test.ts 78 -> 101; apps/web 376 green; backend media+llmRedaction+importGuard 382 green; tsc clean both sides. STATE.md still carries six accumulated frontmatter blocks, both top blocks hand-edited. DO NOT RUN state advance-plan ON THIS FILE. NOT browser-verified - 33-10 owns that gate and now also owes e2e assertions for a citation chip opening the real doc, a confirm click clearing the Generate block, and a held reel routing to the failed scene's fix menu. Next: 33-09."
