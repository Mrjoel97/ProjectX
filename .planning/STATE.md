...
current_plan: 9
status: in_progress
stopped_at: "Phase 26 Plan 26-08 COMPLETE (2/2 tasks, commit cadbb28) — **ALL THREE SPEND RAILS ARE NOW INSTRUMENTED.** NOTHING IS LIVE-VERIFIED; no media has been generated and $0 spent on this rail. Media writes ONE `reserved` row per BATCH (`mediabatch:<batchId>`, whole-job estCents — per-line rows would NOT sum back because chooseMediaBatch floors the TOTAL once, D12a) and one `actual` row per landed line (`mediabatch:<batchId>:<jobId>`). **THE LEDGER RECORDS actualCents, NOT THE LIMITER DELTA — the ONLY rail where the two planes diverge in AMOUNT, deliberately.** The limiter took the whole estimate up front and never refunds (20-04) so it needs only the overrun; the ledger answers what the LINE COST. Recording the delta would report an ordinary clip that came in at or under estimate as costing NOTHING — every normal landing. reserved minus actual IS the never-returned over-reservation and aggregateSpend already reports it as `unlanded`. A FAILED line writes NO movement (stays unlanded, never zero). **THE MEDIA RAIL HAS NO REFUND PATH AND A TEST ASSERTS ZERO `refunded` ROWS ON IT** — if one appears, either the rail grew a credit path (argue it) or something is minting money. BOTH sites DERIVE (approve CAS = reserve-once; a re-delivered fal webhook is a replay); the `<jobId>` segment is LOAD-BEARING because every line of a batch shares one batchId. **BOTH WRITES GUARDED ON > 0 AND THE ZERO CASE IS REAL:** a voice take can price under half a cent, and a zero-cent movement is REJECTED — inside landResult that throw ABORTS THE LANDING and fails a sub-cent take's own webhook. Rounding up to 1c would invent money. Five existing audio tests caught it immediately. **BEST FINDING OF THE PLAN: a mutation SURVIVED.** Collapsing the landing correlation to the batch passed all six tests, because replay identity is (tenant, correlation, phase) and reserved != actual MASKED the collision — the real defect was between SIBLINGS (a 13-line reel would record one clip and lose twelve). Fix was a NEW TEST, not a code change. RULE: a surviving mutation is a COVERAGE HOLE, not a pass. **GATES:** backend 1284/1284, typecheck clean, check-playbooks exit 0, 3/3 mutations now caught by three different tests. **NEXT: 26-09** builds the Finance reads over spendLedger.listEvents + aggregateSpend. TWO THINGS IT MUST HONOUR: (1) startCoverage has STILL never been called anywhere, so every tenant correctly reports `unknown` — 26-09 is the first plan that may legitimately open it; (2) on the media rail `unlanded` is the PERMANENT by-design over-reservation, not a transient in-flight balance. **DO NOT RUN `gsd-tools state advance-plan`** — it corrupted this block on 26-06. Hand-edit. **SHARED TREE:** the blueprint lane commits here continuously; never `git add -A`."
