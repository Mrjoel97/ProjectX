...
current_plan: 3 (of 7 — 19.1-03 complete)
status: in_progress
stopped_at: "19.1-03 COMPLETE — the shared contact writer learned import semantics WITHOUT moving its two existing callers (`f281fce`, 3 files, ONE commit, $0.00). `upsertContactRow` now takes `company`/`phone`/`title`, a `fillEmptyOnly` FLAG and an optional `consent` block, and returns `{ id, created, filled }`. **THE FLAG IS DEFAULT-OFF AND THAT IS THE WHOLE POINT:** `fillEmptyOnly: false` keeps the OLD rule ('a blank name must not ERASE a name on record', where a non-blank one STILL overwrites), so hand-add and the Approve-gated applier behave exactly as before — playbook invariant 13 (ONE implementation of the write rule) stays true with a flag, not a second writer. **CONSENT IS WRITTEN ONLY ONTO A ROW THAT HAS NONE** (`existing.consentAt === undefined`) and is never patched otherwise in either direction — a batch attestation may not replace per-person `asserted-by-user` evidence. The four consent columns are folded into the SAME insert/patch (zero extra writes); `assertConsent` is deliberately NOT called per row because it re-reads the row you just wrote and HARDCODES the wrong source. `filled` counts the FOUR MAPPABLE FIELDS ONLY — a consent-only write returns `filled: 0`, or 19.1-04's `unchanged` would be structurally always 0 and the preview a lie. A call that changes nothing SKIPS `db.patch` entirely, so an `unchanged` row does not move its `updatedAt`. **EIGHT NEW TESTS, EVERY ONE ASSERTING ON ROWS READ BACK** (a reply agrees with itself; that is how 'don't erase' passed as 'fill empty only' for a whole phase), including the NON-VACUITY floor — hand-add with a non-blank name STILL overwrites, without which deleting the flag entirely would leave the import-side tests green. Consent wording is compared against `IMPORT_ATTESTATION` from `@pikar/core`, not a re-typed copy. Tests 6 and 7 patch a SENTINEL (`1_234`) instead of reading a clock, so a same-millisecond rewrite cannot pass as `unchanged`. **ALL THREE GUARDS MUTATION-PROVEN RED AND REVERTED:** deleting the `fillEmptyOnly` guard takes down THREE tests (`expected 'S. CHEN (OLD CRM)' to be 'Sarah Chen' // Object.is equality`, `expected 'Stale Name' to be 'Sarah Chen'`, `expected 1 to be +0`); dropping the `consentAt === undefined` condition gives `expected 'imported-attested' to be 'asserted-by-user'`; patching unconditionally gives `expected 1786375538546 to be 1234`. MEASURED: backend `tsc --noEmit` exit 0 (delta 0), `pnpm typecheck` 10/10, `contacts.test.ts` **73/73** (was 65), full backend **72 files / 1502 passed CLEAN ON THE FIRST RUN** (no shared-fork hazard), biome clean on both code files. **DEVIATIONS:** (1) added a `CONSENT_WORDING_REQUIRED` floor to the helper — the same guard `assertConsent` carries, placed at the shared writer because 19.1-04 takes the attestation off the wire; (2) took the plan's ONE-commit instruction over per-task commits, as 19.1-02 did; (3) RECORDED HONESTLY — the no-op skip DOES change one hand-add behaviour: a nameless re-save no longer moves `updatedAt`. No test asserted the old behaviour and the plan requires the skip, but 'hand-add is byte-identical' is true of its FIELD semantics, not of that timestamp. **NOT DONE AND NOT CLAIMED:** nothing calls the flag yet — no `matchExisting`, no `importContacts`, no panel, no e2e; 19.1-04 is the first caller and is what turns `created`/`filled` into the preview counts. `COVERED` untouched (no new public function). **FOREIGN LANES IN THE SHARED TREE:** `packages/contracts/skills/*` went dirty MID-EXECUTION (skills lane) and ROADMAP.md + four `20*`/`20.1*` plan files were dirty at start (media/Drive lanes) — none touched or staged. `check-playbooks` therefore reports a block for **`skill-registry.md`**, which is the skills lane's uncommitted change, NOT this plan; `contacts-crm.md` is satisfied. ROADMAP.md was staged HUNK-ONLY via `git hash-object`/`update-index`. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it)."
