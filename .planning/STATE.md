...
current_phase: 25.1
current_plan: 2 of 7 (phase 25.1 wave 2 complete)
status: executing
stopped_at: "Completed 25.1-02-PLAN.md 2026-08-21 (D3+D4, TDD, 3 commits 8183ff8/089378a/28925cc, $0). ONE 30-MIN CRON NOW TERMINALIZES EVERY SEVERED SCHEDULER CHAIN. reliabilitySweep.ts = two @convex-dev/migrations sweeps (the vaultSweep shape) behind crons.interval('reliability-sweep',{minutes:30}) -> runSweep, which runOne()s BOTH with {reset:true} (a completed migration no-ops on a bare invocation - the stranded-.xlsm lesson). THE JOB SWEEP WRITES NO TERMINAL OF ITS OWN: a >45min 'submitted' row is landed through mediaComplete.landResult, so it reconciles spend, emits its one media.landed audit line, and cascades the render trigger to incomplete_batch through the ORDINARY path - which is also why llmRedaction's 'only media.ts+mediaComplete.ts write a terminal mediaJobs status' pin still holds. The plan sweep terminalizes stale rendering/armed-pending (>60min, watchdog_render_timeout) and stranded transcribing (>30min, watchdog_caption_timeout, renderStatus untouched), and flips a dead-dispatch collecting+kind:memo row to PROPOSED with a fixed system-voice body (D4 - PlanCard renders only at proposed, so that row was invisible for ever; no new status union member was needed). notification kind watchdog.stalled is DELIBERATELY outside NOTIFICATION_KINDS - that list arms notifyExternal.dispatch and reaches a Gmail token. FOUR DEVIATIONS, all in 25.1-02-SUMMARY.md. THE ONE WORTH REMEMBERING: the inherited draft suite from the dead executor was VACUOUS on the in-flight guard - deleting `if (inFlight(jobs)) return false;` left the WHOLE suite green, because both in-flight fixtures seeded updatedAt=now-1000 (fresh), so the staleness clock decided them and the guard never ran. Fixed by seeding them stale. Same class as phase 19. ELEVEN mutations now each redden exactly one test; the exclusions (un-armed pending, in-flight, fix-menu deck hold, caption-waits-for-render, renderRetriedAt, the media.render_retry_manual AUDIT row - retryRender stamps no plan field so that row is a manual retry's only clock, scheduler liveness by args.planId) are the load-bearing half. Also: the draft dropped the plan's collecting THRESHOLD entirely; COLLECTING_STALL_MS is back as a cheap pre-filter because age ALONE is unsafe (plan rows are RECYCLED - resetPlan keeps the old createdAt) and liveness alone would scan _scheduled_functions per plan row. _generated/api.d.ts was hand-regenerated (npx convex codegen refuses without a local backend on :3210) and carries 17-09's calendarEvents catch-up in the same commit. cockpit.md was staged SURGICALLY again - the foreign lane's 5-line entry stayed unstaged, as did schema.ts/calendar.test.ts/README/graphify-out/actionType.ts. Measured: reliabilitySweep 23/23, media+renderReel+evaluations+dispatch 401 passed/24 skipped, llmRedaction+importGuard+worm+vaultSweep+sweep 210 passed, backend tsc clean, biome clean, check-playbooks exit 0. THE SWEEP HAS NEVER RUN AGAINST A REAL DEPLOYMENT - watch its first live pass for a false positive on a slow render. Counters below carried forward +1 completed plan, NOT re-derived. Next: 25.1-03 (wave 3, media vault persistence D5-D8 - it can move the reel vault save to the render terminal knowing the sweep only TERMINALIZES a stranded caption and never attempts the save)."
last_updated: "2026-08-21T11:05:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 412
