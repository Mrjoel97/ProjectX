...
current_plan: 7 of 10 (phase 33)
status: completed
stopped_at: "Completed 33-07-PLAN.md - the guided-intake chips and the two-deck switcher. The media canvas now opens with the BRIEF: briefChips(brief, deckLocked) renders the parsed ask as five editable chips ABOVE the hero, and it is the only place a mis-parse is visible before money moves. ONLY TOPIC AND LENGTH ARE REQUIRED and that is a PHASE-11 OBLIGATION, not a preference - a chip row demanding an audience would re-gate exactly the idea-stage tenants Phase 11 admitted, so audience/tone/brandVoice render empty-but-never-blocking. A brief.defaulted field carries a 'from your profile' MARKER IN WORDS (never colour) and stays editable, because editing it is what makes it the user's (editBrief drops patched fields from defaulted). THE LENGTH CHIP CANNOT PRODUCE ITS OWN REFUSAL: its options ARE TARGET_DURATIONS imported from @pikar/core/storyboard - literally the set media.editBrief validates against - rendered as a native <fieldset> of aria-pressed buttons, so illegal_duration is unreachable from the control. The 60 s cost note has ONE CARRIER (on the option while 60 is not current, on the chip once it is), so the component renders both slots with no conditional and the sentence appears once. NOTHING AUTO-FIRES: a chip edit calls editBrief and stops; deckStale(briefChangedAt, deckProposedAt) - true iff BOTH stamps exist and the brief moved last - raises the badge, and the free 'Re-propose' button sends ONE canned message through useSendCockpitMessage into THIS thread. THE HOOK IS CALLED IN THE CANVAS, NOT LIFTED AS A PROP (plan preferred the prop): six surfaces already instantiate it including cards.tsx which MOUNTS this canvas, and the canvas has TWO mount points - threading a callback through both is two chances to forget it, the exact 19-12 defect the hook exists to kill. Same hook, same action, same clock = no second UI->dispatch door. variationView folds BOTH decks through ONE deckSummary (concept, scene count, KIND_LABEL mix, SUMMED durations - never targetDurationSeconds); switchDeck needs no estimate wiring since jobEstimate prices whatever is in shots. AFTER GENERATE THE SWITCHER IS ABSENT, NOT GREYED, and every chip goes read-only off the same deckLockedAt flag - a greyed switch can only ever answer deck_locked. NO logic entered MediaCanvas.tsx. Web 351 green (mediaCanvas 59->78), tsc clean in apps/web AND packages/backend, core storyboard 92 green, check-playbooks exit 0. Touched NO backend file, so the llmRedaction log-plane pins were untouched again. GSD-TOOLS HAZARD RE-CONFIRMED AND REVERTED: `state advance-plan` PREPENDED a bogus third frontmatter block (current_plan '7 (done)', total_plans 7, status verifying, completed_plans 288, stopped_at 'Completed 33-06') AND rewrote block 2's status - reverted with git checkout, both blocks hand-edited. DO NOT RUN state advance-plan ON THIS FILE. NOT browser-verified - 33-10 owns that gate and now also owes e2e assertions for chip-edit->badge, one-turn re-propose, and the compare region vanishing after Generate. Next: 33-08."
last_updated: "2026-08-16T15:40:00.000Z"
progress:
  total_phases: 53
  completed_phases: 31
  total_plans: 412
