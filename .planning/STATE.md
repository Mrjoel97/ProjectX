...
current_phase: 25.1
current_plan: 5 of 7 (phase 25.1 wave 5 complete)
status: executing
stopped_at: "Completed 25.1-05-PLAN.md 2026-08-21 (D11, TDD, 4 commits 876c5ad/b3e7052/bf99f70/e1d9aa3, $0). A PAID RESEARCH TURN NOW READS AS A DOCUMENT WITH CHECKABLE REFERENCES. D11a: the sources were computed from the search tool's own RESULT parts, deduped by URL, billed for, audited as a COUNT and written into the vault document - and then DROPPED ONE HOP SHORT of the plan row, so the one surface that ALWAYS exists (the card) was the one with no provenance at all. Worse than it sounds: persistResearchFindings is SKIPPED ENTIRELY on a zero-search run (16-09's structural floor) and SWALLOWED on a persist failure, so the vault was never a reliable second home for them. plans.sources ({title,url,retrievedAt}[], optional, additive, no migration) now lands with the body it attributes, threaded through dispatchAndLand's landing object into landSpecialistResult. THE WRITE DOOR WAS CHOSEN, NOT DEFAULTED: sources is deliberately NOT a patchPlan arg - patchPlan is what the model's own cockpit tools write through and a source list is a PROVENANCE CLAIM ('these pages were read for this memo'), so it goes in by DIRECT ctx.db.patch from args the model cannot reach. That is the calendarEventId/calendarRunId rule applied to a new field, and it is the fourth door of the provenance-laundering class this repo keeps shipping. resetPlan clears it (one line, plans.ts, outside the plan's file list) or a stale list would sit under the NEXT memo as if those pages had been read for it. Omitted-when-empty, never [] - the card renders on PRESENCE and the guard also stops a later re-land blanking a filled list. SECTION-4 HELD AND IS NOW PROVEN RATHER THAN ASSERTED: dispatch.test.ts's audit scan walked every row of a real research run looking for the URLs, but on a run where the plan row never received them - true of a run in which no URL went ANYWHERE. It now stages the landing and ends by asserting the URLs ARE on the plan row. Same run, both halves. llmRedaction.test.ts was NOT TOUCHED and did not need to be: no audit payload gained a field, so dispatch.ts's payload-count pin stays at 12. D11b: MemoCardBody replaces the white-space:pre-wrap paragraph with the EXISTING MarkdownDocument (compact) - no second renderer, no tokenizer change - plus a references block, one row per page with title, the URL as READABLE text (checking a paid finding must not need a mouse) and that source's OWN retrieval date. No sources means NO block; an empty title (a real shape - sourcesFromToolOutput defaults it to '') links its URL rather than rendering a blank anchor. A plain list, NOT a GroundedSources <details> fold: that fold is for a list accumulated across a whole thread. BRAND: --teal-900 link text and --ink-soft caps label, because §6 bars --teal-600 as small text on white, which is why capsTeal was NOT reused. D11c: the approvals gate showed markup in TWO places, not the one the plan named. previewText strips headings/bullets/emphasis then cuts at 320 (strip-BEFORE-slice: a cut lands anywhere including mid-** , and slicing first spends the budget on characters nobody sees). THE SECOND SITE WAS FOUND BY THE FIRST FIX: titleFor builds a memo's headline from body.split('\n')[0], which is that body's markdown H1, so the CARD TITLE carried the marker one element ABOVE the preview being fixed. TWELVE MUTATIONS, each reddening only its own test, AND ONE VACUOUS ASSERTION CAUGHT: the headline pin asserted toContain('Pricing findings</h3>'), which matches the DEFECT too because '># Pricing findings</h3>' contains it - it stayed GREEN with the strip removed. Tightened to '>Pricing findings</h3>'; the leading > IS the assertion. General rule now in dashboard-pages.md: when the defect ADDS A PREFIX, the pin must carry the delimiter. THE OTHER FINDING WORTH KEEPING (mutation B5): leaking sourceUrls into the subagent.completed payload reddens dispatch.test.ts's RUNTIME scan and leaves llmRedaction.test.ts FULLY GREEN - that static scan bans reply|body|text|output BY NAME and knows nothing about a URL. Do not treat the static pin as cover for this class. Measured: apps/web 453 passed across 29 files (was 443), backend blast radius (plans, evaluations, dispatch, llmRedaction, research, dispatchGuard, importGuard) 364 passed, the plan's own gate 165 passed, tsc clean in backend AND web, biome clean on all six changed source files, check-playbooks exit 0, graphify updated. TWO FILES IN THIS PLAN'S LIST CARRY FOREIGN UNCOMMITTED HUNKS (schema.ts's 50-line table index, cockpit.md's calendar.test.ts watch entry) and git stash is banned here - handled by snapshot / git checkout -- / re-apply only my block / stage / commit / restore, with git diff --cached inspected before each commit. Both foreign hunks are back in the tree, unstaged and byte-identical. evaluations.ts was normalised CRLF->LF on disk (its whole file read as a biome formatting error and hid real diagnostics); git normalises on add so the commit is unchanged. NOT SEEN IN A BROWSER: MemoCardBody, the references block and the stripped preview are renderToStaticMarkup-only - 25.1-07 is the owner checkpoint. ONLY NEW research turns carry sources; there is no backfill from the vault document, so an older memo renders no block. Research DEPTH stays out of scope. STILL RED, not mine and not touched: skills.activateAgentCandidate in isolation.test.ts (23-05, deferred item 1). Counters below carried forward +1 completed plan, NOT re-derived. Next: 25.1-06 (wave 6)."
last_updated: "2026-08-21T13:20:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 412
