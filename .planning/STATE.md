...
current_plan: 8
status: in_progress
stopped_at: "Completed 15.3-07-PLAN.md — the folder surface (VALT-05/06/08/10/11), built by a 14-agent workflow. Shipped: `folderEstimate` (a consume-nothing tenantQuery mirroring `reserveFolderInner`'s refusal ORDER), `preflightCopy.ts` as the ONE writer of the locked refusal sentence (naming both figures is the DEFAULT arm with a three-reason deny-list), folder picking via a second `webkitdirectory` input with per-file outcome accumulation, an INLINE pre-flight panel (NOT a modal — the app has no shared dialog pattern and PreviewModal has no focus trap), folder cards in the shared grid unit, a breadcrumb REPLACING the tabs inside a folder, subscription-driven sealed progress, and the 17.1 rebuild idiom. Every atom a user would be angry to lose (`currentFolderId`, `picked`, `phase`) lives in VaultPage because `nonce` is VaultBody's remount KEY. **AN ADVERSARIAL VERIFY PASS FOUND FOUR MAJORS AFTER THE SUITE WAS GREEN AND THE BUILD PASSED. THE UNIFYING CAUSE IS INFRASTRUCTURAL AND MATTERS BEYOND THIS PHASE: `apps/web` HAS NO UNIT-TEST RUNNER — no `test` script, no vitest dependency, no config at the app or repo root, and `pnpm test` is `turbo run test`, which skips a workspace declaring none. ANYTHING ASSERTED INSIDE apps/web IS ASSERTED BY NOBODY.** (1) `preflightCopy.test.ts` executed nowhere while reading as coverage — deleted, assertions moved into `packages/core/src/vaultSurface.test.ts` as its ONLY behavioural import; a source scan CANNOT prove \"names both numbers\" and stays green if the remaining-cents interpolation is dropped while the words \"left today\" remain. (2) Estimate/reserve parity — the plan's own <verification> line — was asserted by nothing; three tests added, parity checked against the WINDOW not just the return value. (3) The Start loop awaited `reserveFolder`/`cancelFolder` OUTSIDE any try/catch in a fire-and-forget `void start()`, so one dropped socket froze the panel on \"Sending 400 of 400\" forever with Cancel a no-op and 400 members parked at `pending_extraction` under a folder no sweep touches — the silent parking this phase abolished, re-introduced in the browser. (4) Per-file failures were accumulated then DISCARDED on partial success. All four fixed; FOUR mutations run RED. NOT fixed, deliberately: the folder Cancel pill is 0.72rem white on --teal-600 (3.67:1) but is a BYTE-IDENTICAL clone of two shipped pills (Retry, Discuss), so fixing one of three makes the surface inconsistent without making it accessible — it needs a surface-wide pass. pnpm test 8/8 backend 1157/1157, vaultSurface 21/21, vaultFolders 22/22, apps/web typecheck 0, backend non-test delta ZERO, web production build green, check-playbooks exit 0, $0 spent. Next is 15.3-08 (wave 8, document identity classification)."
last_updated: "2026-08-04T02:10:00.000Z"
progress:
  total_phases: 42
  completed_phases: 28
  total_plans: 247
