...
current_plan: 8
status: in_progress
stopped_at: "Phase 26 Plan 26-07 COMPLETE (2/2 tasks, commits df69693 + 05420f9 + deviation 84c83b9). **NOTHING IS LIVE-VERIFIED AND NO MONEY HAS MOVED THROUGH THE LEDGER** — this is offline parity evidence against the REAL rate-limiter component only. Every reasoning and ingest limiter movement now writes a matching spendEvents row IN THE SAME TRANSACTION via a PLAIN FUNCTION CALL (spendLedger.recordMovement), never ctx.runMutation: a second transaction could leave the window moved with no record, and an append-only table has no backfill. It writes the SAME `cents` the limiter consumed, so the planes cannot disagree about the amount. **THE CORRELATION POLICY IS SPLIT ON ONE QUESTION: can this site be re-entered WITHOUT new money changing hands?** Replayable sites (prepare -> `req:<id>:prepare` estimated; reserveFolderInner -> `f:<folderId>:<reservedAt>` reserved; settleFolder -> the SAME string rebuilt from folder.reservedAt, refunded) DERIVE. recordSpend MINTS A PER-EXECUTION NONCE, because all ~15 of its callers are ACTIONS where re-entry re-runs the model call and the second charge is REAL MONEY. **A TOO-COARSE CORRELATION IS WORSE THAN NONE** — it puts the ledger BELOW the limiter, and a missing movement is indistinguishable from money never spent, while a duplicate is findable by reconciling the two planes. A 26-agent design workflow attacked 19 proposed correlations and BROKE 16 — every single defect was an UNDER-count. That is what set the policy. **OWNER DECISION 2026-08-08:** correlationId stays OPTIONAL (plan scope, 6 files) rather than REQUIRED (~10 source files + ~6 playbooks, and media.ts overlaps 26-08); the nonce means no site is uninstrumented meanwhile. **reservedAt IS STAMPED ONCE** for both the returned value and the correlation — a second Date.now() orphans every refund while leaving the money perfectly correct, a defect nothing else notices. **A ROLLED WINDOW WRITES NO MOVEMENT:** a zero-cent row is rejected and that throw runs inside tryComplete, so folder completion would fail for an accounting reason; the reservation reads as `unlanded`, which is the literal truth (new reason `settled_window_rolled`). The DEPLOYMENT refund is deliberately NOT ledgered — spendEvents is per-tenant and the two clamps differ. **OWED BEFORE 26-09 READS THIS:** the other ~15 recordSpend sites carry no correlation, so no replay suppression; the 19-site design is in guardrails.md Known gaps, and **pipeline.ts:190 + vaultIngest.ts:194 MUST get `{ unstableArgs: true }` when they change or in-flight journaled workflows die on deploy** (option verified present in the pinned @convex-dev/workflow). Charges the pricer never sees are invisible to BOTH planes. folder.spentCents is permanently 0. **DEVIATION 84c83b9:** dashboardSchema.test.ts was red from the BLUEPRINT LANE's b74c7af, which re-wrapped `cancelKind` onto one line — same schema, old line-wrapping pinned. Added dense(): strips whitespace AND normalizes the trailing comma before `)`, because the formatter adds one when wrapping and drops it when a call fits. **RULE: a source scan pins the DECLARATION, not the print width.** **GATES:** backend 1255/1255, typecheck clean, check-playbooks exit 0; all four mutation checks caught by four different tests. **NEXT: 26-08** (media, wave 5, disjoint) — its webhook landing IS replayable, so it DERIVES from job/batch refs rather than minting a nonce. Call startCoverage only once all three rails write. **DO NOT RUN `gsd-tools state advance-plan`** — it deleted current_phase and pulled stopped_at from another lane's block on 26-06. Hand-edit this block.  **FOLLOW-UP SWEEP COMPLETE 2026-08-09, commit 5a69982 (owner-directed: implement the deferred design).** ALL TWELVE recordSpend call sites now pass a stable correlation — the gap this block listed as owed is CLOSED. correlationId stayed OPTIONAL (the earlier decision was preserved, not reversed); what replaces compiler enforcement is a STATIC SCAN in guardrails.test.ts that balances braces from each call's argument object and fails the build if a site ships without one, with a non-vacuity floor of 10 sites. Full per-site table with each discriminator: guardrails.md "Phase 26". The three that mattered: vaultExtract bills PER PAGE so `:p<i>` stops one page being recorded for a 50-page scan (image branch omits the segment, never fakes `:p0`); voice.ts meters REPEATEDLY per session so the cumulative token `offset` — not sessionId — is the discriminator; every llm.ts try/catch model pair is TWO fully-billed calls so `:a0`/`:a1` is required. BOTH journaled sites (pipeline.ts, vaultIngest.ts) carry `{ unstableArgs: true }` — CONFIRMED on the PUBLIC RunOptions type of @convex-dev/workflow@0.4.4, not an internal one. pipeline.ts does NOT interpolate the handler correlationId because smoke.seedPipeline declares it an unconstrained v.string(). **MY OWN GUARD SHIPPED WITH THE BUG IT EXISTS TO CATCH:** it balanced PARENS from the next `(` after the callee, but the call is ctx.runMutation(internal.guardrails.recordSpend, {...}) so that paren opens BEFORE the name — it returned a garbage span containing `correlationId` further down voice.ts and reported an UNINSTRUMENTED site as GREEN. Now balances BRACES from the argument object, mutation-checked. **THIRD source scan in two days to pass for a reason unrelated to its subject** (vault scan pinned a variable name; schema scan pinned a line wrap). RULE: a scan needs a non-vacuity assertion AND one run against known-bad input before its green is worth anything. **SHARED-TREE HAZARD OBSERVED LIVE:** the blueprint lane committed fabc51d while a sweep agent was editing blueprint.ts and discarded that agent's edit — nothing was lost only because the guard reported the site red. GATES: backend 1271/1271, typecheck clean, check-playbooks exit 0. **SHARED TREE:** the blueprint lane landed b74c7af, 8959be6 and a 69bfafa/97e8062 bump-then-revert of guardrails.md mid-plan. Never `git add -A`."
last_updated: "2026-08-08T20:20:00Z"
progress:
  total_phases: 45
  completed_phases: 30
  total_plans: 272
