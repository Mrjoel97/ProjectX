...
current_plan: 9 (done)
status: in_progress
stopped_at: "**PHASE 15.3 IS 9/9 — IMPLEMENTATION COMPLETE, ONE LIVE GATE OWED.** Completed 15.3-09-PLAN.md, the Google Drive rail (VALT-13), inline (no workflow), $0 spent — nothing in the plan makes a model call. THE ORDER IS THE DESIGN AND IT IS NOT THE UPLOAD RAIL S ORDER: scope check -> reauth BEFORE freshAccessToken and before any network call; enumerate METADATA ONLY; RESERVE from that metadata; only then download. files.list returns count/size/type before a byte is exported, so a refused Drive folder is refused without downloading a gigabyte and refuse-intact is proven by ABSENCE rather than asserted. TWO LOCKED DECISIONS CORRECTED ON FACTS (CONTEXT A6/A7): the per-file byte cap DOES apply (non-native files download into a ~512 MB action; a 600 MB video OOMs it before any cap could refuse), and Drive reports NO size for Google-native docs (estimatedBytesFor NEVER returns 0 — reading a missing size as 0 prices a 500-Doc folder at $0, reserves nothing and strands it mid-run; that one test is the whole reservation invariant on this rail). **BOTH SHARED-DRIVE PARAMS RIDE EVERY CALL AND ARE PINNED BY A SOURCE SCAN, NOT A BEHAVIOURAL TEST** — omit either and Drive answers HTTP 200 with an EMPTY files array, which the product would report as imported 0 files, folder complete; no stub can catch what a stub is free to return. Re-import is keyed driveFileId + modifiedTime and NEVER contentHash (OOXML is a zip, so two exports of an unchanged Sheet are not byte-identical and a hash key duplicates the whole folder every refresh); an unchanged listing issues ZERO export fetches and creates ZERO rows. A dedup hit gets IDENTITY BUT NOT MEMBERSHIP — driveFileId yes, folderId never, because annexing a pre-existing document into an ingesting folder would SEAL one the user could ground on yesterday. **THE FAN-IN MUST COUNT FAILURES**: driveLandedCount counts inserted/deduped/updated/failed-to-export, and counting only insertions hangs the folder in reserving forever the first time one export 404s, holding a reservation nothing settles (there is no folder-level watchdog). vaultFolders.tryComplete gained export for that reuse; vaultDrive.ts is DELIBERATELY NOT use node because its fan-in is a mutation, and adding it later would silently break them. DEVIATIONS, all in vault.md: enumeration is BOUNDED below the 10-minute action limit by DRIVE_MAX_FILES/DRIVE_MAX_FOLDERS instead of a self-scheduling continuation; DRIVE_ID_RE guards the folder id at the trust boundary because it is interpolated into Drive s q= QUERY LANGUAGE, which percent-encoding does not protect; three optional vaultFolders schema fields were added (zero tables/indexes/migration/backfill). **SIX MUTATIONS RUN, AND ONE OF THEM CAUGHT MY OWN SCAN BEING VACUOUS** — the audit name-leak check tested for name: and the shorthand name, walked straight through it; fixed to a property-position regex, then re-run RED. Full suite 9/9 packages (backend 1177/1177 across 62 files, core 603/603, vault 170/170); backend typecheck back at the EXACT 15-error baseline, all pre-existing convex/*.test.ts, zero non-test and zero from these files; apps/web typecheck 0; web build green; check-playbooks exit 0. Commits cf675c7 (the rail) and 3ddbdad (a pre-existing uncommitted banner fix, committed separately first at the owner s direction). **THE ONE THING OWED IS A LIVE RUN and it is the ONLY thing that can settle it**: every Drive response in the suite is a stub, so what is proven is that we SEND both shared-drive params, never that Google honours them for a real shared drive. The phase checkbox and VALT-13 stay UNTICKED until one real shared-drive import returns a non-zero file count (the Phase-17 human_needed convention, and the 17.1-01 early-flip trap)."
last_updated: "2026-08-04T22:40:00.000Z"
progress:
  total_phases: 42
  completed_phases: 28
  total_plans: 247
