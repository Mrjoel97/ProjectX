...
current_plan: 10
status: in_progress
stopped_at: "Phase 26 Plan 26-10 Task 1 COMPLETE (commit 91462d2) — BLOCKED AT TASK 2, the blocking owner UAT checkpoint. The connected Cost route ships at `/dashboard/finance` with its nav item STILL `Soon` (activation is Task 3, after approval). **THE BROWSER GATE IS NOT GREEN AND MUST NOT BE CITED AS PASSED:** `e2e/finance.spec.ts` was authored and EXECUTED, and stopped in the canonical `auth.setup.ts` because this shell has no `E2E_USER_EMAIL`/`E2E_USER_PASSWORD` — 27 assertions did not run. Same wall plan 26-05 hit on approvals. Resume with local `convex dev` (NOT `--once`) + Next on :3111 + both credentials exported: `pnpm --filter @pikar/web test:e2e -- e2e/finance.spec.ts`. Green now: web 88/88 (29 new Cost tests), web typecheck, production build with `/dashboard/finance` in the route table, check-playbooks exit 0. **THREE PAGE RULES, each mutation-checked:** an `unknown` window renders NO currency mark at all (a zero there is indistinguishable from a watched-and-empty period); media's `unlanded` gets DIFFERENT copy (`unlandedResolves.media === false`, permanent, never 'pending'); `bound.partial` says the totals are a FLOOR. A non-owner's DOM carries no ceiling value and no switch position, and the owner queries are passed `\"skip\"` rather than fired-and-caught — OWNER_REQUIRED would drop the whole page into the error boundary for someone who is simply not the owner. `requiresConfirmation` is honoured as a BACKEND fact with an in-component arm/commit step, never `window.confirm` (a browser modal blocks the page the spec must drive). **ALMOST NONE OF THE MOCKUP'S CLASSES EXIST IN globals.css** — only `stat-grid/stat-tile/stat-head/stat-badge/stat-value/caps-label`; `.card`/`.meter`/`.pill`/`.btn`/`.sec`/`.bars` are mockup-only and `.ledger` IS defined but is the DARK marketing audit block from the landing page. The meter is a native `<progress>` with the percentage also in words; the mockup's amber `.meter.warn` is deliberately unused because BRAND §2 keeps amber for the approval gate. Component tests are `.test.ts` NOT `.test.tsx` (vitest includes `app/**/*.test.ts` only — a `.tsx` is SILENTLY SKIPPED); `apps/web/vitest.config.mts` now sets `esbuild: { jsx: \"automatic\" }` to match Next, which removes the dead default `React` import every component otherwise needed. **NEXT: Task 2 owner UAT (blocking), then Task 3 nav activation.** Task 3 is a one-line diff — the layout branch keys off `item.href`, not `soon`."
