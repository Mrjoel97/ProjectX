...
current_plan: 6 (of 7 — 19.1-06 complete)
status: in_progress
stopped_at: "19.1-06 COMPLETE — THE CSV IMPORT PANEL EXISTS AND IS REACHABLE (`4b78f5f`, 4 files, ONE commit, $0.00). **UNTIL THIS COMMIT `matchExisting` AND `importContacts` HAD ZERO CALLERS** — five plans of tested, isolated, mutation-proven capability that no user could reach, which is the phase-19 clock-plane failure rebuilt on purpose. **WHAT SHIPPED:** `apps/web/app/(app)/dashboard/pipeline/ImportPanel.tsx` (599 lines, NEW) — one file, one `step` union (`choose` | `preview` | `done`; never `stage`, which the PIPE-01 structural scan bans and now scans this file for too). Choose: a hidden file input + ref + a visible button that also accepts a drop, a `resolveMimeType` sanity check, `await file.text()`, and the input value cleared in a `finally` so re-picking the same filename re-fires `change`. Preview: a select per field over the header cells so EVERY column is overridable, the four counts `N new · M enriched · K unchanged · R rejected`, and every rejected row by its PHYSICAL FILE LINE inside a `scroller`. Done: the totals and a line saying the table below is current. **THE FILE NEVER LEAVES THE BROWSER** — `file.text()` not `arrayBuffer()`+`TextDecoder` (Dropzone only uses the two-step form because it HASHES, and nothing here is hashed), only mapped rows cross the wire, and a test scans the panel source for `generateUploadUrl`/`vaultUpload`: if either ever appears, the 'no PII at rest so no retention rule' claim is false and that question re-opens. **THE COUNTS ARE A PROJECTION, NOT A GUESS:** `matchExisting` is a ONE-SHOT `useConvex().query` chunked at `IMPORT_MATCH_CHUNK` (the boundary REFUSES a longer array rather than slicing, so the chunking is not optional), and `enriched` means the row carries a value for a field the match reports EMPTY — the same four-field definition the write uses, so preview and result cannot disagree. Confirm loops `IMPORT_BATCH_ROWS` slices with honest `Importing batch 3 of 7` progress; a failed batch stops, reports what LANDED, and says re-running the same file is safe (true by construction: upsert-by-address + fill-empty-only). **THE ATTESTATION CHECKBOX STARTS UNTICKED AND CONFIRM CARRIES `disabled` UNTIL IT IS TICKED**, with `IMPORT_ATTESTATION` rendered from the constant and never re-typed — a pre-ticked box would make the byte-for-byte stored wording a false statement about what the user did. **EVERY REFUSAL IS INLINE AND GREY** through `PipelineStateNotice` (role=status, `--ink-soft`): no `window.alert`, no `window.confirm`, no dialog element, never amber (BRAND §2). Over 1 000 rows is refused AT THE PICKER naming the limit; a missing email column is refused IN the preview, beside the selects that fix it. **`ConnectedImport` IS THE FOURTH SECTION, MOUNTED BETWEEN THE TILES AND THE CONTACT TABLE** — not between contacts and unassigned, because the e2e document-order assertion pins that `pipeline-unassigned` FOLLOWS `pipeline-contacts`. Ten style objects are now EXPORTED from `PipelineView.tsx` instead of re-declared, and the resulting `PipelineView` <-> `ImportPanel` module cycle is safe ONLY because every imported binding is read inside a component body — DO NOT hoist one into a top-level const. **THREE DEVIATIONS, ALL RULE 1/3.** (1) The plan said to update `ContactsEmptyState`'s 'EXACTLY ONE action' DOC COMMENT and never mentioned that the same claim is an ASSERTION (a button-count of 1) — it went red with `expected 2 to be 1`, which is itself proof the second button reaches the rendered DOM. Rewritten to TWO, keeping every no-mailbox-suggestion assertion: typing one person and importing under an attestation are BOTH deliberate human acts, and invariant 1 forbids a row appearing because software went LOOKING. (2) A strict `resolveMimeType(...) !== text/csv` refusal would bounce valid files — Chrome on Windows reports `application/vnd.ms-excel` for `.csv` when Excel owns the extension — so the check is `.csv` extension OR `text/csv`. (3) The row cap is checked at the picker with the shared constant, because `mapRows` checks the email column FIRST and a 5 000-row file with an odd header would otherwise send the user to fix a column only to be refused for size. **TESTS +5 AND ONE REWRITTEN:** Confirm disabled at ticked=false AND the attribute absent ENTIRELY at ticked=true (its own non-vacuity floor); the attestation compared against the IMPORTED constant, never a re-typed copy; the four counts including a real `0`; every rejected row's LINE and reason; and a source scan covering no-upload, no-dialog, no-amber, no-`stage`, plus the three KEY LINKS (the `@pikar/core/contactImport` SUBPATH import and both `api.contacts.*` names) — because a tested function with no caller is invisible to every green suite in this repo. **TWO MUTATION-PROOFS, RED AND REVERTED:** dropping `!ticked ||` from Confirm gives a failure to match the import-confirm-plus-disabled regex; dropping the line number from a rejected row gives `... to contain 'Line 4'`. MEASURED: `pipelineView.test.ts` **27/27** (was 22), `@pikar/web` **176/176** across 10 files (was 171), `pnpm typecheck` **10/10** delta 0, **`npx next build` GREEN** with `/dashboard/pipeline` compiled (the style cycle resolves in the REAL client bundler, not only in vitest), `check-playbooks` **exit 0** (the pipeline directory is already a watched prefix — verified by RUNNING the hook), biome clean. **NOT DONE AND NOT CLAIMED: NO BROWSER HAS EVER DRIVEN THIS PANEL WITH A REAL CSV.** 19.1-07 owns the fixture, the e2e and the owner judgement; until it runs, 'a user can import contacts' rests on a compiled bundle and a DOM-free renderer — precisely the evidence phase 19 had when it was wrong. **SHARED WORKING TREE:** nothing was staged by another lane at commit time; the code commit used an explicit four-path `git add` and the ROADMAP was staged HUNK-ONLY via a `git hash-object -w` blob built from `HEAD` + this plan's edits. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it)."
last_updated: "2026-08-10T21:45:00.000Z"
progress:
  total_phases: 52
  completed_phases: 31
  total_plans: 365
