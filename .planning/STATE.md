...
current_plan: 7
status: in_progress
stopped_at: "Phase 26 Plan 26-06 COMPLETE (2/2 tasks, commits 9c9555b + bda09e0). **NOTHING IN 26-06 IS LIVE-VERIFIED AND THAT IS BY DESIGN — NO RAIL WRITES TO THIS LEDGER YET AND NO MONEY HAS MOVED THROUGH IT.** Built the FIN-01 accounting seam: `packages/core/src/spend.ts` (closed rail/phase vocabulary, positive-safe-integer cents, ref-safe token validation, `aggregateSpend`) and `packages/backend/convex/spendLedger.ts` (`record`, `startCoverage`, `coverage`, capped half-open `listEvents`). **THE REPLAY IDENTITY IS (tenantId, correlationId, phase), NOT correlationId ALONE** — a reservation and its later actual charge deliberately SHARE one correlation because that is what makes them reconcilable, so a correlation-only guard swallows the actual as a duplicate of the reserve and LOSES THE MONEY; `by_correlation` is not tenant-scoped, so the tenant comparison is part of the identity, not an assumption. Dropping either component fails a DIFFERENT test — both mutations were run. **FIRST WRITE WINS:** a replay returns the stored id and IGNORES the replayed amount, so drift is corrected by appending an `adjustment` (owe more) or `refunded` (money back), never by a rewrite; direction lives in the PHASE, never in the sign. Validation runs BEFORE `ensureCoverage` so a rejected movement leaves nothing behind — a coverage row it did not earn would flip a tenant from honest Unknown to a false 'we were watching, and it was zero'. **PRE-COVERAGE IS UNKNOWN, NEVER $0**, and the gap can never be backfilled. **NON-NEGOTIABLE ROLLBACK RULE, now in dashboard-pages.md: Finance may hide its route and owner controls; it may NOT stop ledger instrumentation.** The limiter stays ENFORCEMENT truth; this table is REPORTING/RECONCILIATION truth, and their disagreement (unlanded money) is the reconciliation signal, not a defect to hide. **OUT-OF-LANE DEVIATION, commit 8dd911b:** `packages/core/src/vaultSurface.test.ts` was RED ON THE BRANCH BEFORE THIS PLAN (638/640) — the 15.4 redesign renamed the download anchor `a`→`anchor` and split the stale-digest affordance across two files, so two source-scan assertions pointed at deleted spellings while the PRODUCT GUARANTEES STAYED INTACT. It went unseen because the 15.4 gates ran `@pikar/web` only, which never executes a scan living in `@pikar/core` — RULE: when a UI change moves a guarded spelling, gate it with the package that OWNS the scan. Repaired, mutation-checked, core now 640/640. **GATES:** backend 1242/1242, core 640/640, both typechecks clean (the former 15-error backend baseline is now ZERO), check-playbooks exit 0. **NEXT: 26-07 + 26-08 (wave 5, parallel)** instrument the reasoning/ingest and media rails against this writer — both MUST mint a STABLE correlationId per logical movement (a random per-attempt id silently defeats replay suppression) and must open coverage before the rail's first paid call. **WARNING — `gsd-tools state advance-plan` CORRUPTED THIS BLOCK on 2026-08-08:** it deleted `current_phase`, pulled `stopped_at` from a LATER lane's frontmatter block (15.4-03) and rewrote the counts to 51/333/247. Hand-restored. Verify this block after every state command. **SHARED TREE:** foreign lane work landed mid-session (commit edac4aa blueprint anatomy; uncommitted BlueprintPanel.tsx, SegmentAnatomy.tsx, segmentCopy.ts). Never `git add -A` here."
last_updated: "2026-08-08T14:45:00Z"
progress:
  total_phases: 45
  completed_phases: 30
  total_plans: 272
