...
current_phase: 25.1
current_plan: 1 of 7 (phase 25.1 wave 1 complete)
status: executing
stopped_at: "Completed 25.1-01-PLAN.md 2026-08-21 (D1+D2, TDD, 4 commits a101f60/0aad1da/717569b/a02274b, $0). EVERY renderReel exit path now writes an honest terminal: all batchToRender refusal classes end failed+reason+dead letter through recordRender's failure arm (renderReel takes planId now — empty_batch cannot name its plan from rows); the three schedule sites run renderReel under the ActionRetrier with onComplete internal.mediaComplete.onRenderComplete, correlated via NEW plans.renderRunId + by_render_run (the calendarRunId/mediaRunId pattern a third time — stale runs miss because every new schedule overwrites the id); the non-JSON 200 at the route response terminalizes inline as route_bad_response (inline deliberately: the sandbox already ran, a retrier retry buys a second one). THREE DEVIATIONS, all recorded in 25.1-01-SUMMARY.md: schema.ts gained the column+index outside the plan's file list (staged SURGICALLY — the foreign lane's uncommitted 50-line comment block stayed unstaged, same for cockpit.md's foreign entry); llmRedaction pins moved deliberately (payload literals 11->12, dead-letter ban became per-module counts renderReel:2/mediaComplete:1); cockpit.ts NOT touched — the plan's premise 'EXTERNAL_TARGETS gains renderReel' is FALSE, EXTERNAL_TARGETS.media stays submitBatch per 20-16, recorded in cockpit.md. TEST GOTCHA worth keeping: a convex-test file in a SUBDIRECTORY needs its vite glob keys remapped (siblings become './x.ts' while the shared prefix is derived from the _generated key) — renderReel.test.ts carries the remap. media.test.ts now observes render scheduling as plans.renderRunId, never _scheduled_functions name matching (component schedules are invisible to the parent). Measured: renderReel.test.ts 22/22, media 237/237, llmRedaction 61/61, cockpit 73/73, backend tsc clean, check-playbooks exit 0. Counters below carried forward +1 completed plan, NOT re-derived. Next: 25.1-02 (wave 2, stuck-work watchdog cron D3/D4 — it can now trust that 'rendering' means in-flight-or-crashed-before-onComplete)."
