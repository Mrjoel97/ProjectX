...
status: complete
stopped_at: "40 CLOSED 2026-09-06 — Document Canvas (G5, DOC-01): vaultSheets capped grid written at ingest (separate table BY DESIGN — the vault read bound counts vaultDocuments.text only), SheetGrid in the vault preview with its ceilings in words, .xlsx on BOTH planes through markdownToSheets + sheetsToXlsx and the new ungated spreadsheet-drafter@1 (stored bytes read back as a workbook in test), regenerateAttachment keeps its format, inline PDF in OutputCard under the restated bearer-URL rule, ADR-036 accepted. cockpit-agent body v3 is seeded as CANDIDATE v28 and NOT activated: two full gate runs both scored 45/46 on `33-research-insufficient-evidence`, which passed on v28 ALONE first-attempt and needed a retry on the active v26 — stochastic research-lane behaviour (the specialist calls declareUnsupported for a sub-question then answers as if supported; the verdict reads the ANSWER's boolean, not the tool call), proven unrelated to this change. EVAL_GATE refused activation, correctly. THE OPEN ITEM belongs to the research lane, not to Phase 40. PRIOR: 39-01 CLOSED 2026-09-06 — the research specialist reads the pages it cites: readPage (Tavily /extract, bounded to URLs the run's own search returned, 6 reads × 6k chars), page-read/snippet-only labels, RESEARCH_STALE_AFTER_MS shared by the reuse window and the card stamp, LIMITS_FOOTER rewritten, skill v4 = research-specialist@10 through the eval gate 46/46 ($0.80) and active on the local deployment (readPage ×20 over 7 runs). Packs deliberately NOT granted readPage until their bodies are revised through the pack gate (pinned by test). Do NOT run gsd-tools state * against this file."
