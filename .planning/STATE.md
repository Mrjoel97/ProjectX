...
current_phase: 25.1
current_plan: 4 of 7 (phase 25.1 wave 4 complete)
status: executing
stopped_at: "Completed 25.1-04-PLAN.md 2026-08-21 (D9, D10, TDD, 2 commits 405117b/3905c62, $0). THE APPROVALS GATE NOW ENDS EVERY CLICK IN SOMETHING A HUMAN CAN READ. D9a: the success message was unreadable BY CONSTRUCTION - a successful approve IS the proposed->approved transition and listAwaiting paginates proposed only, so the card that called setResult was unmounted by the very mutation that produced the message and the user saw a row vanish with no outcome, indistinguishable from a broken button. dashboard-pages.md NAMED this exact rule back in 19-12 (\"any state produced BY a status transition must not live in a component gated ON that status\") and then LEFT THE DEFECT STANDING; this plan obeys it. The outcome map moved to AwaitingSection as Record<planId,string|null> and persistentOutcomes(outcomes,liveIds) returns the entries whose row has LEFT the list, each rendered as a ResolvedOutcomeCard (green --released stripe, never amber, --ink-soft label text because --released is ~3.3:1 on paper). A row still LIVE is filtered out so its own card carries the message rather than a second one. DISCARD AND SCHEDULE RIDE THE SAME CHANNEL - both drop the row for the same reason and were equally mute; the shared fix is smaller than three later ones. D9b: the notice is now the FIRST child of the card - it used to render below the discard fieldset, under a collapsed confirm box, at the bottom of a scrolling list. D9c: executePlan returns alreadyStarted for ANY non-proposed status, discarded and canceled INCLUDED, because its CAS only knows it did not win - so \"This plan already started\" asserted a start that may never have happened; STALE_PLAN_MESSAGE now says only what that return can promise. D10: an image plan renders NO Approve button - executePlan's media arm reserves against a SHOT DECK and a standalone image has none, so that button returned no_deck on every click it has ever received; IMAGE_CANVAS_NOTE points at the workspace canvas where api.media.generateImage already works. approvals.ts UNTOUCHED (the card still lists, only the mute button goes) and the reel path keys off the same mediaMode discriminator planKind uses. DELIBERATELY NOT DONE: cockpit.ts / cockpit.test.ts / cockpit.md, all three in the plan's file list - the plan allowed copy-only and copy-only is what STALE_PLAN_MESSAGE needs; widening a money-path mutation's return contract to improve honest copy is surface for nothing, and it kept the commit clear of cockpit.md's foreign uncommitted hunk. STRUCTURAL: AwaitingCard is now a connected shell over an EXPORTED presentational AwaitingCardBody - apps/web has NO jsdom by deliberate config decision, but renderToStaticMarkup needs none (the blocker was the hooks, not the environment), so six of eight new assertions run against REAL MARKUP instead of a regex over a 130-line JSX blob, which is the shape every vacuous test this phase has found took. EIGHT MUTATIONS, each reddening only its own test, and NO VACUOUS TEST THIS ROUND - the first wave of 25.1 where nothing had to be rewritten, structurally rather than luckily: on rendered markup \"the logic was deleted\" and \"the element is absent\" are the same observation. Measured: approvals suite 34 passed (26 pre-existing + 8 new), full apps/web 443 passed across 28 files (was 435), apps/web tsc clean, biome clean on both changed files, check-playbooks exit 0. Backend untouched - no convex file, no codegen, api.d.ts unmodified. e2e/approvals.spec.ts:311 deliberately NOT edited: its media fixture has no mediaMode so planKind returns reel, keeps its Approve button and still reaches no_deck. NOT SEEN IN A BROWSER: ResolvedOutcomeCard, the top-of-card notice position and the image copy are renderToStaticMarkup-only - 25.1-07 is the owner checkpoint that should look. The persistent outcome is per-mount (AwaitingSection useState), cleared by a reload; the durable record stays the Scheduled/In-flight/Cleared lanes. STILL RED, not mine and not touched: skills.activateAgentCandidate in isolation.test.ts (23-05, deferred item 1) was already red when this plan started. NO git stash - staged by explicit path, three files, verified with git diff --cached --name-only. Counters below carried forward +1 completed plan, NOT re-derived. Next: 25.1-05 (wave 5)."
