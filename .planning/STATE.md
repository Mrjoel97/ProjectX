...
current_plan: 10
status: in_progress
stopped_at: "Phase 26 Plan 26-09 COMPLETE (2/2 tasks, commit e983d45) — the Cost Console BACKEND. NOTHING IS LIVE-VERIFIED; no money has moved through any rail and these are offline projections over the real rate-limiter component plus directly-inserted ledger rows. **TWO THINGS WOULD HAVE SHIPPED A LIE AND BOTH ARE NOW REGRESSION-TESTED.** (1) `guardrails.remainingDailyCents` and its media/ingest siblings return `min(tenant, deployment)` — correct for sizing a sub-agent envelope, an INFORMATION DISCLOSURE on a tenant page, because that minimum tells every tenant how drained the KEYLESS shared ceiling is. `finance.summary` reads the PERSONAL window only; the three ceilings are `finance.globalRails`, an ownerQuery. The test drains all three deployment windows, leaves the tenant's barely touched, and asserts no deployment constant appears anywhere in the serialized tenant payload. (2) **`getValue` RETURNS THE LAST WRITE, NOT THE PRESENT** — its own `calculateRateLimit` call passes `now = state.ts`, so a window that rolled overnight with nothing written since reports YESTERDAY's drained value. A tenant with a full allowance would have been shown `$0 left` with nothing to distinguish it from real exhaustion. `railView` rolls forward with the component's OWN exported `calculateRateLimit` — the same fix and the same reasoning as `guardrails.refundableCents`, which learned it by refunding 2900 against a capacity of 2500. **THE REPORTED WINDOW REACHES `aggregateSpend` UNCLAMPED, DELIBERATELY:** clamping it up to `coverageStartedAt` (the obvious `resolveDashboardWindow` coverage argument) turns an unknown stretch into a silently SHORTER window with a confident, wrong total — `unknown` has to be able to reach the page. Each `spendSeries` bucket aggregates on its own for the same reason. **`resetsAtMs` IS NOT MIDNIGHT ANYWHERE** and the mockup's "resets 00:00 UTC" is only true for a rail nothing has ever spent from: a fixed window with no `start` is anchored to its FIRST spend, so each rail resets on its own offset. It is returned as an epoch instant labelled `resetTimeZone: \"UTC\"` — the ENFORCEMENT clock, kept separate from `window.timeZone`, the browser display timezone that never reaches a query. Reads go through NEW plain-function halves in `spendLedger.ts` (`coverageFor`/`listEventsFor`, the ONE Rule-2 deviation: a tenantQuery cannot `runQuery`, and re-querying in `finance.ts` would put a second copy of the 500-row cap in the module the playbook designates a READER). `mediaLedger` uses Convex NATIVE cursor pagination because every line of a batch shares one `createdAt`. Owner controls are ownerQuery/ownerMutation (rejected BEFORE the handler reads or writes), one upsert writes every config field, one audit row per ACCEPTED transition and none for a no-op, payload keys exactly control/from/to, and every mutation returns the RE-READ effective state rather than echoing its argument. Gates: finance 15/15, finance+owner+spendLedger+guardrails 77/77, backend 1304/1304, typecheck clean, biome clean, check-playbooks exit 0. Five mutation checks run, all five caught, each by a different test. **NEXT: 26-10** — the connected route; it must never render an `unknown` window as `$0`, must give media's `unlanded` different copy (`unlandedResolves.media === false`, PERMANENT over-reservation not 'pending'), must render `bound.partial` (a capped window's totals are a FLOOR), must keep the two timezone labels apart, and must honour `requiresConfirmation` on all three controls."
