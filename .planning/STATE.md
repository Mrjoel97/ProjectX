...
milestone_name: - Platform -> Private Beta
current_phase: 27
current_plan: 8 of 9 executed (27-01..27-08) -- 27-08 is PARTIAL; next is 27-09
status: executing
stopped_at: "27-08 IS PARTIAL, AND THE MISSING HALF IS BLOCKED ON MONEY, NOT ON CODE. **THE SIX PACKS NOW EXIST ON THE DEV DEPLOYMENT, ALL AT v1, ALL `candidate`, ALL DARK** -- `provenanceValid: true`, `evidenceValid: false`, `browserValid: false`, and re-running the publisher inserts nothing. Dev skillIds and body hashes are in 27-08-SUMMARY.md. **DOLLARS SPENT: $0.00.** **THE SIX PAID PER-PACK EVAL RUNS DID NOT HAPPEN: the dev OpenAI key returns `insufficient_quota` / `credit_balance_exhausted`.** The harness is built, self-tested and committed; it needs a topped-up balance and a verified-stable `convex dev`, one invocation per pack. **THE FINDING THAT SHAPED THE PLAN: 24 of the 30 wave-2 fixtures asserted something the shipped runtime cannot produce.** 21 named a `vault`/`web` state `probeSources` never returns (both are hardcoded `available` -- an empty vault is the tool's own honest answer, not a preflight fact; also `calendar` is never `unavailable` and `finance-inputs` is never `partial`), and 3 expected an outcome `outcomeFor` never derives -- **NOTHING ANYWHERE EMITS `refused`**. Owner ruled FIX THE CORPUS: `workflowPackBinding.ts` is byte-unchanged, 36 fixture edits landed, and `PACK_SOURCE_PROBE_STATES` + two validator rules + a test that SCANS `probeSources` own return block keep them reconciled. **DECISIONS 27-09 MUST CARRY.** (1) The pack gate's eval plane is now `hasPassingPackEvalEvidence`, NOT `hasPassingEvidence` -- it also requires `runner: \"eval:pack\"`, `PACK_EVAL_SUITE.revision`, the pack's exact fixture-file hash/count, and full coverage; a global-scope blob carries no suite identity, so an `eval:golden` row would otherwise have gated an activation silently. (2) NO WALL CLOCK may enter pack provenance -- `(body, provenance)` IS the version identity, so a `Date.now()` mints candidate N+1 forever; `ts` is the pinned upstream commit's. (3) The version is resolved BEFORE provenance is built, or the duplicate check and the pin check deadlock against each other permanently. (4) The adapted body hash is over LF-NORMALIZED bytes (`* text=auto` makes raw bytes machine-dependent, and LF is what ships). (5) `drive` is honestly `unavailable` for EVERY eval case -- there is no Drive fixture seam, so `findInDrive`/`listDriveFolders` have NO eval coverage; stated, not papered over. **KNOWN GAPS:** `citationCoverage`/`unsupportedClaimRate` still report `not_applicable: no_data` in the production plane (the runner grades them for the eval corpus only; `workflowPackEvents` is insert-only and the binding owns its rows); `vaultSmoke:seedCorpus` is NOT free (it embeds through OpenAI) and gets one retry. A provider/quota/network failure or a failed tenant seed now aborts with exit 2 rather than reporting six broken bodies -- the mis-signal this repo has already paid to learn. EVIDENCE: backend 99 files/2457, core 43/1176, contracts 6/93, three typechecks clean, biome ci clean over the touched files, `--check` green with three mutations observed RED, `--self-test` 35 rejections observed RED by disabling one scorer check, the cross-registry parity test and the `probeSources` scan each observed RED. **NEXT IS 27-09** -- the browser evidence plane and the discovery surface. It cannot activate anything until the six eval runs happen. Working branch feat/27-02-pack-contracts."
...
milestone_name: - Platform -> Private Beta
current_phase: 27
current_plan: 7 of 9 executed (27-01..27-07) -- WAVE 3 COMPLETE; next is 27-08
status: executing
stopped_at: "27-07 IS COMPLETE. **THE SIX PACKS ARE NOW EXECUTABLE AND STILL DARK** -- nothing is deployed, no pack row exists in any deployment, and 27-08 publishes the candidates. The one executable native path exists end to end: selected pack -> the EXISTING `runSpecialistTurn` under a code-owned tool allow-list -> governed output -> measured outcome, inside `traced()`. **`llm.ts` IS BYTE-UNCHANGED**, as the plan's scope correction asked -- artifacts are observed through `internal.vaultSources.latestCreated` instead of by editing `createDocument`. NEW: `convex/workflowPackBinding.ts` (\"use node\"; `runWorkflowPack` + the offline twin `__runWorkflowPackWithScript`, the dispatch.ts precedent) and `convex/workflowPackOutcomes.ts` (a bounded tenant report applying 27-03's eight pure metrics). `cockpit.startWorkflowPack` is `sendCockpitMessage`'s shape with ONE substitution, sharing the extracted `ensureThreadAndPlan` so both reach the SAME plans row. **DECISIONS THE NEXT PLANS MUST CARRY.** (1) `runId` IS the correlation id -- it rides as the loop's `turnId`, so `spendEvents` and `agentSteps` both carry it and the projection JOINS rather than re-emitting; BUT `runAgentLoop` charges under `agentloop:<runId>:a<attempt>` (plus a `:search` sibling), NOT the bare id, so `ledgerCorrelationIds` rebuilds those four and a journey test pins the projection's total to the ledger's own rows. (2) Plan decisions are attributed to the run that STAGED the plan (a `plan_proposed` pack event on that row), never to the thread -- a pack run and an executive turn share the thread's single plans row, and `customer-complaint` is the only pack granted `proposePlan`. The approve emission sits AFTER the CAS and after every governed stop, or `gmail_not_connected`/`no_postal_address`/`all_recipients_suppressed` would each count as an approval. (3) A pack run takes the same `guardrails.preCall` gate as every other paid model path. (4) `useful` is the NARROWEST outcome arm because it is what `timeToFirstUsefulOutcome` counts; most pilot runs are `partial` by construction. **TWO TEST-DESIGN TRAPS WORTH NOT REPEATING, both caught only by trying to make the test fail.** (a) `ai@7` does NOT throw `NoSuchToolError` out of the loop -- it turns the call into a `tool-error` part and carries on, so a probe that read that message never failed. The grant is now proven by reading `agentSteps` (the SDK never fires `onToolExecutionStart` for an absent tool), with the same script run first through the EXECUTIVE record as a control so every negative is falsifiable. (b) `stopWhen: stepCountIs(8)` truncated the 19-call probe after the seventh, so every later tool read as ABSENT -- a green 'exactly its grant' over a run that stopped a third of the way through. It is CHUNKED at six calls sharing one turnId. **TWO OF MY OWN TESTS WERE VACUOUS AND WERE FIXED** (a cost-isolation test seeded the bare run id the join never looks up; an attribution test never reached the CAS because it seeded no gmailTokens/postal address). Mutations observed RED and restored: delete `preCall`; drop the tenantId filter in `costForRun`; attribute on the newest event instead of `plan_proposed`; drop the terminal check from `live`. **DEVIATIONS:** one new index `workflowPackEvents.by_tenant_plan` (schema.ts) so the plan-decision join is not a 500-row scan on a hot mutation path; one new export `onboarding.onboardingCompletedAt` returning the OLDEST profile doc (the newest moves forward with every profile edit and would report real outcomes as `useful_precedes_onboarding`); `skillVersions` threaded onto the binding because 27-08's eval runner MUST pin the exact candidate or it certifies the ACTIVE body. **KNOWN GAPS, STATED NOT PAPERED OVER:** `citationCoverage`/`unsupportedClaimRate` report `not_applicable: no_data` because nothing writes the claim counts -- 27-08's eval runner is the only plane that grades a body, and they light up unchanged once it does; `recommendation_shown` has no caller until 27-09's discovery surface; `artifact_created` is proven offline only in the NEGATIVE direction (the positive needs a real `createDocument`, which calls a model) -- DO NOT read the green suite as covering it. EVIDENCE: binding 31/31, outcomes 9/9, eleven touched-module suites 500/500, core 43 files/1174, FULL backend 99 files/2446 green on two consecutive runs, three typechecks clean, biome ci clean over the eight touched files. Two EARLIER full-suite runs each hit ONE intermittent failure in `media.test.ts` ('a transcript with no usable words never buys a sandbox', its fetch spy saw 1 call) -- it passes alone (227), passes with both new files (267), the suite minus the binding test passes (98 files/2415), and it did NOT reproduce when instrumented to print the leaked URL. Ruled out at the source: foglamp arms its flush timer only when enabled, and FOGLAMP_API_KEY is unset. Load-dependent flake in a file this plan does not touch; named, not fixed. **NEXT IS 27-08** -- final provenance/parity, six candidate publications and exact-version eval evidence on DEV. It is the FIRST plan in this phase that costs money and it needs a live stack. Working branch feat/27-02-pack-contracts."
last_updated: "2026-08-23T19:20:00.000Z"
progress:
  total_phases: 53
  completed_phases: 34
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 27
current_plan: 6 of 9 executed (27-01..27-06) -- WAVES 1 AND 2 COMPLETE; next is 27-07
status: executing
stopped_at: "WAVE 2 IS COMPLETE. All six pack bodies now exist as byte-identical .md/.ts pairs with drift rows in skillBodies.test.ts, every granted tool is TAUGHT in its own body, and the eval corpus is 30 fixtures (5 per pack, each failing differently). Nothing is deployed and no pack row exists in any deployment -- the pilot is still dark by construction. **THE THREE WAVE-2 LANES DO NOT ACTUALLY SHARE NO FILES, contrary to what 27-01-SUMMARY said** -- they share skillBodies.test.ts, skill-registry.md and workflow-packs.md, so running them in parallel would race and silently drop two lanes' drift rows (a body with no drift row ships with ZERO protection against .md/.ts divergence). They were run SEQUENTIALLY. **TWO EARLIER DECISIONS WERE CORRECTED DURING 27-04**: (1) the `0 or 6 bodies` guard I wrote in 27-02 was wrong about how this phase lands -- three lanes write TWO bodies each, so it reddened the moment the first lane committed and made wave 2 unlandable; the per-body tool-taught check stays and completeness is enforced by 27-01's manifest instead. (2) The fixture runner took one file per CASE while all three wave-2 plans name one file per PACK; it now parses each <packId>.json as an array, and needle uniqueness moved from file-keyed to case-keyed because a file-keyed check stopped seeing collisions inside one file. **BODY DESIGN NOTES WORTH CARRYING**: Business Pulse loses QuickBooks/PayPal/Square/HubSpot/Gmail/Slack -- everything its upstream depends on -- so `What I could not see` is a MANDATORY section and the body forbids calling one data point a trend. Customer Complaint teaches BOTH replyToMessage and proposePlan, because without the second the draft sits at `collecting` where nobody can approve it. Sales Call Prep reads the calendar and is granted no calendar write. Brand Review's FIRST section is `What I reviewed against` -- there is no brand store, so a review that does not name its basis is a false claim; with neither stated guidance nor vault material it reviews against general principles and SAYS so rather than declining. Process/SOP writes `Unassigned` for any owner the user never stated. **EARLIER IN THE PHASE**: 27-01's mandatory re-inventory overturned the planning files (ticket-deflector DOES exist; call-prep and process-doc live outside small-business/; four of six pack ids are exact upstream skill names; no pack needed upstreamSource: null), and two owner rulings landed -- workflowPackEvents is audit_immutable, and customer-complaint is the only pack granted proposePlan. An adversarial review of 27-02's diff found 4 confirmed defects plus 3 its own skeptic wrongly refuted; all 7 fixed. **THE LESSON THIS PHASE KEEPS RE-TEACHING: mechanism coverage is not behaviour coverage.** Six tests in this phase passed while the thing they named was broken. EVIDENCE: backend 97 files/2404, core 43/1174, contracts 5/79, four typechecks clean, biome ci clean over 691 files, provenance verifier green, 30 fixtures valid, runner self-test 25 rejections. **NEXT IS 27-07** -- it binds toolsForWorkflowPack into the runtime and lands the workflowPackEvents call sites, and it is the FIRST plan in this phase whose gates need a live deployment rather than an offline check. Working branch feat/27-02-pack-contracts."
last_updated: "2026-08-23T17:40:00.000Z"
progress:
  total_phases: 53
  completed_phases: 34
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 27
current_plan: 3 of 9 executed (27-01, 27-02, 27-03) -- WAVE 1 COMPLETE, wave 2 unblocked
status: executing
stopped_at: "WAVE 1 IS COMPLETE (27-01 + 27-02) AND 27-03 IS DONE. Nothing is deployed; no pack row exists in any deployment; the pilot is dark by construction. **27-01 PINNED THE UPSTREAM MATERIAL AND ITS STEP-0 RE-INVENTORY OVERTURNED THE PLANNING FILES.** Listing the FULL recursive tree at 5267cf7bff3031921d4474b8e8f86ad02d2b8f6d (1657 entries, untruncated) showed: (a) `small-business/skills/ticket-deflector/` DOES exist -- 27-READINESS says it does not -- and it IS the right Customer Complaint Response source; (b) Sales Call Prep and Process/SOP Builder were 'never inventoried' only because they live at `sales/skills/call-prep` and `operations/skills/process-doc`; (c) only 2 of the 6 sources are under `small-business/` at all. ONE ROOT CAUSE: both 27-RESEARCH and 27-READINESS searched only the small-business subtree the context named, while four of the six pack ids are EXACT upstream skill names. NO pack needed an `upstreamSource: null`. `handle-complaint` exists too and was REJECTED (it is a router that chains two skills; a pack carries a tool allow-list and structurally cannot dispatch). third_party/knowledge-work-plugins/ now holds 15 files snapshotted byte-for-byte with per-file SHA-256 AND the upstream git blob sha; manifest.json is the AUTHORITATIVE inventory and 27-RESEARCH's table is annotated as superseded. LICENCE FINDING, recorded not cleaned: the upstream ROOT LICENSE at this commit carries ~249 bytes of unrelated text after the Apache-2.0 appendix; it is snapshotted verbatim (a tidied snapshot can never be diffed against upstream again) and the redistributed copy is the clean per-plugin text marketing/ and sales/ ship identically. A local .gitattributes sets `* -text` on the vendored tree because the repo root's `* text=auto` would rewrite it on a Windows checkout and invalidate every hash -- verified with git check-attr. **EARLIER: TWO OWNER RULINGS.** (1) workflowPackEvents is `audit_immutable`, NOT tenant_owned (under tenant_owned, erasing one tenant silently rewrote the denominator of every pilot measure); the bare by_tenant index was REMOVED with it, the writer must be INSERT-ONLY, and no field may ever become personal data. (2) customer-complaint is granted `proposePlan` and is the ONLY pack that is -- replyToMessage never writes status, so the draft_reply contract terminated at `collecting`: visible, read-only, approvable by nobody. **AN ADVERSARIAL REVIEW OF 27-02's DIFF FOUND 4 CONFIRMED DEFECTS PLUS 3 ITS OWN SKEPTIC WRONGLY REFUTED; ALL 7 FIXED.** Sharpest: the `--packs` fixture gate could not go red (it is 27-04/05/06's ONLY blocking automated evidence and exited 0 on an empty corpus), and a mispinned provenance manifest created a permanently unactivatable immutable row. **THE LESSON THIS PHASE KEEPS RE-TEACHING: mechanism coverage is not behaviour coverage.** Five separate tests in this phase passed while the thing they named was broken -- an output contract that asserted a tool was GRANTED, a self-test counting CASES not RULES, a bounded-read seeded below its own ceiling, a --packs guard conditioned on corpus size, and a manifest field nothing cross-checked. All five now bite. EVIDENCE: core 43/1174, backend 97/2404, contracts 5/73, four typechecks clean, biome ci clean over 679 files, provenance verifier green, runner self-test 25 rejections. Nineteen mutations observed RED and restored across the three plans. **NEXT: 27-04, 27-05 and 27-06 -- they are wave 2, independent of each other, share no files, and can run in parallel.** Each authors two pack bodies (BOTH the canonical .md under packages/contracts/skills/ AND the hand-mirrored .ts constant, plus an entry in one of the two drift tables -- there is NO generator) and its fixtures. Their blocking evidence `--packs <a>,<b> --fixtures-only` is RED until that lane writes fixtures, which is the fix from 27-02's review round. Working branch feat/27-02-pack-contracts."
...
status: phase_complete
stopped_at: "PHASE 26 IS COMPLETE (21 of 21) AND SHIPPED TO PRODUCTION 2026-08-23. 26-19 and 26-20 executed and owner-approved (HOME-01); /dashboard renders CommandCenter.tsx. main and production both run 8f76aac and https://www.pikar-ai.com returns 200. THIS ENTRY CORRECTS THE TWO BELOW IT ON THREE POINTS. **(1) THE FOGLAMP PUSH-BREAKER IS FIXED -- IGNORE THE STANDING WARNING IN THE TWO ENTRIES BELOW.** They say convex/lib/foglamp.ts has no 'use node' directive so EVERY convex push fails for every module. It has the directive now, the lane is COMMITTED (63ca792), and pushes land: `npx convex function-spec --prod` lists home.js:summary, home.js:health, dispatch.js:runSpecialist/runResearch/runMedia, llm.js:* and intake.js:*. The lane was REVIEWED before committing rather than trusted, and carried three defects that would each have reddened CI (llm.ts + dispatch.ts failed `biome format`, vaultExtract.ts failed organizeImports) plus a CLAUDE.md section-6 violation (foglamp added as ^0.9.0, the only caret among the runtime deps, on a pre-1.0 package -- pinned to 0.9.0). Tracing ships DORMANT: FOGLAMP_API_KEY is not set on prod, verified. **(2) 26-19/26-20 SHIPPED A REAL PRODUCT DEFECT, NOW FIXED (85daa4b).** HOME_PRIORITY_ORDER had connection-failure at priority 0, so the live Command Center told owners to 'Connect your mailbox' above every pending approval, blocked job and imminent send. Owner ruled the same day: business work outranks the email channel. Order is now unresolved-dead-letters > stale-approval > scheduled-risk > diagnostic-blocker > binding-constraint > connection-failure > workspace. **THE LESSON TO CARRY INTO 27: THE WRONG ORDER HAD A FULLY GREEN SUITE OVER IT.** Every render-layer test held the five business signals at ok and varied only the mailbox, so all of them passed under BOTH orderings -- mechanism coverage, not behaviour coverage. FIVE files re-declare the order as a literal (none import it, deliberately), so fixing core alone would have left the visible symptom standing. The gate that pins it is new: cockpitAccess.test.ts -> 'a triggered business signal OUTRANKS a broken mailbox', which triggers a business signal AND the mailbox together. MUTATION-VERIFIED TWICE: reverting the array fails exactly it plus the ladder walk in commandCenter.test.ts and nothing else, and reverting + rebuilding + re-running the BROWSER fails rung 1 with 'a broken mailbox outranked the business work'. Any future test of an ordering must trigger two signals at once or it asserts nothing. **(3) THE E2E LADDER LOST ONE ASSERTION TO A FIXTURE LIMIT, NOT A REGRESSION.** command-center.spec.ts is 9/9 green in a real browser (prod build on :3111 + local backend). Its 'at least one strict advance' check was only ever satisfiable because connection-failure sat at priority 0 and the spec OWNS the mailbox (its finally deletes the token row). With the channel at sixth, the two rungs above the hero are the two undrivable from a browser (unresolved-dead-letters needs a v.id('requests') with no seam; stale-approval needs a backdated createdAt that plans:patchPlan refuses), and this deployment's 1056 day-old proposed plans pin stale-approval triggered forever -- observed walk stale-approval -> stale-approval -> stale-approval -> stale-approval. It is now a disjunction: either the hero walked strictly down, or it stayed put AND the code holding it still reports 'Needs attention'. To restore strict descent from a browser, add a seam that backdates plans.createdAt or mints a deadLetters row. GATES: CI green on main (typecheck/lint/test/build) and deploy-production succeeded, including its 'Dry-run Convex production bundle' step. Local: core 1123/1123, web 569/569, backend 649 across the touched modules, three typechecks clean, `biome ci .` clean over 669 files, e2e 9/9. ONE CI RED ALONG THE WAY, worth not repeating: `biome check` was run on the changed .ts files only and missed docs/playbooks/watch.json in the same commit -- biome formats JSON too, and a failed Lint step means Test and Build never run. Use `npx biome ci . --diagnostic-level=error --max-diagnostics=none`. DEPLOY GATE, CORRECTED: pushing to main NO LONGER auto-deploys. deploy-production.yml requires the head commit SUBJECT to START with [deploy] (startsWith, not contains); without it the job reports skipped in 1s. 'Push to main' and 'deploy to production' are now separate acts. WORM: owner asked, then ruled LEAVE IT OFF -- no Object-Lock bucket exists. Do NOT set WORM_BUCKET; it is the ARMING SWITCH (worm.ts:55 returns the stub-skip before any S3 call), so setting it alone would hand the 03:00 UTC cron a credential-less S3Client. Prod has none of the four required vars. The privacy policy is ACCURATE about this and needs no change. REPO HYGIENE THIS SESSION: branches 45 local/18 remote -> 8 local/4 remote (only merged ones removed, each re-verified as an ancestor of origin/main); the gutted pikar-wan-cutover-20260813 worktree removed (1707 tracked files deleted on disk, nothing unique); the remaining 6 worktrees fast-forwarded to 8f76aac and clean; 14 stashes untouched; no unpushed work anywhere; .foglamp/ and .uat-*/ now gitignored. NEXT: PHASE 27 -- curated knowledge work-pack pilot. 27-01 (pin upstream sources, Apache-2.0 provenance, reviewed-update controls) and 27-02 (native pack contracts, code-owned grants, candidate lifecycle, fixture runner) are authored. Working branch feat/27-knowledge-work-pack-pilot at 8f76aac. STILL OPEN, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, and 26-VALIDATION rows 26-06/07/08. ROADMAP.md checkboxes for 26-14..26-20 remain unticked -- the known `phase complete` no-op; STATE is the source of truth."
last_updated: "2026-08-23T03:17:20.000Z"
progress:
  total_phases: 53
  completed_phases: 34
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 19 of 21 executed (26-01..26-17 + 26-18); next 26-19
status: executing
stopped_at: "26-17 IS CLOSED 2026-08-22 -- **OWNER UAT APPROVED ('The report interface is okay') AND THE REPORTS NAV IS LIVE.** Phase 26's Reports lane (26-14..26-17) is done. **THE ONE UAT CHANGE: the Governance and Deployment cards collapse and arrive CLOSED** -- they were taking the whole page. Native <details>/<summary>, NOT a useState toggle: the element brings keyboard operation, the disclosure triangle, correct AT semantics and the open/closed state for free, and a hand-rolled toggle re-implements all four while getting the third wrong. ONLY those two collapse -- Business, Operations and Board pack stay open, because collapsing a section nobody complained about hides a number the reader expects on arrival. A CLOSED CARD STILL DISCLOSES WHETHER IT HAS ANYTHING via a summary hint ('3 shown, more available', '12 active skills'): hiding content is fine, hiding the EXISTENCE of content would make an empty governance record and a full one look identical. TASK 3: the rail item's soon:true became an href. The branch keys off href, so ROLLBACK IS DELETING IT, and rollback touches NO DATA -- a board pack is an ordinary vault row, nothing on that rail rewrites one, and neither pack module contains .patch/.replace/.delete. The e2e's nav test FLIPPED with it (it asserted DARK before the UAT, LIVE after); the record of the dark run lives in 26-17-SUMMARY.md, not in a test asserting a state the product left behind -- the 26-13 move. **A MUTANT CAUGHT A TEST A PROSE COMMENT COULD SATISFY.** The first hint assertion was expect(source).toContain('shown') -- and the explanatory COMMENT above the hint satisfied it, so deleting the hint left the suite GREEN. It now renders Section and asserts the hint appears INSIDE </summary> (mutation-verified: moving it to the body turns it red). The CALL-SITE hints are covered by the browser gate's getByText(/shown/), not by the component suite -- recorded because 'covered' and 'covered where' are different facts. **THREE THINGS ONLY RUNNING THE GATE COULD TEACH, all folded back in:** (1) wait for the window line by PATTERN, never by 'it changed' -- switching periods hands every subscription new args so useQuery returns undefined and the header honestly reads 'Resolving the window...'; (2) NEVER hardcode a timezone -- the first draft asserted 'UTC' and FAILED A CORRECT PAGE on a runner reporting Africa/Dar_es_Salaam, when the contract promises a named IANA zone plus the '(from your browser)' disclosure; (3) **`convex run` ENDS THE BROWSER SESSION** so the one test calling it mid-test (owner bootstrap/revoke) MUST RUN LAST -- in the middle it left every later test unauthenticated and auditPage never resolved, presenting as a HUNG QUERY rather than a dead session. Also: a 307 from an unauthenticated curl proves NOTHING about whether a route exists (the auth middleware redirects before the 404 is determined), and a stale `next start` will serve an old build through a rebuild -- restart it. EVIDENCE: component 28/28, e2e 7/7 EXECUTED against a rebuilt :3111 with the nav live, web typecheck + prod build clean, core 40 files/1096, backend typecheck clean, biome clean, watcher clean. THE BOARD PACK IS THE ONE NON-SEEDED CLAIM: markdownToPdf (pdf-lib, deterministic, no provider, no cent) and ctx.storage.store really ran and the href was a minted https: storage URL. DEVIATIONS: reportsView.test.**ts** not .tsx (apps/web/vitest.config.mts includes app/**/*.test.ts ONLY; a .tsx is SILENTLY SKIPPED); packages/core/src/reports.ts exports five copy helpers so the screen and the PDF share ONE vocabulary; invites.ts + beta-admission.md for the __seedInvite seam; apps/web/e2e/seed-user.setup.ts (inert unless invoked -- it matches no project's testMatch). Typed fixtures caught THREE fictions the render cast had hidden (an invented 'not-comparable' movement state, a p50/p95 latency that does not exist, a DashboardBound missing nextCursor). **THE SECOND LANE (foglamp tracing) WAS PARKED WITH OWNER AUTHORISATION AND RESTORED BYTE-EXACT** -- 13 files backed up by raw copy (not git stash), verified with cmp/md5 on the way back. None of it is in any commit. **ITS PUSH-BREAKING BUG IS BACK WITH IT: convex/lib/foglamp.ts has no 'use node' directive, so the V8 bundle pulls in foglamp@0.9.0's node:http import and EVERY convex push fails, for every module.** The deployment runs the last good push (which contains all of 26-14..26-17); no new backend edit will deploy until that lane fixes it (lazy `await import('foglamp')` inside the traced path, or move the fog instance out of convex/). STACK LEFT RUNNING for the owner: :3111 prod build (nav live, collapsible cards), :3210 local backend. Seeded non-owner account e2e-2617@pikar.test / Pikar-e2e-2617!aA; owner can be toggled on userId kn72w6h02t7k6xpeztttz83s398cy8zs via owner:bootstrapOwner / owner:revokeOwner. Still open, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, 26-VALIDATION rows 26-06/07/08. Next: 26-19 and 26-20 are the remaining plans in phase 26 (26-18 already executed)."
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 18 of 21 executed (26-01..26-16 + 26-18); 26-17 Task 1 done + gate executed, Tasks 2-3 open
status: awaiting_checkpoint
stopped_at: "26-17 TASK 1 IS COMPLETE WITH THE BROWSER GATE **EXECUTED 7/7**; **TASK 2 (OWNER UAT) IS OPEN AND BLOCKING** and Task 3 (nav activation) has not started -- the Reports rail item is still soon:true and no a[href=/dashboard/reports] exists anywhere. THIS CORRECTS THE PREVIOUS ENTRY, which recorded the gate as unrunnable. It ran. **THE BLOCKER WAS NOT THE CREDENTIALS, IT WAS A BROKEN CONVEX PUSH.** Nothing had deployed since ~19:0x: convex/lib/foglamp.ts (the OTHER LANE's file) carries no 'use node' directive, so the V8 bundle pulled in foglamp@0.9.0's `import { createServer } from 'node:http'` and EVERY push failed, for every module. The convex dev process was alive and failing silently. Diagnosed, then the owner authorised temporarily parking the foglamp lane. **THE LANE WAS PARKED AND RESTORED BYTE-EXACT** -- 13 files backed up by raw copy (not git stash), tracked ones reverted with `git checkout HEAD --`, the two untracked ones moved aside, then all 13 copied back and verified with cmp/md5. Its diff is identical to what was found. NONE of it is in any of my commits; the shared-file hunks in cockpit.md/vault.md/watch.json were withheld each time and restored afterwards. SEEDING THE ACCOUNT: /signup is invite-gated, and the only issuance path is `approve`, an ownerMutation that `npx convex run` can never call. So invites.__seedInvite (a `__`-prefixed internalMutation seam, the onboarding.__seedOnboardedTenant convention) mints the row, and apps/web/e2e/seed-user.setup.ts drives the REAL signup form. That file is INERT unless invoked explicitly -- it ends in .setup.ts, which matches neither the `setup` project's testMatch nor Playwright's default spec/test glob. **THE BOARD PACK IS THE ONE NON-SEEDED CLAIM IN THE RUN.** The audit rows are seeded and prove UI states only, but generateBoardPack executed the real markdownToPdf (pdf-lib, deterministic, no network, no provider, no cent) and the real ctx.storage.store; the download href was a minted https: storage URL and the SECOND click returned 'Already generated for this window' -- **26-16's content-hash replay observed end to end from a browser**, not asserted in a unit test. **THREE THINGS ONLY THE RUN COULD TEACH, all folded back into the spec:** (1) wait for the window line by PATTERN, never by 'it changed' -- switching periods hands every subscription new args, so useQuery returns undefined and the header honestly reads 'Resolving the window...'; reading at the moment it merely DIFFERS captures that intermediate state (all three sections blank together, so no stale number ever sits under a new header); (2) never hardcode a timezone -- the first draft asserted 'UTC' and FAILED A CORRECT PAGE on a runner reporting Africa/Dar_es_Salaam, when what the contract promises is a named IANA zone plus the '(from your browser)' disclosure; (3) **`convex run` ENDS THE BROWSER SESSION** (already in e2e/README.md and the spec's own header) so the one test calling it mid-test -- the owner bootstrap/revoke -- MUST RUN LAST; in the middle it left every later test unauthenticated and auditPage never resolved, which presents as a HUNG QUERY rather than a dead session. ALSO LEARNED, cheaply: a 307 from an unauthenticated curl proves NOTHING about whether a route exists -- the auth middleware redirects before the 404 is determined. :3111 was `next start` on a stale build and had to be restarted on the fresh one. EVIDENCE: e2e 7/7 executed, component 22/22, web typecheck + prod build clean, core 40 files/1096, backend typecheck clean, biome clean, watcher clean. DEVIATIONS: the component test is reportsView.test.**ts** not .tsx (apps/web/vitest.config.mts includes app/**/*.test.ts ONLY; a .tsx is SILENTLY SKIPPED); packages/core/src/reports.ts exports five copy helpers so the screen and the PDF share ONE vocabulary; invites.ts + beta-admission.md for the seam; apps/web/e2e/seed-user.setup.ts is new. **THE FOGLAMP LANE IS BACK IN THE TREE AND ITS PUSH-BREAKING BUG IS BACK WITH IT** -- convex/lib/foglamp.ts still has no 'use node' directive, so the deployment will refuse every push again until that lane fixes it (lazy `await import('foglamp')` inside the traced path, or move the fog instance out of convex/). The local backend and :3111 are both up (200/200). NEXT: the owner's UAT verdict on Task 2 (periods at desktop/tablet/mobile + keyboard, owner vs non-owner governance, generate/download one pack and compare its as-of to the page, and the disablement demo), THEN Task 3 nav activation + 26-17-SUMMARY.md, which the plan says to write only after approval."
last_updated: "2026-08-22T21:05:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 18 of 21 executed (26-01..26-16 + 26-18); 26-17 Task 1 done, Tasks 2-3 open
status: awaiting_checkpoint
stopped_at: "26-17 TASK 1 IS COMPLETE AND COMMITTED; **TASK 2 (OWNER UAT) IS OPEN AND BLOCKING, AND THE BROWSER GATE HAS NOT RUN.** Task 3 (nav activation) has not started -- the Reports rail item is still `soon: true` and no a[href=/dashboard/reports] exists anywhere. SHIPPED: /dashboard/reports, reachable directly while its nav item stays dark. THE PAGE ADDS NO BACKEND SURFACE -- three tenantQuerys (business/operations/auditPage), two ownerQuerys (wormExport/activeSkills), one tenantAction (generateBoardPack) and api.vault.vaultDownloadUrl, all shipped in 26-14/15/16. **THE ONE DECISION EVERYTHING ELSE HANGS OFF: THE WINDOW ANCHOR IS PINNED AT MOUNT** -- `const [anchorMs] = useState(() => Date.now())`, and a source-scan test asserts Date.now() appears EXACTLY ONCE in the module. A live clock in render is wrong twice: every re-render mints a new untilMs so every Convex subscription gets a new query key and the page refetches forever; AND landPack's replay key is the CONTENT hash, so a drifting upper bound makes every click a different report and fills the vault with near-duplicate packs. Pinning is what makes 'generate twice, get one artifact' true in a browser and not only in a unit test. All three sections plus the pack action receive ONE `args` object, asserted by name. The DISPLAYED window is the one the SERVER resolved (auditPage echoes resolveDashboardWindow's output), so 'every section shows the same resolved window' is checkable rather than assumed. **THE COVERAGE VOCABULARY IS NOW SHARED**: countCell/coverageWord/floorCell/fmtDate/fmtDateTime are exported from @pikar/core and used by BOTH the PDF builder and the screen. A page saying '0' where the pack says 'not measured' is the 26-14 defect inverted, and two surfaces drift by each owning a copy. A NON-OWNER NEVER CALLS AN OWNER QUERY -- useQuery(..., isOwner ? {} : 'skip'). Hiding a control is presentation, not the boundary. The WORM card renders lastCursorAdvanceMs and NO health word; a component test bans Healthy/Degraded/OK from it, because 26-15 removed exactly that claim from the backend AND the mockup. **TWO FIXTURE FICTIONS CAUGHT BY TYPING THE FIXTURES**: the first draft of reportsView.test.ts invented `movement: {state:'not-comparable'}` and a p50/p95 latency, and BOTH rendered plausibly because the render() cast hid them. Typing the fixtures as BoardPackInput['operations'|'business'] turned them into compile errors, and then caught a THIRD (DashboardBound requires nextCursor). A fixture that seeds a literal nothing writes is the 26-14 `edit: 0` defect wearing a test's clothes. DEVIATION, same as 26-13: the component test is reportsView.test.**ts**, not .tsx -- apps/web/vitest.config.mts includes app/**/*.test.ts ONLY and a .tsx there is SILENTLY SKIPPED. Also packages/core/src/reports.ts was modified (the five copy exports), which the plan does not list. EVIDENCE SO FAR: component 22/22, web typecheck clean, prod build clean (/dashboard/reports compiled), core 40 files/1096 passed + typecheck, backend typecheck, biome clean on every touched file, watcher clean. **BLOCKER, AND IT IS THE HONEST REASON THIS PLAN IS NOT CLOSED: e2e/reports.spec.ts IS WRITTEN (7 tests) BUT HAS NOT RUN.** The stored apps/web/e2e/.auth/user.json JWT EXPIRED 2026-08-22T11:43Z, and auth.setup.ts requires E2E_USER_EMAIL / E2E_USER_PASSWORD, which are in NO env file in this repo. The local stack is up (:3111 next dev, :3210 convex) and already serves the route. Nothing in the playbooks may be cited as executed browser evidence until this runs. The spec covers: the nav-dark gate; one-window synchronization across periods (same upper bound, only the start moves); per-section render incl. 'not measured' for an unmeasured source; a PRIVACY sweep that seeds a recipient address and a prose draft into audit payloads and scans page.content() for them (paired with a PRESENCE assertion on the allowlisted refs, so it cannot pass on an empty table); the owner/non-owner split via owner:bootstrapOwner + owner:revokeOwner in a `finally` (a leaked owner bit would make every later non-owner assertion vacuous); and a REAL generate->download->replay (markdownToPdf is pdf-lib, deterministic, no provider, no cent -- genuine render+storage evidence, the only external-looking claim in the file). **THE SECOND LANE (foglamp tracing) IS STILL LIVE AND UNCOMMITTED IN THIS TREE** and none of it is in this commit; the cockpit.md hunk was withheld and restored afterwards. NEXT: (1) supply E2E_USER_EMAIL/E2E_USER_PASSWORD (or authorise seeding a fresh e2e user at /signup) so the browser gate can run; (2) THEN the owner UAT (Task 2, blocking); (3) THEN Task 3 nav activation + 26-17-SUMMARY.md, which the plan says to write only after approval."
last_updated: "2026-08-22T20:20:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 18 of 21 executed (26-01..26-16 + 26-18); next 26-17, wave 13
status: executing
stopped_at: "26-16 IS CLOSED 2026-08-22 -- the board pack. ONE TRANSACTION IS THE SNAPSHOT, and there is no snapshot table (26-02 decided that; Convex read transactions are serializable, so the value ONE internalQuery returns IS the immutable input and the renderer never re-reads). reportPackData.snapshot reads all three planes in one Promise.all; reportPack.ts has EXACTLY ONE ctx.runQuery and a test asserts it. **asOf IS THE WINDOW'S EXCLUSIVE UPPER BOUND, NEVER A WALL CLOCK** -- a Date.now() there makes every replay produce different markdown, different bytes and a different content hash, which destroys the replay key and turns every double-click into a second artifact. The generation instant lives on the vault row's createdAt, where a fact about the render belongs. NOTHING IS A SECOND MECHANISM: resolveDashboardWindow, the 26-14/26-15 planes lifted to plain readBusiness/readOperations/readAuditPage functions (the readLiveForTenant precedent), a pure @pikar/core markdown builder, the shipped markdownToPdf, vault.insertCreatedDoc UNCHANGED, and the insert-only audit.log. kind is `created_document`, NOT a new `board_pack` -- that kind is already inside content.ts's POSITIVE LANE_BY_KIND whitelist, so the Content shelf, preview and promotion work with zero new code; a new kind would have landed OUTSIDE the whitelist and been silently absent, which is the 26-13.1 defect repeated. `use node` split (reportPack.ts action / reportPackData.ts query+mutation) because markdownToPdf lives in llm.ts which imports node:crypto -- the media.ts/mediaComplete.ts precedent. BOTH modules are in _generated/api.d.ts, so the real deployment analyzed them, not just the harness. DELIBERATELY ABSENT from the pack: wormExport (reads audit.by_ts with no tenant predicate -- aggregates over OTHER tenants), activeSkills (deployment-global), sentMail rows (a 50-row sample against a 1000-row sentCount prints a floor as a ratio, and a recipient address must not ride a distributable PDF). **HOW THIS PLAN RAN: an ultracode workflow (4 subsystem readers -> 1 design spec -> 1 implementer) that the OWNER STOPPED MID-FLIGHT because the adversarial-review phase spawned a refuter PER FINDING and the fan-out multiplied without a ceiling (~38 agents, too slow). The implementation had already landed; the review it never reached was done BY HAND and found a real defect.** **THE DEFECT: the replay guard admitted only origin:'agent', and vault.promoteToReference -- the control 26-13 shipped -- patches THAT VERY FIELD to 'agent_promoted'.** So promoting a pack made the guard stop recognising it: regenerate the same window and it inserted a DUPLICATE, and every regeneration after that inserted another. BOTH HALVES WERE ALREADY TESTED ('replaying returns the SAME row' and 'the SAME reads DO change when promoted') AND NEITHER TEST COMPOSED THEM -- 19/19 green over the defect. Fixed to agent OR agent_promoted (promotion is a trust decision the user took ABOUT the artifact, not a change of authorship), and .first() -> .take(DUP_SCAN)+.find() because .first() returns whatever the index orders first, so one foreign row sharing the hash would mask ours. Two tests added, BOTH MUTATION-VERIFIED. HONEST NUMBERS REACH THE BYTES: countCell refuses to print a number under unknown coverage, floorCell prints 'at least N (partial)', and partialSections is derived in the markdown builder and handed back so the audit reports the SAME number rather than recomputing it. RECORDED IN vault.md AND WORTH REMEMBERING: **_scheduled_functions is NOT the discriminator for 'did ingest start'** -- workflow.start schedules INSIDE the workflow component, so the ROOT scheduler stays empty for a real ingest too. The non-groundability proof is the field flip under promotion, not an empty scheduler. EVIDENCE: reportPack 21/21, backend 95 files/2338 passed/0 failed, core 40/1096, contracts 4/64, all three typechecks clean, biome clean, watcher clean. DEVIATIONS: reportPackData.ts (new, forced by `use node`), reportsBusiness.ts + reportsGovernance.ts (handler bodies lifted to plain functions so one transaction can call all three), core/reports.ts (the pure builder), contracts/auditProjection.ts (the report.pack_generated allowlist row -- 26-15's drift guard REQUIRES it, and its 14 keys were checked against the write site by hand), audit-dead-letter.md + watch.json. No kill switch: a refusal nothing can produce is a lie in a discriminated union; rollback is 26-17's nav gate or reverting the module, and neither module contains .patch/.replace/.delete (statically scanned). **A SECOND LANE IS LIVE IN THIS WORKING TREE (foglamp tracing: lib/foglamp.ts, llm.ts, dispatch.ts, intake.ts, vaultExtract.ts, package.json, pnpm-lock.yaml, tracing.md, cockpit.md, intake.md, and hunks inside vault.md + watch.json). NONE OF IT IS IN THIS COMMIT** -- the shared-file hunks were withheld and restored to the tree afterwards, so that lane's uncommitted work is intact. Still open, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, 26-VALIDATION rows 26-06/07/08. Next: 26-17 (wave 13, the Reports page + board-pack download), the LAST plan of phase 26 alongside 26-19/26-20."
last_updated: "2026-08-22T19:45:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 17 of 21 executed (26-01..26-15 + 26-18); next 26-16, wave 12
status: executing
stopped_at: "26-15 IS CLOSED 2026-08-22 -- the governance READ boundary, and the mockup claim it had to destroy. **THE MOCKUP SAID THE AUDIT VIEWER IS 'SAFE BY CONSTRUCTION, NOT BY FILTERING', BECAUSE THE ROWS CARRY REFS AND COUNTS ONLY. THAT IS NOT A SCHEMA PROPERTY.** audit.log declares payload: v.any() and then writes `const payload: AuditPayload = args.payload` -- an interface, erased at runtime, assigned straight out of `any` with no check -- and 96 write sites across 40 modules feed it. The contract is ALREADY broken in production: piiCounts: Record<string,number> is a NESTED OBJECT in the audit table right now (intake.ts:253, pipeline.ts:196, vaultExtract.ts:497), a shape AuditPayload's own header calls 'not representable'. It is representable; it is simply not TYPED. So the viewer is safe by FILTERING and packages/contracts/src/auditProjection.ts is the filter: a per-event KEY allowlist (72 events read off the write sites; an unknown event yields a SHELL -- ts, actor, correlation, zero payload detail -- so it fails CLOSED), then a SHAPE gate on every surviving value (bounded, whitespace-free, markup-free primitives and string arrays; objects, arrays of objects and oversized strings DROPPED, never truncated, never stringified). THREE KEYS ARE DELIBERATELY ABSENT and the reasons are the reusable part: piiCounts (nested -- kept out of the KEY gate so `unsafeDrops` stays a real signal instead of firing on every redaction row; upgrade path is a flat piiTotal at the write site), userId/ownerUserId (a raw identity walking around the actor normalizer), and tenant.deleted's deleted_<table> counts (key names computed from deletableTables() at runtime, so they cannot be enumerated honestly). **A MUTANT FOUND A GUARANTEE WITH ZERO COVERAGE: 'never stringify the payload as a fallback' had TWO tests and a JSON.stringify fallback passed both.** Both fed a nested object under a key the ALLOWLIST already refuses, so the SHAPE gate was never entered -- the tests measured gate 1 while claiming to measure gate 2. A TEST THAT REFUSES INPUT AT GATE 1 PROVES NOTHING ABOUT GATE 2. Fixed with an object in an ALLOWLISTED key, and the injection sweep now feeds every needle bare, object-wrapped and array-wrapped across every allowlisted key of every event. **THE SECOND MOCKUP LIE: the WORM card's green 'Healthy' pill.** exportCursors holds ONE number -- the ts the exporter last SAID it had written. No S3 object is read back, no Object Lock retention is checked, and a cron that died mid-upload after advancing looks identical. reportsGovernance.wormExport returns lastCursorAdvanceMs (null until it has ever advanced, a different fact from '0 rows behind') and returns NO `healthy` and NO `status` -- a test asserts those keys are ABSENT, because a verdict this data cannot support must not be inventable downstream. rowsAwaitingExport is a floor; oldestAwaitingMs is exact and is the honest 'lag'. **BOTH CORRECTIONS WERE ALSO MADE IN docs/design/mockups/pending-pages.html**, because 26-17 builds the UI from that file and a backend that refuses to claim health does not help if the page still renders a green pill -- the 26-14 lesson applied before it could repeat. **TWO EXISTING GUARDS CAUGHT THIS PLAN'S OWN CHANGES, and neither was mine:** isolation.test.ts's self-growing owner surface went red the moment reportsGovernance added two ownerQuerys (16 -> 18, and its generated sweep now covers non-owner rejection for both independently), and llmRedaction.test.ts's RCE-door scan went red because lifting seeds to module scope as SEEDS broke its non-vacuity anchor -- A RENAME WOULD HAVE SILENTLY DISARMED A SECURITY SCAN if that guard had been written any softer. EVIDENCE: contracts 4 files/64 with auditProjection 21/21 and 5/5 mutants caught; backend 94 files/2314 passed/0 failed with reportsGovernance 17/17 and 6/6 mutants caught; both typechecks clean; biome clean on every touched file; watcher clean. DEVIATIONS: contracts/src/index.ts NOT modified though the plan names it (that file's own header says domain modules ship via the ./* subpath export and must NOT be re-exported there); four backend files the plan did not list (skills.ts -- seeds lifted to module scope as SEEDS with REGISTRY_SKILL_NAMES derived from it, so activeSkills drives one indexed read per skill and no second hand-typed name list can drift; wormCursor.ts -- CURSOR_NAME exported so the reader names the row the exporter advances; reportsBusiness.ts -- MAX_WINDOW_MS exported so Reports cannot grow two window ceilings; isolation.test.ts); plus the mockup and three playbooks (audit-dead-letter, dashboard-pages, skill-registry). **ALSO REPAIRED THIS SESSION, BEFORE 26-15 STARTED: HEAD DID NOT TYPECHECK.** e89da61 committed reportsBusiness.ts with `import { readLiveForTenant } from './blueprint'` while the `export` keyword on that function stayed in the working tree, along with three written-but-unstaged 26-14 playbook blocks. The working tree was always green, which is exactly why nothing caught it -- THE VERIFICATION RAN AGAINST THE TREE, NOT THE COMMIT. Committed as 85164ef. NOT DONE: no apps/web file -- 26-17 owns the route and must render known:false rows as SHELLS rather than hiding them (a governance record with holes is worse than one with shells). activeSkills reports only skills that HAVE an active version; a skill with none is ABSENT, not v0 (ponytail: surfacing 'no active version' is real -- an unseeded document-classifier makes classifyDoc fail closed -- and the upgrade path is version:null for the gaps). Still open, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, 26-VALIDATION rows 26-06/07/08. Next: 26-16 (wave 12, the board pack)."
last_updated: "2026-08-22T17:30:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 15 of 21 executed (26-01..26-13.1 + 26-18); next 26-14, wave 11
status: executing
stopped_at: "26-13.1 IS CLOSED 2026-08-22 -- the image lane is back, the shelf is searchable, and the whole Content lane (26-11..26-13.1) is done. THIS PLAN WAS A REPAIR, NOT A FEATURE: 26-12 excluded kind:'image' from the shelf under a comment calling those rows 'media intermediates', and THAT COMMENT WAS FALSE. mediaComplete.saveImageToVault says in its first sentence that it is 'scoped to the STANDALONE IMAGE' (plans.mediaMode === 'image') and is saveReelToVault's twin; a reel's scene image shares the mediaJobs.kind token but NEVER becomes a vault document at all, because deleteIntermediates removes its bytes at the render terminal. Both halves are now pinned: a standalone image appears with bytes, provenance and promotion:not-applicable; a bare mediaJobs row produces NO shelf card. **THE LESSON IS THE COMMENT, NOT THE LINE: a membership decision justified by a claim about another module is only as true as that claim, and mine was written from the FIELD NAME rather than the WRITE SITE.** Found by the owner in one sentence at the 26-13 UAT. SEARCH: the window is sliced to `limit` FIRST and the title match runs over that page, so search narrows what is SHOWN and never what is LOOKED AT. Two load-bearing consequences: (1) THE CURSOR COMES FROM THE LAST ROW OF THE WINDOW, NOT THE LAST ROW RETURNED -- otherwise a page whose every row is filtered out reports nextCursor:null and strands the rest of the shelf behind a search term (mutation-checked); (2) the result line carries the DENOMINATOR ('3 of the 24 newest artifacts match'), because a bare match count has no scale and 'nothing found' would be indistinguishable from 'nothing found ON THIS PAGE'. ponytail ceiling named at the call site: a substring match over the page, not a search index; upgrade path is withSearchIndex on (tenantId,title). **ONE SOURCE-SCAN ASSERTION WAS LOOSENED ON PURPOSE and the reasoning is the reusable part:** contentView.test.ts banned api.vault.vaultDownloadUrl from the view entirely, and the thumbnail needs it -- a flat ban FORBIDS THE FEATURE RATHER THAN THE FAILURE, and the failure is a URL minted for a row nobody looked at. It now asserts the call appears EXACTLY ONCE, inside ImageThumb, and that ImageThumb renders only on the image lane. EVIDENCE: backend content 25/25 with 4/4 new mutants caught (drop the image lane; cursor from the returned rows; the match count as denominator; filter before slicing), component 27/27, e2e/content.spec.ts EXECUTED 9/9 on a rebuilt :3111, web suite 30 files/480, backend 92 files/2268, web typecheck + prod build clean, and THE WATCHER CAUGHT A REAL OMISSION FIRST (cockpit.md watches apps/web/e2e/ and the spec had changed). OWNER CONTEXT CARRIED FORWARD: the owner also asked for a content calendar, funnel templates driving organic/paid campaigns, and a social-accounts tab with per-post KPIs. ALL THREE ARE ALREADY THEIR OWN 2026-08-07 DECISION (ADR-015) and are ON THE ROADMAP: a calendar is deliberately not a surface while email is the only outbound channel (scheduled email lives in Approvals' 'Scheduled -- approved, not yet fired'); funnels are PHASE 31 (tranche A, link-only funnel v0, NO legal entity required) and the owner chose to FINISH PHASE 26 FIRST; social connection/publishing/per-post metrics are PHASE 32, structurally blocked on the LEGAL ENTITY, which the owner reconfirmed 2026-08-22 as STILL NOT STARTED (it also gates Google OAuth verification, custom domains, CASA and billing -- the ROADMAP blocker section is re-dated). PROPOSED AND NOT SCHEDULED: a 'what happened to it' line on each card (sent to N people, cited N times -- derivable from the thread/plan the artifact already carries), and a delete control on the card (promotion is one-way and deleting is the only undo, currently one level in). Still open, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, two red finance tests from 26-10, 26-VALIDATION rows 26-06/07/08. Next: 26-14 (wave 11, Reports)."
last_updated: "2026-08-22T13:55:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 14 of 21 executed (26-01..26-13 + 26-18); next 26-13.1, wave 10.1
status: executing
stopped_at: "26-13 IS CLOSED 2026-08-22 -- OWNER UAT APPROVED ("The page is minimalistic. It works great.") AND THE CONTENT NAV IS LIVE. Task 3 replaced the disabled Soon item with an href; the branch keys off href, so ROLLBACK IS DELETING IT -- and rollback does NOT touch an artifact's origin, because promotion is a trust decision the USER took about their own reference material, not a property of a route. e2e/content.spec.ts EXECUTED 7/7 TWICE: once with the nav dark (the gate) and again after activation. Web typecheck, prod build and the watcher clean; component 23/23 with 7/7 view mutants caught; web suite 30 files/476; backend 92 files/2263. The vault.promoted audit row was read back from the deployment: refs, ids and a closed enum, no title, no body. **WHAT THE OWNER DID NOT ADJUDICATE, recorded rather than rounded up:** the 390x844 responsive pass, the keyboard pass and the each-kind-opens list -- those are covered by the executed browser gate, not by an owner verdict. **THE E2E'S FIRST RUN FAILED AND THE FAILURE GENERALISES: a fire-and-forget follow-up call is not observable through the state the first call changes.** 'Reference material' appears the moment the reactive query sees the patched row, while the caller-side audit call is still in flight; the test ended, Playwright tore the context down mid-mutation, and the next test found an empty audit table for a promotion that had genuinely happened. Fixed with a SEQUENCING POINT (the confirm block detaches only after recordPromotion resolves), never a sleep. THREE DEVIATIONS, all recorded in 26-13-SUMMARY: the component test is .ts not .tsx (apps/web's vitest config includes app/**/*.test.ts ONLY and a .tsx there is SILENTLY SKIPPED); contentAudit.ts is a new backend module because the vault.promoted row had no legal home -- and note that a tenantMutation in audit.ts WOULD HAVE PASSED auditImmutability.test.ts, whose PUBLIC_BUILDER regex only matches the raw builders, which is a reason to respect the stated invariant rather than use the gap; and smoke.seedContentShelf is an E2E fixture seam. **THE OWNER FOUND A REAL 26-12 DEFECT: the shelf excludes kind:'image' on a FALSE premise.** mediaComplete.saveImageToVault is explicitly 'scoped to the STANDALONE IMAGE' (plans.mediaMode === 'image') and is saveReelToVault's twin; a reel's scene images NEVER become vault docs (deleteIntermediates removes them), so every kind:'image' vault row is a finished deliverable and the shelf of 'everything Pikar has made' is missing one of the two things Pikar makes. THE LESSON IS THE COMMENT, NOT THE LINE: a whitelist entry justified by a claim about another module is only as true as that claim, and mine was written from the field name rather than the write site. **26-13.1-PLAN.md IS WRITTEN AND IS NEXT** (image lane + bounded title search + on-demand thumbnails; 3 tasks, autonomous). FOUR OWNER DECISIONS THIS SESSION: (1) UAT approved, nav activated; (2) the image lane is its own follow-up plan, before anything else; (3) FUNNELS STAY ON THE ROADMAP -- Phase 31 (tranche A, funnel v0 link-only) needs no legal entity and could be pulled forward, but the owner chose to FINISH PHASE 26 FIRST; (4) **THE LEGAL ENTITY HAS STILL NOT STARTED** (reconfirmed 2026-08-22 while asking for the social tab it blocks) -- it simultaneously gates Phase 32 (social connection/publishing/per-post metrics), Google OAuth verification, custom domains (Phase 25 SC#6), CASA and billing; the ROADMAP blocker section is re-dated rather than duplicated. The owner also asked for a content calendar, funnel templates and social KPIs -- ALL THREE ARE ALREADY THEIR OWN 2026-08-07 DECISION (ADR-015): Content narrows to an artifact shelf, Marketing takes channel/funnel/metrics, and a calendar is deliberately not a surface while email is the only outbound channel (scheduled email already lives in Approvals' 'Scheduled -- approved, not yet fired'). Still open, not introduced here: the agent-relayed citation-label gap from 26-12-SUMMARY, two red finance tests from 26-10, 26-VALIDATION rows 26-06/07/08. Next: 26-13.1, then 26-14 (wave 11, Reports)."
last_updated: "2026-08-22T12:40:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 413
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 13 of 20 in progress (26-01..26-12 + 26-18 executed; 26-13 Task 1 done, Task 2 UAT OPEN)
status: awaiting_checkpoint
stopped_at: "26-13 TASK 1 COMPLETE AND COMMITTED (cd63ebf) 2026-08-22; **TASK 2 (owner UAT) IS OPEN AND BLOCKING, TASK 3 (nav activation) HAS NOT RUN — the Content rail item is still 'Soon'.** The UAT script and the live-stack details are in 26-13-UAT-SCRIPT.md; 26-13-SUMMARY.md is deliberately NOT written (the plan says only after approval). EVIDENCE, all executed: e2e/content.spec.ts **7/7 IN THE BROWSER** against a rebuilt :3111 + the local convex backend; component 23/23; web suite 30 files / 476 passed; backend suite 92 files / 2263 passed; both typechecks and the prod build clean; 7/7 view mutants caught; playbook gate verified LIVE for the smoke.ts/agent-runtime.md coupling (ack cleared + playbook reverted => a real block). SHIPPED: /dashboard/content, reachable directly while its nav item stays disabled (the spec asserts aria-disabled AND that no a[href=/dashboard/content] exists anywhere). THE PAGE ADDS NO BACKEND SURFACE — open/download is the Vault's own PreviewModal reached BY ID (workspace/cards.tsx's shipped VaultDocModal pattern), playback is api.media.reel whose non-null url IS the validated-assembly guarantee, promotion is api.vault.promoteToReference called directly. A REEL IS DELIBERATELY NOT OPENED THROUGH THE MODAL: that would play the row's own bytes, and the difference between Vault (shows you your files) and Content (presents governed artifacts) is a Play button that is ABSENT rather than dead. Promotion opens a confirm block carrying the one-way sentence verbatim plus 'this cannot be undone', and the transition renders from the ROW's own status. **THE vault.promoted AUDIT ROW NEEDED A NEW MODULE, contentAudit.ts, and its header names why it is in none of the three obvious homes:** vault.ts is kept log-free by construction (vaultRedaction.test.ts), content.ts is read-only by construction (content.test.ts), and audit.ts states it exposes no client-callable builder — a tenantMutation there would have SLIPPED PAST auditImmutability.test.ts, whose PUBLIC_BUILDER regex only matches the raw builders, which is a reason to respect the stated invariant rather than a licence to use the gap. Verified live in the deployment: {result:'processing', sourceThreadId:'…', sourcePlanId:null, vaultDocId:'…'}, actor user, correlation vault:promote:<docId>. **THE E2E'S FIRST RUN FAILED AND THE FAILURE WAS REAL AND GENERALISABLE:** test 6 asserted only that the card said 'Reference material', which appears the moment the reactive query sees the patched row — WHILE the caller-side audit call is still in flight. The test ended, Playwright tore the context down mid-mutation, and test 7 found an empty audit table for a promotion that had genuinely happened. **A FIRE-AND-FORGET FOLLOW-UP CALL IS NOT OBSERVABLE THROUGH THE STATE THE FIRST CALL CHANGES** — the fix is a sequencing point (the confirm block detaches only after recordPromotion resolves), never a sleep. TWO DEVIATIONS FROM files_modified, both recorded: (1) the component test is contentView.test.**ts**, not .tsx — apps/web/vitest.config.mts includes app/**/*.test.ts ONLY and its own header records that a .tsx there is SILENTLY SKIPPED, which is exactly how a test file becomes decoration; (2) contentAudit.ts/.test.ts and a smoke.seedContentShelf fixture seam are backend files this web-only plan did not list (both registered in watch.json). SEEDED ROWS PROVE UI STATES ONLY — the E2E's 'reel' is a few bytes with a video mime and its sidecar is a marker; nothing rendered, nothing embedded, no provider ran, no cent spent. A FRESH never-promoted document is already seeded on the e2e tenant for the UAT (promotion is one-way, so re-running on a promoted row exercises already_promoted instead of the transition). THE STACK IS LEFT RUNNING for the UAT: prod build on :3111, convex dev on :3210, sign in as e2e-wave6@pikar.test. Still open, not introduced here: the agent-relayed citation-label gap recorded in 26-12-SUMMARY, two red finance tests from 26-10, 26-VALIDATION rows 26-06/07/08. Next: the owner's UAT verdict, then 26-13 Task 3 (nav) and 26-13-SUMMARY.md."
last_updated: "2026-08-22T12:05:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 412
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 13 of 20 executed (26-01..26-12 + 26-18); next 26-13, wave 10
status: executing
stopped_at: "26-12 COMPLETE 2026-08-22. content 20/20, full backend suite 91 files / 2258 passed / 0 failed, backend typecheck clean, biome clean, playbook gate verified LIVE (ack entry cleared + playbook reverted => a real block naming content.ts). SHIPPED: packages/backend/convex/content.ts -- THREE tenantQuery EXPORTS AND NOTHING ELSE (listArtifacts / summary / artifactById). The plan asked for tests proving zero reuse-side writes and that signed URLs never reach a log; a QUERY CANNOT WRITE and a module with no getUrl call site has no URL to leak, so those are STRUCTURAL facts scanned out of the source (no db write, no scheduler, no runMutation, no audit call, no storage URL minting, and every `export const` must bind a tenantQuery) rather than payload assertions. THREE TERMINALS ALREADY EXISTED AND NONE WAS REBUILT: download = api.vault.vaultDownloadUrl, playback = api.media.reel (its non-null url IS the validated-sidecar guarantee), promotion = api.vault.promoteToReference per the 26-11 owner decision. content.ts reads `origin` only to decide what to OFFER, and one test pins each projected verdict (eligible/promoted/retry/not-applicable) against the REAL mutation's {ok:...} so the page can never grow a button that always refuses. The shelf is a POSITIVE KIND WHITELIST (created_document, created_content, next_step_memo, reel) because vaultDocuments.kind is v.string() and grows every phase -- that closes the invariant 26-11 handed on (research briefs are not projected into Content) and sent mail never appears because `requests` is never queried, which is itself scanned. A REEL PLAYS ON THREE TERMS AND THE THIRD IS THE ONE A NAIVE CHECK MISSES: bytes, the live artifact triple, AND plan.reelVaultDocId === doc._id -- without it a re-rendered thread serves reel #2's video under reel #1's title. DEVIATION (one file the plan did not list): schema.ts -- by_tenant_kind gained createdAt, a THIRD FIELD ON AN EXISTING INDEX rather than a fifth index, because the union's cursor needs a range on the same read; safe because the one production caller (onboarding.currentProfileDoc) .collect()s and re-sorts in memory. No playbook watches schema.ts; vault.md deliberately untouched (the vault's own paths are unchanged and a second copy of the rationale is duplication). NINE MUTANTS APPLIED AND REVERTED, NINE CAUGHT -- but M9 (drop the tenant term on the reel's plan join) SURVIVED THE FIRST PASS: the join reads reelMeta.planId, a field on a row the tenant owns, and the code TRUSTED that the plan it names is theirs. Nothing asserted it until a test was written. ASK WHAT THE CODE TRUSTS, NOT ONLY WHAT IT CHECKS. CORRECTIONS IN PASSING: every verify command in dashboard-pages.md was the non-filtering `test -- <filters>` form and is fixed; the Playwright lines went further because `test:e2e <file>` does not filter EITHER (the playbook already recorded that while its own code block still quoted the broken form) so they now say `npx playwright test <file>` from apps/web; the THREE historical entries that quote the broken form on purpose were restored after a blanket replace mangled them; 26-VALIDATION's row 26-12 and Quick-run row likewise. **A THIRD WAY THE PLAYBOOK GATE READS AS FINE WITHOUT RUNNING: check-playbooks.mjs reads its hook input from stdin (fd 0), so running it bare in a terminal HANGS FOREVER instead of failing -- use `echo '{}' | node scripts/check-playbooks.mjs check`.** FINDING HANDED ON, NOT FIXED: the `agent-relayed` citation label keys ONLY on origin === 'agent_promoted', but next-step memos, reel transcripts and research briefs are all agent-authored, all ingested at their write site, and all carry NO origin -- so evaluations.ts:382 cites them as source:'vault', the same label the owner's own uploaded P&L gets, and runEvaluation's labelled-number scan runs over EVERY grounded chunk. Same laundering class 26-11 closed, one door over; traced in code, NOT observed in production. NOT DONE: no apps/web file -- 26-13 owns the route, the one-way promotion warning, the refs-only vault.promoted audit row and the E2E, and must NOT re-wrap any of the three existing surfaces. Still open, not introduced here: two red finance tests from 26-10, 26-VALIDATION rows 26-06/07/08. Next: 26-13 (wave 10, the connected Content route + owner UAT)."
last_updated: "2026-08-22T04:30:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 412
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 12 of 20 executed (26-01..26-11 + 26-18); next 26-12, wave 9
status: executing
stopped_at: "26-11 COMPLETE 2026-08-22. Full backend suite 90 files / 2237 passed / 0 failed, backend typecheck clean, playbook gate verified LIVE (not merely silent). THE PLAN WAS RE-PLANNED BEFORE IT RAN: a 10-agent audit found it unexecutable as written -- two of Task 1 three write sites take provenance from callers (llm.ts, dispatch.ts) the plan never listed; the promotion action omitted the status:processing its own failure-visibility test depended on; it demanded a concurrent-replay state convex-test cannot produce; and BOTH verify commands used `pnpm ... test -- <filters>`, which DOES NOT FILTER. **THE VERIFY-COMMAND DEFECT IS THE GENERALISABLE ONE:** pnpm forwards `--` literally, so vitest run '--' 'vault' collapses vitest filter matching and runs ALL 90 files -- measured, `test -- zzzznotarealfile` happily ran dispatch.test.ts. The failure is INVERTED (slower, appears to do more, passes), so a focused-suite verify silently proves nothing about scope; 26-VALIDATION row 26-11 carried the same broken form PLUS a dead `content` filter term matching no file. Also measured: `node scripts/check-playbooks.mjs` can never fail an && chain (all six exit sites are process.exit(0); it signals by PRINTING a block decision), and it has an ACK CACHE keyed by the changed-file hash -- reverting a playbook alone will not re-block, which confounded my first attempt to prove the gate was live. SHIPPED: provenance (sourceThreadId + sourcePlanId, both v.optional, no backfill) at FOUR write sites -- approved memo, created document, research brief, and rendered media (image + reel, owner decision) -- always taken from the plan row the writing path already holds, never from a model-supplied tool argument. `api.vault.promoteToReference`: one guarded serializable transaction (get + tenant term + POSITIVE origin whitelist + patch), origin->agent_promoted AND status->processing, exactly one startIngest, returning processing/already_promoted/ineligible. All 5 named mutants CAUGHT by exactly the named tests. THE PROVENANCE-LAUNDERING DOOR IS SHUT ON BOTH PLANES: fillVault stamped every grounded chunk {confidence:high, source:vault} -- the same label the owner own uploaded P&L gets -- so a figure the model invented in its own createDocument output would have been cited back to the owner as their own source. ownedDocsMeta now carries origin, vaultGroundHydrated returns a parallel origins array FROM THE READ IT WAS ALREADY DOING (so the audited plan extra query was DELETED, not added), evaluations cite agent-relayed, and the cockpit searchVault fence marks the label the model reads. Confidence stays high deliberately -- only the ATTRIBUTION changes. **DEVIATION, and the plan was wrong not the code:** Task 2 specified an audit row inside promoteToReference; vaultRedaction.test.ts is a shipped STATIC SCAN asserting the vault content plane emits NO log-plane call or insert, and adding it reddened the full suite (the filtered command could not see it). That invariant is STRONGER than the payload assertion the plan proposed -- vault.ts is the module holding raw text, so it is log-free BY CONSTRUCTION; a shaped payload is one careless edit from carrying doc.title, an absent call site is not. Audit removed; the CALLER audits (vault.searched in llm.ts is the precedent) and 26-13 now owns a refs-only vault.promoted row. ALSO CORRECTED IN PLACE: the plan own mutation check 'drop origin from ownedDocsMeta -> typecheck reddens' is FALSE -- the field is `origin?: string`, so omitting it is legal TS and tsc exits 0; only the behavioural test catches it. **A `?:` field can never be mutation-checked by the compiler.** ALL FIVE OWNER OPEN QUESTIONS CLOSED THIS SESSION: (1) vault.md parked 18-09 question is CLOSED -- the note banned BLANKET ingest, not an explicit per-row promotion; that block is REWRITTEN and ADR-025 records it. (2) CONT-01 AMENDED, sent mail reassigned to RPRT-01 -- as ratified it could never be checked off since 26-11/12/13 all exclude the lane. (3) reel provenance landed here, pulling media.md in as the FOURTH playbook. (4) the cockpit searchVault fence closed -- and it made the plan SMALLER, one fewer query and one more door shut. (5) one promotion surface confirmed; 26-12/26-13 plan text edited to call it directly and never re-wrap the guard. NOT DONE: no apps/web file -- the mutation is API-only, 26-13 builds the control and rollback means deleting the export. One UNIDENTIFIED FLAKE recorded rather than hidden: an early filtered run reported 1 failed/650 passed without the name being captured; three re-runs clean at 651 and the full suite green. Still open, not introduced here: two red finance tests from 26-10, 26-VALIDATION rows 26-06/07/08. Next: 26-12 (wave 9, the Content union / pagination / signed-URL adapters)."
last_updated: "2026-08-22T03:40:00.000Z"
progress:
  total_phases: 53
  completed_phases: 33
  total_plans: 412
...
milestone_name: - Platform -> Private Beta
current_phase: 26
current_plan: 11 of 20 executed (26-01..26-10 + 26-18); next 26-11, wave 8
status: executing
stopped_at: "26-10 CLOSED 2026-08-22 — owner UAT APPROVED, and it earned its keep by finding a real defect. THE FULL ARC: the plan was unexecutable as written (a 21-agent audit found four blockers: the UAT it demanded had already happened 2026-08-09; two demanded states cannot exist - the 'managed by the operator' copy is unreachable markup and there is NO runtime presentation kill-switch; the run was SINGLE-USE because bootstrapOwner had no inverse; and its verify command is the non-filtering 'test:e2e --' form). owner:revokeOwner was added to close blocker 3, the plan was rewritten against evidence, and the gate then RAN. GREEN: 'connected cost console: coverage, rails, unlanded meaning and the owner boundary' - coverage renders Unknown never $0, movements land once with the replay deduped, the two unlanded sentences differ, the clock is UTC, a non-owner sees no Operator tab and NOT ONE deployment ceiling in the DOM, and finance:controls refused live with OWNER_REQUIRED. THE UAT FOUND A DEFECT AND IT IS FIXED: the Cost Console clipped its own copy at 390x844 on ?tab=spend. Root cause was GRID INTRINSIC SIZING, not the tables - every container is display:grid with no explicit columns, so the implicit track is auto (sizes to widest item, refuses to go below) and grid items default to min-width:auto; the ledger widened its column to 441px inside a 339px main and every sibling stretched to 425px. The three scroller wrappers ALREADY had overflow-x:auto and were USELESS, because an element that cannot shrink never scrolls - it expands. Three properties, no markup: minWidth:0 on scroller, gridTemplateColumns minmax(0,1fr) on stack + the FinanceTabs page container + FinanceView's outer grid. Before 441/339 with 103 clipped; after 339/339 with 0 clipped (50 remain past the edge, all INSIDE the ledger scroller, which is intended). WHY NO ASSERTION CAUGHT IT, and this generalises: documentElement.scrollWidth stayed EXACTLY 390 because the content clipped rather than scrolled, so a page-level overflow check reports CLEAN on a visibly broken page - the first probe written for it did exactly that, and only a screenshot found the bug. The working metric is PER-CONTAINER (main.scrollWidth vs main.clientWidth) plus classifying each overflowing element by whether it sits inside a real horizontal scroller; recorded in dashboard-pages.md for every page there. FIVE SPEC DEFECTS were also found by running the E2E for the first time: getByLabel substring-matching the input AND aria-label='Save ...' (loose 2, exact 1); getByText matching two deliberate renderings; section 5 asserting literal dollar figures that are WINDOW TOTALS and accumulate (predicted to the cent by the audit - the tenant hit 10 spendEvents rows and failed exactly there, now baseline+delta); page.content() snapshotting owner ceilings before globalRails resolved, catching 'Loading cost...'; and MY OWN revokeOwner placed in seedOnboarded, which the boundary test never calls - moved to beforeAll, because a helper three of five tests call cannot carry a file-wide precondition. THE IDENTITY IS NOT THE OBVIOUS ONE: .env's user_email is the owner's GOOGLE row with no password credential, so /signin can never authenticate it; the run used joel.feruzi+phase21b@gmail.com, a password-auth NON-OWNER from the phase-21b invite path - so the deployment owner was never demoted. SC#7 DEVIATION recorded not normalised: nav went live 2026-08-09 on owner direction with the run and three UAT items outstanding, which is also why 26-18 closed before 26-10. ALSO THIS SESSION: production had NO human promotion gate (environment protection needs a PAID plan on a private repo - both rules 422'd), so deploy-production now requires the merge SUBJECT to START with [deploy]; the first version used contains and NEARLY DEPLOYED ITSELF when a merge body said 'No [deploy]-marker'. media.test.ts's scheduled-tail drain was a no-op (convex-test registers a job in-flight INSIDE its setTimeout callback, so finishInProgressScheduledFunctions returns having done nothing) - fixed with a macrotask yield, 17 live jobs at test end before, 0 after. PR #20 CLOSED: its content was already on main (git cherry: both code commits upstream; 437/444 lines present). STILL OPEN: two red finance tests characterised in 26-10-SUMMARY (one NOT explained), the rollback gap (no runtime presentation switch), 26-VALIDATION rows 26-06/07/08. Production NOT deployed since 24ad7f1 @ 2026-08-21 14:36; the only runtime delta waiting is owner:revokeOwner. Next: 26-11 (wave 8, artifact provenance and idempotent promotion)."
...
milestone: v2.0
milestone_name: - Platform -> Private Beta
current_plan: 8 of 8 (phase 21 CLOSED)
status: phase_complete
stopped_at: "PHASE 21 IS CLOSED 2026-08-18 - three of three success criteria proven LIVE, $0.00 spent this session. The gate 21-07 bought (de976d8e, 41/41, $0.4947) was READ, never re-run. PASSED: non-owner refusal (OWNER_REQUIRED, request c868a28876ec1d28) with owner:viewer {isOwner:false} captured in the same batch and zero state change; owner activation (current-effective moved to the tenant row qx73bwsh... v12 hash aee0008c..., baseline archived, global + foreign unmoved); exact-baseline rollback (current-effective = qx73cg6g... v1 hash 4b6a29f9..., candidate archived with hash/evidence/self-target intact, requiredEval FALSE - the structural exemption is only observable by noticing nothing was purchased). **NOT DONE AND NOT DEFERRABLE: plan 21-08 steps 3-4, tenant runtime attribution.** The author tenant kn790hj6... is e2e-wave6@pikar.test, a SYNTHETIC row - no password anywhere in the repo (full-tree search: 22 hits, all prose), no reset flow, and smoke.ts has no entrypoint that runs the agent loop for an arbitrary tenant. A specialist turn is a PAID model call (costUsd 0.00256..0.0541 on sibling subagent.completed rows), so the plan truth every step costs $0 is FALSE for 3-4. The only rows attributing a run to qx73bwsh... belong to eval-de976d8e / eval-a88a4597, the harness per-run tenants, and userSkillRuntimeAttribution re-checks tenant equality so A returns null correctly. 21-LIVE-RESULT.json was deliberately NOT written - Task 3 rejects a partial object by design and a half-filled artifact reads as a pass. **A SECOND IDENTITY NOW EXISTS**: kn735m0cmfyf8hx7khygbqjwx18cqndt, joel.feruzi+phase21b@gmail.com, non-owner, admitted through the BETA-01 invite door (requestAccess -> owner approve at /admin -> /signup redemption, redeemedSubject password|... provider-qualified). The invite is SPENT. **IDENTITY TOPOLOGY, verified not assumed:** the ONLY loggable human account before today was the owner kn73kmc... (joel.feruzi@gmail.com, Google, users.owner true); every other row is synthetic. Phase plans that say as non-owner user B assume a world that does not exist - and 21-08 inverted the roles, calling the OWNER tenant the foreign tenant B. **PLAN BUG worth not re-hitting:** 21-08 Task 1 verify does $H=path; $h=ConvertFrom-Json - PowerShell variable names are CASE-INSENSITIVE, so the path is destroyed and compare-refs dies with ENOENT on @{schema=...}. **PRODUCTION IS UNTOUCHED AND DIVERGENT:** all Phase-21 rows are LOCAL; prod tenantSkills is EMPTY, prod has NO owner row (bootstrapOwner never ran there, so /admin correctly refuses), the /admin route is not in origin/main at all (51 commits behind), and prod therefore still runs OPEN SIGNUP on a metered key. Counters incremented, NOT re-derived. Next: the steps 3-4 re-cut (three options in 21-LIVE-PARTIAL-2026-08-18.md), or the ACTIVE LANE stays Phase 25."
...
milestone: v2.0
milestone_name: - Platform -> Private Beta
current_plan: 10 of 10 (phase 33 CLOSED)
status: phase_complete
stopped_at: "PHASE 33 IS CLOSED 2026-08-18, 10/10, owner browser gate PASSED. Verdict verbatim: 'it all works two storyboards we created and i viewed them and not error occured'. OBSERVED: two storyboards produced from one brief, both viewed, no error - the FIRST time the VARIATION A/B contract has been honoured in production (every deck persisted before this carried variations:1). NOT individually adjudicated and NOT claimed: brief chips, the switch control, confirm-blocks-Generate, hero layout, estimate breakdown - covered by the Playwright spec (41b5dd1) but carrying no owner verdict. **THE PREVIOUS stopped_at WAS FALSE AND IS CORRECTED HERE: it claimed THE LIVE DEPLOYMENT STILL RUNS THE OLD BODY, no seedSkills ran, no pointer moved. Read back from prod 2026-08-18: ACTIVE media-director is VERSION 5, 20041 chars, byte-length identical to packages/contracts/skills/media-director.md, teaching VARIATION A/B, BRIEF, Target duration and Source:.** Source cannot see deployment state - always read the live row. CLOSING THE PHASE TOOK TWO PRODUCTION DEFECTS NO OFFLINE GATE COULD FIND, both shipped: (1) ac6a4d5 - an over-running narration line now BUYS seconds by trading duration between scenes (seconds never words, never a generated_video at either end, total never moves, every second disclosed as a SceneAdjustment); (2) 8ad8b51 - **A BRIEF IS A SUBJECT, NOT A TASK.** buildSpecialistPrompt QUESTION branch was built for RESEARCH where the question IS the task; runMedia reused it (llm.ts passes question: brief), so a media specialist s entire user turn was the executive s free-text brief capped at 500 chars with NO instruction to produce the deck. Identical brief produced a 5-shot deck on one run and 1270 chars of prose on another, same skill v5, same body hash 4f41120c, incomplete:false both times. MEDIA_TASK_LINE (route===media only) is the fix; ONE root cause explained FOUR symptoms - no_deck coin flips, bad_target_duration on a 6331-char deck, variations:1 on every deck ever persisted, and no injected target duration. Also shipped c7f0d2c audit.recentByType, the read surface that made all of this diagnosable in four CLI calls with no browser session, and plans.refusedBody so the next refusal is READ not inferred. STILL UNOBSERVED: what those 1270 chars actually SAY - do not write the inference down as fact. DEPLOY NOTE, CORRECTED: the branch was red mid-session and is GREEN again as of 4ef7646 - and the root cause was the GUARD, not the code. The section-5 scan read JSDoc backticks as template literals, so a745d36 merely EXPLAINING ITSELF IN COMMENTS took cockpitCapabilities.ts from 1 backtick to 29 and tripped a false positive; stripComments() fixed it at the root. Ship by scoped-branch cherry-pick anyway - this branch carries the BETA-01 invite gate, and merging it to main would close production signup as a side effect (ADR-020). Counters below are carried forward, NOT re-derived. Next: THERE IS NO PHASE 34 - phase 33 is the last numbered phase on the roadmap. The ACTIVE LANE IS PHASE 25 (private beta productionization, 14 plans unchecked). Milestone close is gated on the GAP LEDGER Group C owner actions, not on starting a new phase."
last_updated: "2026-08-18T02:10:00.000Z"
progress:
  total_phases: 53
  completed_phases: 32
  total_plans: 412
...
milestone: v2.0
milestone_name: - Platform -> Private Beta
current_plan: 9 of 10 (phase 33)
status: completed
stopped_at: "Completed 33-09-PLAN.md - media-director v3 is AUTHORED AND PINNED, AND IS NOT LIVE. The body now teaches the three phase-33 contracts the parsers have read since 33-01: a BRIEF echo (topic + duration required, everything else optional, `(defaulted)` on any value taken from the business profile rather than from the user's own words - the Phase-11 thin-profile rule, so a sparse account is never handed invented specifics); VARIATION A and VARIATION B, each a WHOLE proposal with its own script, art direction, deck and prompts, both running the brief's duration, A being the deck the owner sees picked; and a `Source:` line on every scene that states a checkable fact - `<title> [doc:<id>]` from a document searchVault actually returned, or `Source: unverified`, with creative copy carrying NONE. THE BODY DELIBERATELY TEACHES THAT THE TITLE IT WRITES IS A LABEL, NOT THE LABEL: when the id names a real document the owner is shown the VAULT ROW's own title (the fix the orchestrator landed between waves 8 and 9, d69fc29) - teaching otherwise would invite a model to compose a source line for a UI that was never going to render it. STRUCTURAL CHANGE WORTH KNOWING: the two partial fenced examples (a deck fragment in the old section 3, a prompts fragment in section 4) were REPLACED BY ONE WORKED ANSWER at the end of the file, and that answer is what storyboard.test.ts parses - with parseVariations FIRST, exactly as persistStoryboard does, then every per-deck rule against BOTH decks. Variation B going unchecked would have been the same hole one level up. media-director is UNGATED - seedSkills publishes at maxVersion+1 straight to active, with NO eval between the prose and production - so the round trip is the ONLY pre-live gate, and it was mutation-checked three ways: the v2 body reddens 17 of 99, dropping the one `Source: unverified` line reddens 1, and giving variation B a different Target duration reddens 15. packages/contracts/src/skills/mediaDirector.ts was regenerated in the SAME commit - the Convex runtime cannot fs.read repo files, so the derived .ts is what SHIPS and skillBodies.test.ts holds them byte-identical; there is no committed generator (regenerate with a throwaway JSON.stringify of the .md, then biome check --write, which prefers double quotes). THE LIVE DEPLOYMENT STILL RUNS THE OLD BODY: no seedSkills ran, no candidate was inserted, no pointer moved. 33-10 owns the seed AND the read-back - optimizer dry-run candidates occupy version numbers, so the version this lands at is not predictable from the plan. Measured: @pikar/core 1010 passed across 39 files (storyboard.test.ts 92 -> 99), backend media.test.ts + dispatch.test.ts 324 passed | 24 skipped, contracts skillBodies.test.ts 22 passed, tsc --noEmit clean in core and contracts, biome clean on the three touched code files. No audit or log-plane site was added, so llmRedaction.test.ts's media count pins are untouched. STATE.md still carries six accumulated frontmatter blocks, both top blocks hand-edited. DO NOT RUN state advance-plan ON THIS FILE. Next: 33-10 (seed the body, read back the active version, and own the live browser gate)."
last_updated: "2026-08-16T17:10:00.000Z"
progress:
  [███████░░░] 70%
  completed_phases: 31
  total_plans: 412
...
> scope. `total_plans`/`completed_plans` are carried forward UNVERIFIED: Phase 20.2 holds its whole
> phase in one `20.2-PLAN.md` with no per-plan summaries, so whether its work is already inside the
> 365 cannot be determined from the artifacts. Do not treat 365/268 as re-derived this session.
...
milestone_name: - Platform -> Private Beta
current_phase: 20
current_plan: 0 (phase 19.1 CLOSED 2026-08-11 — 7/7 plans, verifier 11/11 passed, both gaps fixed; phase 20 not started)
status: phase_complete
stopped_at: "**PHASE 19.1 IS CLOSED 2026-08-11. THIS SUPERSEDES THE THREE-OPEN-ITEMS HEADLINE BELOW.** After the owner gate closed, gsd-verifier returned gaps_found 9/11 naming TWO of the three items as real, and the owner directed a direct fix rather than a --gaps cycle. Commit 5746523 closed both: the done screen now sums server + browser rejections (the arithmetic moved INSIDE a new exported ImportDone so a RENDER test covers it -- a source scan would have been mechanism coverage, which is the trap this repo keeps falling into), and disabledLook (globals.css's own .vault-button:disabled values, reused not invented) is spread at BOTH disable-able primary sites -- import-choose included, which the gate had NOT flagged. Re-verification is 11/11 passed with both closures confirmed by revert-and-observe; report at 19.1-VERIFICATION.md. **ITEM 3 (the consent-chip side-by-side) WAS CLOSED BY OWNER DECISION ON THE Origin COLUMN, NOT BY OBSERVATION** -- the data-layer facts are asserted via consentRecord and the non-downgrade floor, but the two chips have STILL never been seen beside each other, and the verification report says so in those words. **A VACUOUS TEST WAS CAUGHT BY MUTATION AND THE SWEEP FOUND TWO MORE:** asserting toContain('>1<') stayed GREEN with the fix removed because another count was also 1; the verifier then proved that transposing enriched/rejected and needingAttention/followUpsDue BOTH left the suite fully green. Commit 921cc5d anchors every rendered number to its label (<strong>N</strong> label, plus a tileValue(html,id) helper for tiles). **THE GENERAL RULE, now in contacts-crm.md: to find this class, mutate by SWAPPING two rendered values, not by deleting one** -- deletion makes a value absent and almost any assertion notices, while transposition keeps every number present and only moves it under a different label. **gsd-tools phase complete 19.1 WAS RUN AND ITS WRITES WERE REVERTED -- DO NOT RE-RUN IT.** It reported roadmap_updated:true while REPLACING the ROADMAP's 7-wave rationale and the entire progress row with generic stubs, and it rewrote this frontmatter from a stale source: DROPPED current_phase, set status:verifying, set stopped_at to 'Completed 19-02-PLAN.md' (phase 19-02!), and DECREMENTED completed_phases 32 -> 31. Both files were restored byte-identical from snapshots taken beforehand, then hand-edited. Prior record follows. 19.1-07 COMPLETE -- **PHASE 19.1 IS 7/7 AND THE OWNER GATE IS CLOSED. A REAL BROWSER HAS IMPORTED A REAL CSV.** Owner verdict at the blocking checkpoint, VERBATIM AND COMPLETE, one word: `approved`. Nothing else was said; do not attribute reasoning or wording to the owner that they did not write. **THE THREE FINDINGS PRESENTED AT THE GATE WERE NOT INDIVIDUALLY ADJUDICATED AND REMAIN OPEN** — not owner-accepted, not owner-waived, not owner-approved-as-is. **WHAT SHIPPED:** `apps/web/e2e/pipeline-uat.spec.ts` `step 3b`, placed immediately after step 3 (serial mode makes ordering the contract, and step 3 is what creates JANE and the NAMELESS contact so ONE file produces all four counts). It uploads `FIXTURE_CSV` through the panel own file input with `setInputFiles`, sees `2 new · 1 enriched · 1 unchanged · 1 rejected` asserted as an EXACT STRING, sees the malformed row reported at PHYSICAL FILE LINE 7 (record index 6 — the quoted newline on line 4 is what makes them diverge), observes Confirm `disabled` BEFORE the tick and enabled after, ticks, confirms, and finds the rows in the table with `Imported from a file` and `· imported` on the imported one. **THE FIXTURE IS AN INLINE TS CONSTANT, NOT A COMMITTED .csv** — `core.autocrlf=true` + `* text=auto` would normalize a committed file CRLF and can strip its BOM, which are two of the five hazards it exists to prove. **THE ORACLE IS STORED STATE, NEVER THE PREVIEW** (the preview is the thing under test): `listContacts` proves Jane name is still byte-for-byte `Jane Doe` and her origin is still `user-entered`; `matchExisting` proves her post-write emptiness is EXACTLY `[phone, title]` so the company was filled and nothing was invented; `consentRecord` proves `imported-attested` plus the wording compared against the IMPORTED `IMPORT_ATTESTATION` constant and the context the browser typed. Step 14 now also opens the import preview at 390px. Both playbook gates bumped (`cockpit.md` WATCH-GATE ONLY — it watches `apps/web/e2e/`; `contacts-crm.md` carries the closing invariants and every ceiling in one place). **MEASURED: the full suite 16/16, spend $0.0500 across 5 events, run twice green; all UAT artifacts regenerated from ONE 16/16 run.** Two mutation-proofs red and reverted: expecting `Line 6` gives `Received: \"Line 7: not a usable email address\"`; expecting `Jane D.` gives `Received: \"Jane Doe\"` FROM STORED STATE. core 813/813, web 176/176, contracts 31/31, backend 72 files/1525 passed, backend+web `tsc --noEmit` exit 0 each (`--listFiles` confirms the spec IS inside the web typecheck scope), `check-playbooks` exit 0, biome clean. **THREE OPEN ITEMS, CARRIED FORWARD, NOT FIXED.** (1) BRAND/a11y: `import-confirm` really carries `disabled` before the tick but is styled from the inline `primary` object which has NO disabled variant, so the closed gate looks exactly as pressable as the open one. (2) **USER-VISIBLE TRUTHFULNESS DEFECT: the preview says `1 rejected` and the done screen then says `0 rejected` for the same file** — both individually true (the client drops the bad row before batching so the SERVER rejected none) but it reads as \"it got in after all\", in a phase whose whole thesis is that the counts do not lie. THE FIX IS ONE EXPRESSION: report `result.rejected.length + mapped.rejected.length` in `ImportPanel.tsx` done branch (`mapped` is still in scope there). DELIBERATELY NOT APPLIED — that file belongs to 19.1-06 and an out-of-scope product edit made while closing a checkpoint is how a gate stops meaning anything. FIRST ITEM OF ANY FOLLOW-UP. (3) The consent-chip comparison COULD NOT BE PERFORMED: all four UAT rows read `Consented … · imported` because none carried consent before the import and one attestation covered the file (correct behaviour); the column that actually distinguishes them is ORIGIN. Nothing was seeded. That judgement stands unmade. **TWO ENVIRONMENT FAULTS, SEPARATE SYMPTOMS, BOTH DOCUMENTED IN `contacts-crm.md` AND THE VALIDATION MAP.** (a) `packages/backend/.env.local` names the CLOUD `dev:woozy-wren-368` while the browser talks to the LOCAL `:3210`, so every harness seed lands where the browser cannot see it and THE WHOLE SPEC — including all 15 pre-existing steps — fails at the onboarding gate with nothing naming the cause. Symptom: `onboarding.status` returns `needsOnboarding: true` for a tenant `__seedOnboardedTenant` just reported writing. Worked around per-run with `CONVEX_DEPLOYMENT=\"local:local-joel_feruzi-pikar_ai_50c69-1\"`; the file was NOT edited (a foreign lane owns it). Two probe seeds were written to the cloud dev deployment before the cause was found — recorded, not hidden. (b) The local deployment `SITE_URL` pointed at `:3000` with nothing listening, so sign-in SUCCEEDED and only the final redirect hop died (dead `http://localhost:3000/dashboard?code=…`); corrected to `http://localhost:3111`, which is what the setup notes specify and what `convex/http.ts` falls back to. Fixing either fault does not fix the other. **ALSO RECORDED:** `pnpm test`/`pnpm typecheck` as single commands OOM at 0.43 GB free of 7.88 GB (exit 134) — re-run per package with `--maxWorkers=1` + `NODE_OPTIONS=--max-old-space-size=3072`, all green; one intervening full run flaked at step 7 (a real-model step, no import code) with two 16/16 runs bracketing it; and the plan own verify `--grep \"step 3b\"` CANNOT PASS ALONE (it needs step 3 contacts) — use `--grep \"step 3\"`. **SHARED TREE:** task 2 four files were staged when the concurrent 21-01 lane committed the whole tree, so they landed inside `f919dfe` instead of under this plan message; content verified intact and shared history was NOT rewritten. `.planning/REQUIREMENTS.md` NOT touched (phase-25 lane owns it)."
last_updated: "2026-08-11T01:50:00.000Z"
progress:
  total_phases: 52
  completed_phases: 32
  total_plans: 365
...
|------|-------|----------|------|-------|
| C19 | **19** Contacts, CRM & Follow-ups | 13/13 plans on disk (10 planned + 19-11/12/13 gap-closure) | **owner JUDGEMENT sign-off on the 7 UAT PNGs — a human act, not a run** | **CURRENT (19-13, 2026-08-10): CODE COMPLETE AND BROWSER-VERIFIED.** `cockpit-agent@18` IS ACTIVE (activated 2026-08-09 on owner authorization). ACTN-05 WORKS: the cause of the 19-10 defect was a dropped trusted clock (19-11) and a browser that never sent `clientContext` (19-12), not the model; fixture 36 is green for the right reason at run `0b2b6b22`. **Owner UAT RAN at 19-12: `e2e/pipeline-uat.spec.ts` 15/15, $0.0400, 7 PNGs on disk.** **19-13 closed the verifier's last code gap** — `consentWording`/`consentContext` were write-only; `contacts.consentRecord` is the bounded tenant-scoped request path (64/64, both guards mutation-proven red-able, no UI surface deliberately) — and corrected five falsified documents, the `gmail.ts` “sole caller” lie (there are TWO callers) and deleted the permanently-red `e2e/pipeline.spec.ts`. Backend 1448 passed, typecheck 8/10 with the finance lane's `cash.ts` the only red, check-playbooks exit 0, **$0.00 spent**. ACTN-05/PIPE-01 still Pending in `REQUIREMENTS.md` — the owner ticks them. **EVERYTHING BELOW THIS POINT IS 19-09-ERA HISTORY AND PARTS OF IT ARE STALE; read it as a reasoning trail, not as status.** ~~**19-09 COMPLETE — GATE GREEN, NOTHING ACTIVATED**~~ (`f5e527f`, `53254e7`, `8a0612b`, `e3baaa4`, `497f8f8`, `ff59e09`). **Gate `086f8267`: 35/35, zero retries, $0.3505, on `cockpit-agent@18`** — evidence stamped on the v18 row, so it satisfies `EVAL_GATE` and is one click from live. ~~**That click was withheld: `cockpit-agent@17` IS STILL ACTIVE, so the model still cannot see `stageCrmWrite` in production.**~~ **SUPERSEDED 2026-08-09: v18 WAS activated on owner authorization; it is the live body and the model does see `stageCrmWrite`.** v18 was hash-verified before spending (28 368 chars, sha256 `6ca4d937639c…`, maxVersion+1 from a clean 17-max). **FIXTURE 36 FAILED ITS FIRST LIVE RUN TWICE for $0.0115 and THE FIXTURE, NOT THE BODY, WAS REPAIRED** — the agent composed an email every time (`recipients`, a `subject`, no `crmOperations`) because turn 1 paired the outreach verb `chase` with an inline address; rewritten in the records grammar turn 2 already used, it passed first try ($0.0071). The body already forbade the behaviour verbatim, so a third prohibition would have bought a third identical failure. No assertion weakened; turn 2 byte-identical; the address deliberately KEPT (it is what gives `recipientCount: 0` teeth, and needles are leak canaries that must appear in a turn). ~~**OPEN: the passing run stages `addContact` with no `due`, NOT `addFollowUp`.**~~ **CLOSED: 19-10 added `datedFollowUpCount` to the CLOSED `EXPECT_KEYS`; 19-11 then fixed the CODE (the dropped clock), not the fixture, and fixture 36 is green for the right reason at run `0b2b6b22`.** **BUDGET OVERRUN OWNED: $0.3691 vs a $0.30 ceiling. A full gate is ~$0.35, not ~$0.12 — v17’s own `skills.evidence` records the prior gate at `costUsd: 0.357`, readable at $0.** `--list` IS NOT A FLAG; it falls through to a full PAID run — use `--self-check`. 20-12 and 20.1-01 must rebase on this body; the next `seedSkills` mints v19. Prior: **19-08 COMPLETE** (`b982c68`, `c9b42eb`, `c3cf060`) — contacts-first resolution + `stageCrmWrite`, all THREE registration surfaces in one commit, BOTH guards mutation-proven RED, and a fifteenth `SMOKE::agent::crm=` op at $0. `resolveContacts` checks saved contacts before Gmail headers and brings their open follow-ups; proven by counting `mailbox.searched` audit rows, and it still writes NO contact row (contacts-crm.md invariant 16). `stageCrmWrite` stages and applies nothing; agent may only ADD; `due` is words through parseSendTime; refuses over a live draft. The tool is NOT yet visible to the model — 19-09 owns the `cockpit-agent` body edit. NEW: dropping a SMOKE-registered tool's schema literal ALSO breaks `tsc`. Prior: **19-07 COMPLETE** — the connected Pipeline route. **19-06 COMPLETE** (`ccad3ce`, `5bce978`) — RESUMED after a session kill (871 uncommitted insertions, zero commits; nothing discarded). `crm_write` is the FIFTH `ACTION_TYPES` member and the `inline` arm's SECOND occupant, landed across **FOURTEEN** registration sites in ONE commit — the plan said eleven. The three it missed: `approvals.ts` planKind and the ApprovalsView trio (both compile errors) and **`llm.ts` buildAgentContext, which is SILENT and whose comment falsely claimed otherwise — a new action type not added there is announced to the model as an email.** The 14-site checklist is now in `cockpit.md`. All-or-none and double-approve are free (one serializable mutation + the existing CAS), asserted not assumed. Pitfall 1 mutation-verified: dropping the `patchPlan` mirror literal alone is 2 RED with every typecheck still green. `contacts.ts` refactored to ONE implementation of the identity/upsert rule. Prior: **19-04 COMPLETE** (`f4ee077`, `9cb5fed`, `2e641b0`) — the phase's ONLY public unauthenticated route. `http.ts` routes six and seven: `GET /unsubscribe/<raw>.<hmac>` renders an inert signed landing page and `POST` on the same `pathPrefix` is the only mutating verb. **THE GET NEVER WRITES, and the test proves it by COUNTING `suppressions` ROWS before and after, not by reading the response** — a handler that suppressed and returned the identical HTML passes a status-only check; mutation-verified RED by adding the `runMutation` to the GET, then reverted. `pathPrefix`, never `path` — Convex's router has no `*` glob (the 20-06 lesson, third use). Only **200 and 404** leave the route; one bare 404 for every rejection, so a stale-but-well-formed token and a malformed one are indistinguishable from outside. **NO second env guard** (`verifyUnsubToken` holds the only one; a second makes it vacuous — playbook invariant 8) and **NO rate limiter** (the operation is an idempotent upsert behind an HMAC-SHA-256 digest; a ceiling would only throttle people legitimately pressing the button — invariant 10). Both refusals are argued IN SOURCE, not left as absences. The page is inline-styled from BRAND §2 hex because an httpAction cannot import `globals.css`; the `ponytail:` note names the `apps/web` middleware+bearer-hop trade as the upgrade path. Echoed addresses are HTML-escaped — the signature proves WE minted the token, not that a tenant-authored recipient string is safe to interpolate. **`UNSUBSCRIBE_SECRET` IS NOW SET ON THE LOCAL DEPLOYMENT** (32-byte hex, `npx convex env set` from `packages/backend` with `CONVEX_LOCAL_BACKEND_STARTUP_TIMEOUT_SECS=180` — the default 30 s never starts the local backend here); **a hosted deployment still has none, and unset means EVERY SEND IS REFUSED, not merely that links 404.** `contacts.test.ts` 40 → 48; full turbo test 9/9 (backend 72 files / 1368 tests), `pnpm typecheck` 10/10 exit 0, `check-playbooks` exit 0. **19-10 UAT OWES THIS PAGE AN EYEBALL — nobody has ever rendered it.** No codegen needed (existing module, no schema change). **19-02 COMPLETE** — `convex/contacts.ts`, the person store (see the frontmatter block). **19-01 COMPLETE** (`0abc73b`, `38ac3d2`, `a78a169`). The substrate: `packages/core/src/contacts.ts` (`normalizeAddress` = trim+lowercase and NOTHING more, `needsAttention`, `followUpIsDue`, `renderFooter`), THREE tables (`contacts`, `followUps`, `suppressions`), `tenantProfiles.postalAddress` optional, and `docs/playbooks/contacts-crm.md` registered in `watch.json`. **EVERY later plan MUST import `normalizeAddress` — never write `.toLowerCase()` at a call site**; that single function is what makes "the guard and the contact row agree by construction" true. **`suppressions` is address-keyed and SEPARATE from `contacts` on purpose**: the send guard reads it and never `contacts`, so `contacts.unsubscribedAt` is a display mirror only and suppression outlives the contact. **THE BACKEND TYPECHECK BASELINE IS 0, RE-MEASURED — `npx tsc --noEmit` from `packages/backend` exits 0 with zero output.** Both the `13` in `19-VALIDATION.md` and the `150` elsewhere in this file are STALE; re-measure before quoting either. Full `pnpm test` 9/9 packages (backend 71 files / 1310 tests), `check-playbooks` exit 0, delta 0. **Schema-only changes need NO `npx convex codegen`** — `_generated/dataModel.d.ts` derives table types generically from `schema.ts`; codegen is only needed for a new MODULE. **A FOREIGN LANE'S WORK LANDED IN `38ac3d2`**: `git commit -m` commits the whole shared index, and the `cash-business-finance` lane had five files staged. NOT rewritten (a `reset --soft` while another agent commits to this branch risks losing their work). **From 19-03 on, use the pathspec form `git commit -m "…" -- <paths>`, which ignores the index** — `a78a169` is clean at exactly 2 files because of it. VALIDATION rows 2, 3 and 22 are green; ACTN-05/PIPE-01 stay Pending (nothing ticked — the 17.1-01 early-flip trap). **19-03 COMPLETE** (`5a72ac4` + `bcf825e`) — the CAN-SPAM postal address END TO END. `saveFacts` takes `postalAddress`, stores it TRIMMED, and refuses blank-after-trim (`EMPTY_POSTAL_ADDRESS`) and >500 chars (`POSTAL_ADDRESS_TOO_LONG`) **before any DB work**, so a refusal takes the whole write with it. **IT IS NOT AN ONBOARDING SLOT AND MUST NEVER BECOME ONE** — absent from `missingSlots`/`canComplete`, pinned by `tenantProfile.test.ts` "the onboarding-completeness result is BYTE-IDENTICAL with and without a postal address" (non-vacuous on both sides). Both readers already surface it: `get` (UI) and `forTenant` (the identity-less read 19-02's `footerFor` uses) — do NOT add a third reader. The typing surface is a `Postal address` textarea in `ShapePanel.tsx` (NOT `page.tsx` — the panel already owns `saveFacts`, so the plan's second file was unnecessary); refusals render inline through the panel's existing `factsError`, never `window.alert`/`confirm` (a browser modal blocks the Playwright specs). **BLANKING A STORED ADDRESS IS SENT, NOT SWALLOWED** — the UI sends `undefined` only when the box is empty AND nothing is stored; clearing a stored one is refused inline on purpose, because there is deliberately no delete-my-address path. `onboarding.md` carries the invariant + names the test. Verified: web typecheck + production build green, core 686/686 (incl. the `no tier control` folder scan that now reads the edited panel). ⚠ `gsd-tools state advance-plan` CLOBBERED the frontmatter block AGAIN (dropped `current_phase`, reverted to a stale `7 (done)`, flipped status to `verifying`) — hand-restored for the SECOND time in this phase. |
...
- Phase 33 added: Media creation UX overhaul (guided intake, storyboard variations, auto-assembled reel, reel-first canvas, clear failure retry, grounded citations)
...
Stopped at: 26-10 pre-flight — premise audit, owner:revokeOwner, spec made re-runnable + kill-switch guarded, 26-10 REPLANNED. Uncommitted; branch needs cutting off updated main.
