...
current_plan: 9
status: in_progress
stopped_at: "Completed 15.3-08-PLAN.md — document identity classification (VALT-12), built by a 13-agent workflow. `DOC_TYPES`/`DOC_TYPE_LABEL`/`isDocType` live in `packages/core` (NOT packages/vault — apps/web already drives a `<select>` off a core union, and a third local re-declaration is the defect that split caused before), with a TWO-DIRECTION compile bridge that was MUTATED both ways and genuinely fails, plus an `@ts-expect-error` `_MISSING_ARM` twin pinning label-table totality. `document-classifier` is DELIBERATELY UNGATED (run-eval-golden derives --skill from GATED_SKILLS and no fixture reaches vault ingest, so gating deadlocks it at v1); its enum literals live in the OUTPUT CONTRACT, never guidance prose. ONE workflow step at the single ingest convergence point (single-file uploads are classified too), cost FOLDED into the existing recordSpend. **CLASSIFICATION DEGRADES THE LABEL, NEVER THE DOCUMENT** — the try/catch is INSIDE `classifyDoc` because a throw at the step boundary exhausts the workflow retries and `onIngestComplete` marks the row `failed`, which would have failed EVERY ingest in an unseeded deployment. User-set wins as an ABSENCE: no branch anywhere writes docType/identityLine over `identityUserSet`. **THE VERIFY PASS FOUND THREE DEFECTS AFTER THE SUITE WAS GREEN.** (1) `identityLine` was the ONE field passed RAW from the model — docType was coerced, identityLine was not, and `jsonSchema()` without a `validate` fn does NOT check the model object at runtime, so a missing/non-string value hit `applyClassification`'s `v.string()`, which throws inside `step.runMutation`, OUTSIDE the try/catch, failing the document. **GENERAL LESSON: coerce one field of a model object and you must coerce them all — the uncoerced sibling is the one that fails the document.** (2) PreviewModal's seed guard was a BOOLEAN on a modal rendered without a `key`, so switching documents reused the instance and the form kept doc A's identity while showing doc B; Save would write A onto B. Now keyed by doc._id INSIDE the component. (3) A classified-but-still-`processing` row seeded an EMPTY form and Save then wrote identityLine:\"\" with identityUserSet:true, blanking and permanently LOCKING it. **THE INTERMITTENT `crypto is not defined` / `process is not defined` BACKEND SUITE FAILURE IS PRE-EXISTING AND NOT THIS WAVE** — reproduced with ONLY `onboarding.test.ts` + `vaultDigest.test.ts` (1 of 3 runs); it is a load-dependent edge-runtime instability where async work outlives the VM context. Two wrong hypotheses are recorded in vault.md so nobody re-tests them (it is NOT leaked fake timers, and stubbing globalThis.process does not fix it). A real fix means serialising file execution and doubling a 140s suite — an OWNER CALL. backend 1162/1162 (61 files) on a clean run, core 611/611, contracts 27/27, vault 165/165, apps/web typecheck 0, backend non-test delta ZERO, web build green, check-playbooks exit 0, $0 spent. Next is 15.3-09 (wave 9, the Google Drive rail) — the LAST plan of phase 15.3."
last_updated: "2026-08-04T06:05:00.000Z"
progress:
  total_phases: 42
  completed_phases: 28
  total_plans: 247
