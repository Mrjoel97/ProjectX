...
> Last verified: 2026-08-16 against `e4e402c` (25-01 — waitlist, owner-gated invites, and the
...
- `packages/backend/convex/invites.test.ts` — 27 tests, $0, the whole contract.
...
- **`@convex-dev/auth@0.0.94`, pinned exact (CLAUDE.md §6).** The security property depends on
  ...
  re-read both files and re-run `invites.test.ts` before trusting admission.**
...
| Command | Proves |
| --- | --- |
| `pnpm --filter @pikar/backend exec vitest run convex/invites.test.ts` | The whole contract, offline, $0. 27 tests. |
...
Mutation checks recorded at 25-01 (each reverted): dropping the `redeemedAt` single-use check
reddens 2; dropping the password code requirement reddens 2; dropping the provider qualifier from
`redeemedSubject` reddens 3.
