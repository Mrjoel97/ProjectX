...
> Last verified: 2026-07-19 (03.10-06) — **eval fixture 21 added (`21-fragment-answer-absorbed`) — the golden set is now 21 fixtures — and the runner now accumulates conversation history**: UAT-E (live 2026-07-19 second replay — conversational amnesia). Turn 1 ("Draft an email to Sarah Chen. Tell her I will attend the workshop.") deterministically parks candidates + drafts the body, leaving subject the ONE open slot the agent asks for; turn 2 is the bare fragment "Meeting reminder". `subjectPresent:true` is the anti-amnesia proof (without history the fragment is uninterpretable — the fixture FAILS against the historyless prompt; that is the point); `bodyPresent:true` proves turn 1's body was KEPT (no re-ask wiped it); `candidatesPending:true` + `recipientCount:0` + `statusAtMost:collecting` prove the stall stayed open and the recipient guard held. RUNNER CHANGE (production parity by construction): `attemptCase` accumulates `{role, content}` pairs from the turn text + the returned `res.reply` and passes them as the loop's `history` arg — the runner calls `llm:runCockpitAgent` DIRECTLY (never `sendCockpitMessage`, and `seedCockpitPlan`'s threadId is synthetic with no agent thread store behind it), so it must supply what the production drivers supply. History injection changes EVERY multi-turn fixture's turn-2+ context — the full-set green pinned run is the regression proof that history breaks nothing. `--self-check` untouched (fixture floor >= 18 holds at 21).
...
`pnpm eval:golden [--skill <name>@<version>]` runs 21 scripted natural-language
