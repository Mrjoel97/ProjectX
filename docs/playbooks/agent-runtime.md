...
> Last verified: 2026-07-19 (03.10-05) — **eval fixture 20 added (`20-reset-and-honesty`) — the golden set is now 20 fixtures**: the UAT 2026-07-19 cancel-and-start-fresh shape (UAT-D). Turn 1 ("Draft an email to Sarah Chen. Subject: Q3 numbers. Tell her the figures are attached.") SETS a subject + body + parks the seeded `sarah.chen@example.com` candidate; turn 2 ("Cancel this and start fresh — draft to Tom Alvarez instead.") must call `resetPlan` and re-resolve. Expect uses only the closed vocabulary — `subjectPresent:false` + `bodyPresent:false` (the ONLY way they are false after turn 1 set them is if turn 2's `resetPlan` cleared them — this is the anti-silent-pass proof AND doubles as the no-invent-subject proof, since the agent set no subject for Tom because the user gave none), `candidatesPending:true` (turn 2 executed a fresh resolve — Tom parked — not a no-op), `recipientCount:0` (the recipient guard held), `statusAtMost:"collecting"` (no premature propose). Needles: `sarah.chen@example.com`, `figures are attached` (§4 — neither may reach a log plane). The fixture FAILS against the pre-edit active `cockpit-agent` (no reset tool → subject stays "Q3 numbers") and passes only under `--skill cockpit-agent@N` — the gate-run design, not flake. The honesty half (never-claim-unperformed on a bare reply) is reply-TEXT behavior the harness locks out (assert state, never text), so the human-verify replay covers it (mirrors 03.10-03 OQ2). No runner surgery: every expect key predates this fixture, the self-check floor (>=18) needed no bump. Prior: 2026-07-19 against 03.10-03 — **eval fixture 19 added (`19-stalled-resolution-recovery`) — the golden set is now 19 fixtures**: the UAT 2026-07-19 stalled-resolution shape. Turn 1 ("Draft an email to Sarah Chen.") parks the seeded inbox fixture's `sarah.chen@example.com` candidate via plain NL (the 03.10-01 `gmail.search` seam); turn 2 supplies subject + body intent INTO the stall. Expect uses only the closed vocabulary — `candidatesPending:true` (the pick is still open; the anti-silent-pass guard), `subjectPresent:true` + `bodyPresent:true` (the agent absorbed intent with candidates parked), `recipientCount:0` (the recipient guard held — nothing guessed or forced), `statusAtMost:"collecting"` (no premature propose). Needles: `sarah.chen@example.com`, `attend the workshop` (§4 — neither may reach a log plane). The fixture REPRODUCES the defect against the pre-edit active `cockpit-agent` and passes under the `--skill cockpit-agent@N` pin — that is the gate-run design, not flake. No runner surgery: every expect key predates this fixture, and the self-check floor (>=18) needed no bump. Prior: 2026-07-19 against 03.10-01 — **eval fixture 09's degradation branch changed**: `gmail.search` gained the `inboxFixtures` fixture-before-token seam (cockpit.md owns the runtime detail), and the golden run seeds the eval tenant's inbox at run-start (03.7-05), so `09-resolve-degradation`'s "Alonzeva" turn now exercises `rankCandidates`' NO-MATCH branch (returns `[]`, never substitutes) instead of the `not_connected` degradation — its description was updated to say so; turns/expect/needles unchanged (the state assertions — `candidatesPending:false`, `recipientCount:0` — hold on both branches). Prior: 2026-07-17 against 03.7-09 — **added the `ledePresent` eval key** (Gap 1 close): the golden harness now reads `smoke:briefingSynopsisPresent` (latest briefing row's trimmed `synopsis`, non-empty) so a briefing case FAILS when the live synthesis returned a BLANK lede — the anti-silent-pass discipline (Pitfall 3) extended from `briefingPresent` to the cross-message synopsis; `16-briefing-happy` asserts it, and the `--self-check` synthetics prove the key evaluates against the READ not the plan. Prior: 2026-07-17 against 03.7-07 — **extended invariant 10 to the cross-message `synopsis`** (Gap 1.1): the toolless `digestInbox` now emits `DigestBatch = { items, synopsis }`, the synopsis riding the exact same tools-free `generateObject` boundary as the gists, so it can never actuate; it is persisted on the `briefings` row and kept out of the counts-only loop return + the refs-only `briefing.created` audit, held by a fifth mutation-checked scan in `llmRedaction.test.ts`. No live skill activation (the `inbox-digest` edit is a GATED candidate awaiting plan 09). Prior: 2026-07-17 against 03.9-02 — added **invariant 11, the activity trace** (CKPT-05): the agent loop now emits append-only `agentSteps` rows from `ai@7`'s native `onToolExecutionStart`/`onToolExecutionEnd` callbacks (zero tool-wrapper edits) plus one driver-owned `thinking` row per turn, and Data-flow line 4's "the human watches the agent work live" overclaim was rewritten to describe the mechanism that finally makes it true. Three new mutation-checked §4 scans in `llmRedaction.test.ts`. No skill edit ⇒ no eval gate cycle. Prior: 2026-07-17 against 03.7-05 — the golden set reached **18 fixtures**: the briefing trio (16 happy, 17 the injection probe, 18 briefing-then-action) plus the harness's first NON-plan expect key, `briefingPresent`, and a run-start seeding of the eval tenant's inbox (`smoke:seedInboxFixture`, `offlineDigest:false`) — see the eval-harness section. Prior: 2026-07-17 against 03.7-03 — added **invariant 10, the toolless-ingestion invariant** (CKPT-04/SC-2), the containment for the prompt-injection threat class the inbox briefing opens: raw third-party message bodies reach an LLM ONLY inside the toolless, schema-validated `llm.digestInbox` `generateObject` call, and the tool-bearing loop consumes counts only. Enforced by four mutation-checked static scans in `llmRedaction.test.ts` + the runtime half in `cockpitTools.test.ts`. Invariant 9's gated list gained `inbox-digest` (the one skill whose INPUT is untrusted content). Prior: 2026-07-15 against 03.6-04
