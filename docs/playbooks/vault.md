...
#### UI half — owner verification

**STATUS AT TIME OF WRITING: PENDING — NOT YET OBSERVED.** This block is written before the
owner's checkpoint so the CLI-half numbers above survive a session cut; it is replaced with the
verbatim verdict by 15.2-05 Task 4. If you are reading this sentence, the checkpoint did not
complete and **the fresh-upload and junk-file halves of SC#4 are unverified live.**

Known hazard for whoever runs it: the web server on `:3000` is `next start` (a **frozen production
build**) launched `2026-07-26 21:35:06`, while `.next` was rebuilt `2026-07-27 16:22:39`. A
`next start` process predating its own build is exactly the Phase-14 trap recorded in
`voice.md` — a UI symptom read as a code failure when the running bundle simply did not contain the
code. **Restart the web server before believing anything the vault UI shows about 15.2-04's
failure copy.**
