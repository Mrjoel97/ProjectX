...
### Live attempt on 2026-09-12 (644408df4c2c630346c5909aac23f9a352ede287)

Last verified: 2026-09-12 — production CI34692922592 and deployment34693161766 were confirmed before any acceptance mutation. The first live suite passed desktop1440/mobile390 channel, keyboard and document-fit checks (17.6s). Its next fresh-context scenario failed initial Marketing readiness before link creation, and the serial lead scenario did not run. A continuous-context harness rerun also failed readiness; closed diagnostics showed the correct www origin but a `/signin` redirect. The existing browser later redirected to sign-in on fresh navigation as well. Refresh-token replay is a possible explanation, not an established root cause. No full browser-suite pass or manual public-route acceptance is claimed.

The harness now preserves one actual BrowserContext through its serial scenarios, allowing native refresh state to carry forward, and waits30seconds for authenticated readiness with closed route diagnostics. This addresses stale per-test export reuse without swallowing failed assertions. Authenticated fixture preparation had already created exactly one controlled example.test contact with consent absent, `user-entered` Pipeline provenance and suppression; it is intentionally retained. No tracked link/token was created, so there was nothing to deactivate; no model/provider/send ran. The independently verified note-a fixture remains unchanged. Ignored `marketing-live-partial.json` and run logs distinguish every pass/failure/not-run result. The exported auth state was removed after the failed attempts; persistent browser profiles and owner/Intuit tabs remain. Fresh sign-in, the public byte/counter matrix, suppressed recapture and explicit navigation approval remain required.

### Fresh-session live acceptance and activation authorization

Last verified: 2026-09-12 — after fresh user sign-in, direct Playwright assertions in the preserved authenticated context passed desktop/mobile controls, one successful synthetic-link matrix, suppressed recapture/Pipeline provenance, and the unsent workspace handoff. Each public stage used a fresh empty anonymous context and returned302 to the same243-byte note-a original/hash; counts progressed `[1,0,0]`→`[1,1,0]`→`[1,1,1]`. HEAD405 did not count; public source tampering preserved the stored source and gave `[2,1,1]`. Deactivation/finally cleanup succeeded, all stages then returned empty404, and counts stayed fixed. Additional unknown/invalid/malformed routes returned identical no-store empty404. The list exposed no token recovery control after panel closure. The exact synthetic contact remained one suppressed user-entered row with no consent; it is intentionally retained. No sends/models/providers ran.

This direct existing-context method was authorized after exported-session restoration failed; the named three-scenario suite is still not claimed passing. The committed harness now locates the native combobox by a stable `Original Vault file` label prefix because wrapped select options contribute to its accessible name. It does not add runtime IDs solely for tests. Full historical failures, scoped before-unload handling, exact receipts, timestamps and runtime-equivalence across644408df/41d9551 are recorded in31-07-SUMMARY.md. The user subsequently replied **APPROVE MARKETING NAV ACTIVATION**. Navigation change/deployment and actual navigation clicks must still be qualified before final phase completion.

### Activated navigation verified

Last verified: 2026-09-12 — after the exact user approval, production`4df076dbdc8330c54ace5a3996259f3e71318024` passed CI34696845981/deploy34697141228 and durable production verification. Existing-context Playwright checks at13:48:12.226Z passed real desktop1440/mobile390 navigation from Vault by keyboard Enter and pointer click, active `aria-current="page"`, keyboard focus and document fit. Marketing is now visible in both navigation surfaces. The test link stayed deactivated at `[2,1,1]`, with no token panel or recovery control; original workspace state was preserved. See31-07-SUMMARY.md and ignored marketing-postactivation-result.json/screenshots for exact evidence. This verifies approved navigation without rewriting the disclosed standalone exported-session suite failures or claiming a fresh full-matrix rerun.
