...
- `packages/backend/convex/onboarding.ts` — thin adapter (§1): `status` (tenantQuery, first-run gate),
  ...
  **Every profile read in this module goes through the private `currentProfileDoc` helper, and that
  helper reads `vaultDocuments.by_tenant_kind` — never `by_tenant`.** Convex has no projection: a
  `by_tenant` query loads each row WHOLE, and this table stores the full extracted `text` of
  book-sized uploads, so scanning it to find one `kind` timed out the 1s query budget on `status` —
  the query the app shell subscribes to on every authenticated page. Same rule as the blueprint read
  plane states at "The blueprint read plane" below. If you add another profile-doc read, call the
  helper; do not re-derive the query.
