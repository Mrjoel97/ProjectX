...
> Last verified: 2026-07-29 — Phase 17.1 Wave 4 (plan 17.1-05, governed synthesis): `deriveCandidates` loads the active `business-blueprint` registry skill before every other branch, returns guardrail refusals as data, redacts the assembled index-parallel source block, and makes at most one strict-schema model call before recording actual spend. The offline `SMOKE::blueprint::` seam runs only after the governed prompt load and pre-call gate, so unseeded deployments still fail closed and tests spend nothing. See "Blueprint draft synthesis" below.
