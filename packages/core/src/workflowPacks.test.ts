...
describe("the workflow-pack registry is total", () => {
  ...
  test("the source vocabulary is disjoint, fully used, and fully described", () => {
    ...
    // …and no source is dead vocabulary: each is read by at least one pack operation, OR the
    // Phase-29 search plane RENDERS it. `KNOWLEDGE_SOURCES` is a named SUBSET of this registry
    ...
    //
    // THE SEARCH-PLANE ARM CALLS `renderSourceGap` INSTEAD OF SPREADING `KNOWLEDGE_SOURCES`. The
    // first repair spread the constant, which discharged a READ requirement with a DECLARATION —
    // the same shape this file rejects for tool grants. Membership in a second const is not a
    // read; producing a user-facing sentence that names the source is. The literal sentence for
    // `support-desk` is pinned in `knowledgeSearch.test.ts`.
    const renderedBySearch = KNOWLEDGE_SOURCES.filter((source) =>
      renderSourceGap({ status: "unavailable", source, reason: "not_landed" })?.includes(
