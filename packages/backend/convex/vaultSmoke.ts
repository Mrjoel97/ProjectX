...
export const seedCorpus = internalAction({
  ...
  handler: async (
    ...
    const briefs = [
      ...
      // ── THE THREE SPECIALIST DOMAINS. Added 2026-08-08 to kill fixture 29/30/31's `citesVaultDoc`
      // flake at its ROOT. Those fixtures assert that the needle reaches a specialist's memo, which
      // it can only do through a live `searchVault` — but the corpus above is a LAUNCH note and a
      // STAFFING plan, and says nothing about offers, pricing or lead channels. So an
      // offer-architect or lead-engine query honestly retrieved NOTHING, the memo correctly said so
      // ("the vault does not contain information on lead channels"), the needle never appeared, and
      // the fixture reddened. Pass/fail was therefore a coin flip on whether the specialist happened
      // to author a query close enough to "launch"/"staffing" — which is exactly the "retrieval
      // variance" cost.ts suspected but never confirmed.
      //
      // THIS DOES NOT WEAKEN THE ASSERTION. The needle still appears in NO fixture turn
      // (--self-check pins that), so it can still only arrive via a real retrieval. What changes is
      // that each specialist's own domain now HAS ground to find — the fixture stops testing whether
      // the model guessed a lucky query and goes back to testing grounding. Keep one brief per
      // specialist; deleting one re-opens that specialist's flake.
      {
        title: `Northwind offer ${needle}`,
        text: `${shared} offer definition: customers buy a done-for-you shipment onboarding package plus a monthly monitoring plan. The ${shared} offer is what a new customer actually purchases. Ref ${needle}.`,
        other: `${shared} onboarding package`,
        rel: "sells",
      },
      {
        title: `Northwind pricing ${needle}`,
        // Split literal: keeps each chunk under the §5 no-hardcoded-prompt scan ceiling (200 chars),
        // the same idiom the tool descriptions use. This is fixture prose, not a prompt, but the
        // scan is deliberately blind to intent — and a scan you can argue your way around is no scan.
        text:
          `${shared} money model: the monitoring plan bills monthly and customer acquisition cost ` +
          `is tracked per new customer, measured against lifetime gross profit. Pricing and margin ` +
          `for ${shared} live here. Ref ${needle}.`,
        other: `${shared} monitoring plan`,
        rel: "prices",
      },
      {
        title: `Northwind lead channels ${needle}`,
        text: `${shared} lead generation: warm outreach to past shippers, cold outreach to freight brokers, posted content and paid ads are the lead channels considered for ${shared}. Ref ${needle}.`,
        other: `${shared} lead channels`,
        rel: "acquires",
      },
    ];
